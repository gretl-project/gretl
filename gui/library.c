...
void open_info (gpointer data, guint edit, GtkWidget *widget)
{
	...
	int resp = 
	    yes_no_dialog(_("gretl: add info"), 
...
void do_lmtest (gpointer data, guint aux_code, GtkWidget *widget)
{
    ...
    view_buffer(prn, 77, 400, title, LMTEST, NULL); 
}
...
void do_run_script (gpointer data, guint code, GtkWidget *w)
{
    ...
    view_file(fname, 1, 1, 77, 450, SCRIPT_OUT);
}
...
void do_new_script (gpointer data, guint loop, GtkWidget *widget) 
{
    ...
    view_file(scriptfile, 1, 0, 77, 350, EDIT_SCRIPT);
}
		   ...
		   const char *myname) 
{
    ...
    rebuild = (exec_code == REBUILD_EXEC);
     ...
     /* for "session" use */
{
    ...
    view_file(cmdfile, 0, 0, 77, 350, EDIT_SCRIPT);
}
