...
static GretlToolItem scalar_items[] = {
    { N_("Add..."),  GTK_STOCK_ADD,    G_CALLBACK(name_var_dialog), 0 }
};
...
static void get_current_location (GtkTreeView *tv,
				  gint *row,
				  gint *col)
{
    ...
    if (col != NULL) {
	if (column != NULL) {
	    *col = get_treeview_column_number(column);
	} else {
	    *col = -1;
	}
    }
			       ...
			       GtkTreeViewColumn *column)
{
	...
	gint cnum = get_treeview_column_number(column);

	sprintf(sheet->location, "%d, %d", atoi(pstr) + 1, cnum);
	g_free(pstr);
    } else {
	GtkTreeModel *model = gtk_tree_view_get_model(sheet->tv);
...
static void start_editing (GtkTreeView *tv, gint row, gint col)
{
...
static void move_row_focus (worksheet *sheet,
			    ...
			    gint delta)
{
    gint next_row = current_row + delta;

#if CELLDEBUG
    fprintf(stderr, "move_row_focus -> row %d, col %d\n", next_row,
	    current_col);
#endif
    if (next_row >= 0 && next_row < sheet->n_rows) {
	start_editing(sheet->tv, next_row, current_col);
    }
}
...
static void move_column_focus (worksheet *sheet,
			       ...
			       gint delta)
{
    gint next_col = current_col + delta;
    gint colmin = 1;

#if CELLDEBUG
    fprintf(stderr, "move_column_focus -> row %d, col %d\n", current_row,
	    next_col);
#endif
    if (next_col >= colmin && next_col < sheet->totcols) {
	start_editing(sheet->tv, current_row, next_col);
    }
}
...
static void maybe_update_column_names (worksheet *sheet)
{
	...
	if (cnames == NULL) {
		...
		col = gtk_tree_view_get_column(sheet->tv, j + 1);
		if (col != NULL) {
...
add_treeview_column_with_title (worksheet *sheet, const char *name)
{
    ...
    cols = gtk_tree_view_insert_column(sheet->tv, column, -1);
    colnum = cols - 1;
				 ...
				 const char *vname)
{
    ...
    gtk_list_store_set(store, &iter, 0, vname, 1, "", 2, sheet->pbuf, -1);
    sheet->n_rows += 1;

    pstr = g_strdup_printf("%d", sheet->n_rows - 1);
    path = gtk_tree_path_new_from_string(pstr);
...
real_add_new_obs (worksheet *sheet, const char *obsname, int n)
{
	...
	for (j=0; j<n; j++) {
	    ...
	    get_obs_string(rowlabel, sheet->n_rows + j, dataset);
			  ...
			  GretlType type)
{
    ...
    if (buf == NULL || *buf == '\0') {
	return;
    } else if (gui_validate_varname(buf, type, parent)) {
	return;
    }
    ...
    *varname = 0;
	...
	err = gui_validate_varname_strict(varname,
					  GRETL_TYPE_DOUBLE,
					  sheet->win);
	if (!err) {
	    add_scalar_callback(sheet, varname);
	}
    }
}

static void name_new_series (GtkWidget *widget, dialog_t *dlg)
{
    name_new_var(widget, dlg, GRETL_TYPE_SERIES);
}

static void name_new_scalar (GtkWidget *widget, dialog_t *dlg)
{
    name_new_var(widget, dlg, GRETL_TYPE_DOUBLE);
}
...
static void name_var_dialog (GtkWidget *w, worksheet *sheet)
{
	...
	msg = g_strdup_printf(_("Enter name for new scalar\n"
			    "(max. %d characters)"),
			      VNAMELEN - 1);
        edit_dialog(0, _("gretl: name variable"),
		    msg, NULL, name_new_scalar, sheet,
		    VARCLICK_NONE, sheet->win);
    } else {
	msg = g_strdup_printf(_("Enter name for new series\n"
			    "(max. %d characters)"),
			      VNAMELEN - 1);
        edit_dialog(0, _("gretl: name variable"),
		    msg, NULL, name_new_series, sheet,
		    VARCLICK_NONE, sheet->win);
    }
...
static int add_data_column (worksheet *sheet)
{
    ...
    old_store = GTK_LIST_STORE(gtk_tree_view_get_model(sheet->tv));
    ...
    for (row=0; row<sheet->n_rows; row++) {
	gchar *str;
	int col;
    ...
    gtk_tree_view_set_model(sheet->tv, GTK_TREE_MODEL(new_store));
...
static void sheet_delete_scalar (worksheet *sheet, GtkTreePath *path)
{
    ...
    store = GTK_LIST_STORE(gtk_tree_view_get_model(sheet->tv));
    gtk_tree_model_get_iter(GTK_TREE_MODEL(store), &iter, path);
...
static void update_dataset_from_sheet (worksheet *sheet)
{
    ...
    int newobs = sheet->n_rows - sheet->orig_nobs;
    int missobs = 0;
    int i, j, t;
...
static void maybe_rename_sheet_cols (worksheet *sheet)
{
	...
	col = gtk_tree_view_get_column(sheet->tv, i);
...
static int rejig_sheet_cols (worksheet *sheet)
{
    ...
    gtk_tree_view_set_model(sheet->tv, GTK_TREE_MODEL(store));
	...
	n = -n;
	for (i=0; i<n; i++) {
	    col = gtk_tree_view_get_column(sheet->tv,
					   sheet->datacols + n - i);
	    gtk_tree_view_remove_column(sheet->tv, col);
	}
    }
...
static int update_sheet_from_matrix (worksheet *sheet)
{
    ...
    if (!resized && (sheet->n_rows > sheet->matrix->rows)) {
	int rdel = sheet->n_rows - sheet->matrix->rows;
    ...
    if (sheet->n_rows != sheet->matrix->rows) {
	sheet->n_rows = sheet->matrix->rows;
	resized = 1;
    }
...
static int add_scalars_to_sheet (worksheet *sheet)
{
    ...
    if (sheet->pbuf == NULL) {
	sheet->pbuf = gtk_widget_render_icon(GTK_WIDGET(view),
					     GTK_STOCK_DELETE,
					     GTK_ICON_SIZE_MENU,
					     NULL);
    }
...
#if SSDEBUG
    fprintf(stderr, " n_rows=%d, orig_vars=%d, orig_main_v=%d\n",
	    sheet->n_rows, sheet->varlist[0], sheet->orig_main_v);
#endif
...
static void create_sheet_cell_renderers (worksheet *sheet)
{
    ...
    /* immutable obs label or scalar name */
    sheet->textcell = gtk_cell_renderer_text_new();
    g_object_set(sheet->textcell, "ypad", 1,
		 "xalign", 1.0,
		 "background", "#DEDEDE",
			    ...
			    worksheet *sheet)
{
...
#if CELLDEBUG
    fprintf(stderr, "** on_sheet_click(): button %d\n", event->button);
#endif
	...
	gtk_tree_view_get_path_at_pos(sheet->tv,
				      (gint) event->x,
				      (gint) event->y,
				      &path, &column,
				      NULL, NULL);
...
#if CELLDEBUG
    fprintf(stderr, "on_sheet_click: returning %d\n", ret);
#endif
				 ...
				 int block)
{
    ...
    gtk_container_add(GTK_CONTAINER(scroller), GTK_WIDGET(sheet->tv));
    ...
    g_signal_connect(G_OBJECT(sheet->tv), "key-press-event",
		     G_CALLBACK(on_sheet_keypress), sheet);
    g_signal_connect(G_OBJECT(sheet->tv), "button-press-event",
		     G_CALLBACK(on_sheet_click),
		     sheet);

    window_list_add(sheet->win, SSHEET);
    gtk_widget_show(sheet->win);
    gtk_widget_grab_focus(GTK_WIDGET(sheet->tv));
				    ...
				    const char *name)
{
    ...
    view_buffer(prn, hsize, vsize, title, VIEW_MATRIX, m);
    g_free(title);
}
