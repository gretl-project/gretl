...
GtkItemFactoryEntry model_items[] = {
    ...
    { "/Tests/Chow test", NULL, gretl_callback, CHOW, NULL },
    { "/Tests/CUSUM test", NULL, do_cusum, 0, NULL },
    { "/Tests/ARCH", NULL, gretl_callback, ARCH, NULL },
		    ...
		    void (*no_func)(), gpointer noptr) 
{
   ...
   if ((yes_d = mymalloc(sizeof *yes_d)) == NULL)
       return;
   yes_d->data = yesptr;

   if ((no_d = mymalloc(sizeof *no_d)) == NULL)
       return;
   no_d->data = noptr;   

   yes_d->all_buttons = no_d->all_buttons = NULL;
   yes_d->dialog = no_d->dialog = dialog = gtk_dialog_new();
   ...
   if (yes_func) {
       gtk_signal_connect (GTK_OBJECT (tempwid), "clicked", 
			       GTK_SIGNAL_FUNC (yes_func), yes_d);
   }
   gtk_signal_connect_object (GTK_OBJECT (tempwid), "clicked", 
			      GTK_SIGNAL_FUNC (gtk_widget_destroy), 
			      GTK_OBJECT (yes_d->dialog));
   ...
   if (no_func) {
       gtk_signal_connect (GTK_OBJECT (tempwid), "clicked", 
			       GTK_SIGNAL_FUNC (no_func), no_d);
   }
   gtk_signal_connect_object (GTK_OBJECT (tempwid), "clicked", 
			      GTK_SIGNAL_FUNC (gtk_widget_destroy), 
			      GTK_OBJECT (no_d->dialog));
   gtk_widget_show (tempwid);

   /* Cancel button -- FIXME need cancel_d ? */
   if (cancel) {
   ...
   yes_d->all_buttons = g_list_append (yes_d->all_buttons, yes_d);
   yes_d->all_buttons = g_list_append (yes_d->all_buttons, no_d);
   no_d->all_buttons = yes_d->all_buttons;
...
void old_yes_no_dialog (char *diagtxt, char *infotxt, int num, ...) 
{
