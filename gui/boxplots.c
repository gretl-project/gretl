...
int retrieve_boxplot (PLOTGROUP *grp, const char *fname);
...
box_key_handler (GtkWidget *w, GdkEventKey *key, gpointer data)
{
    ...
    else if (key->keyval == GDK_d) {  /* dump: test */
	PLOTGROUP testgrp;

	dump_boxplot(data, "boxdump");
	if (retrieve_boxplot(&testgrp, "boxdump"))
	    fprintf(stderr, "didn't read boxplot right\n");
	else
	    dump_boxplot(&testgrp, "redump");
    }
#ifdef G_OS_WIN32
