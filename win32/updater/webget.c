...
/* webget.c for gretl_updater -- based on parts of GNU Wget */
		       ...
		       int *dt, struct urlinfo *proxy)
{
	...
	*dt &= ~TEXTHTML;
    ...
    hs->res = get_contents(sock, fp, u->local, &hs->len, &rbuf);
...
void read_registry (int *use_proxy, char *dbproxy) 
{
    char value[128];
    ...
    *dbproxy = '\0';
    ...
    if (read_reg_val(HKEY_CURRENT_USER, "useproxy", value) == 0)
	str_to_boolvar(value, use_proxy);
...
/* grab data from URL.  If savebuf is non-NULL write the result 
   to the saver buffer, otherwise write it to the file named
   localfile */
{
    uerr_t result;
    struct urlinfo *u;
