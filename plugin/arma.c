...
/* negative of the coefficients of the lag operator in the 
   expansion of (1-L)^d * (1-L^s)^D
*/

static int *diffcoeffs (int d, int D, int s)
{
    int i, k = d + s * D;
...
#if 0 /* not ready yet */
    if (arima_levels(ainfo)) {
	int *c = diffcoeffs(ainfo->d, ainfo->D, ainfo->pd);
