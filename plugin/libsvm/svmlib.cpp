...
// libgretl functions that we want to use
...
// Q_{(i, k), (i', k')} = z[(i, k)] z[(i', k')] *
//  { K(x[i], x[i']) + th_cost * [ k == k' ] }
...
{
public:
    RNK_Q(const svm_problem &prob, const svm_parameter &param,
	  const int _nr_thres, const double _th_cost = 1)
	:Kernel(prob.l, prob.x, param)
    {
	l = prob.l;
	nr_thres = _nr_thres;
	...
	phase = new schar[nr_thres*l];
	for (int i=0;i<l;i++) {
	    int idx = i * nr_thres;
	    y[i] = (int)prob.y[i];
	    QD[idx]= (this->*kernel_function)(i,i) + th_cost;
	    sign[idx] = ((y[i] <= 1) ? -1 : +1);
	    index[idx] = i;
	    phase[idx] = 1;
	    idx++;
	    for (int k=2; k<=nr_thres; k++) {
		QD[idx] = QD[idx-1]; //takes the same value as QD[i * nr_thres]
		sign[idx] = ((y[i] <= k) ? -1 : +1);
		index[idx] = i;
		phase[idx] = k;
		idx++;
	...
	buffer[0] = new Qfloat[nr_thres*l];
	buffer[1] = new Qfloat[nr_thres*l];
	next_buffer = 0;
    }
...
#pragma omp parallel for private(j) schedule(guided)
	...
	schar si = sign[i];
	for (int j=0; j<len; j++) {
	    double value = data[index[j]];
	    if (phase[j] == phase[i])
		value += th_cost;
	    buf[j] = si * sign[j] * value;
	}
	return buf;
    }
			...
			double *thres, int nr_thres)
{
    ...
    Solver s;
    s.Solve(bigl, RNK_Q(*prob,*param, nr_thres, 1), minus_ones, y,
...
static void sigmoid_train(int l, const double *dec_values,
			  const double *labels,
			  double& A, double& B)
{
...
// Using cross-validation decision values to get parameters for
// SVC probability estimates
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	...
	for (i=0; i<prob->l; i++)
	    if (prob->y[i] > nr_class)
		nr_class = (int)(prob->y[i]);
	...
	for (i=0; i<param->nr_weight; i++) {
		...
		if (param->weight_label[i] == label[j])
		    break;
	    if (j == nr_class)
		fprintf(stderr, "WARNING: class label %d specified in weight is "
...
double svm_predict(const svm_model *model, const svm_node *x)
{
	...
	for (int j=1; j<model->nr_class; j++) {
	    if (pred_result < model->rho[j]) {
		pred_result = j;
		found = 1;
		break;
	    }
...
{
    FILE *fp = gretl_fopen(model_file_name, "w");
    if (fp == NULL) return -1;
	...
	param.kernel_type == SIGMOID || param.kernel_type == LAPLACE ||
	param.kernel_type == EXPO)
...
{
    FILE *fp = gretl_fopen(model_file_name, "rb");
