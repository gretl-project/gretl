...
static NODE *aux_node_for_type (GretlType t, parser *p)
{
    int gen_t = gen_type_from_gretl_type(t);
...
/* evaluation of binary operators (yielding x op y) for scalar operands
...
double *copy_colvec_as_series (NODE *n, parser *p)
{
    double *ret = NULL;
...
static GretlType remedy_old_exists (const char *s)
{
    ...
    if (ns == 2) {
        ...
        if (b != NULL) {
            gretl_bundle_get_data(b, S[1], &ret, NULL);
        }
    }
...
#if EDEBUG
    fprintf(stderr, "get_bundle_member: %s[\"%s\"], querying = %d\n",
...
static int set_bundle_value (NODE *lhs, NODE *rhs, parser *p)
{
    ...
    if (p->op != B_ASN) {
        ...
        */
        ...
        lp = gretl_bundle_get_target(bundle, key, &ltype, &err);
        if (!err) {
	...
	*/
    ...
    if (targ == GRETL_TYPE_NONE && null_node(rhs)) {
        ...
        */
        void *lp = gretl_bundle_get_target(bundle, key, &targ, NULL);
    ...
    if (!err && targ == GRETL_TYPE_LIST) {
        ...
        case MAT:
            if (targ == GRETL_TYPE_DOUBLE && scalar_matrix_node(rhs)) {
                ...
                ptr = copy_colvec_as_series(rhs, p);
                if (!p->err) {
                    type = GRETL_TYPE_SERIES;
                    donate = 1;
                } else {
		...
		if (gtype == GRETL_TYPE_SERIES) {
			...
			gretl_bundle_set_series(b, key, ptr, p->dset);
                    ...
                    if (donate) {
                        ...
                        gretl_bundle_set_data(b, key, ptr, gtype);
                    }
                } else {
                    p->err = e_types(e);
                }
