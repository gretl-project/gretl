...
/**
 * gretl_matrix_columns:
 * @A: source matrix.
 * @sel: selection vector.
 * @err: location to receive error code.
 *
 * Produces a matrix which contains zero or more of the
 * columns of @A, selected and ordered according to the
 * indices in @sel; the columns of @A may thus be reordered
 * (and/or repeated). Note that since this function relates
 * to a "genr" function in userspace, the columns indicated
 * in @sel should be 1-based.
 *
 * Returns: the generated matrix, or %NULL on failure.
 */
...
gretl_matrix *gretl_matrix_columns (const gretl_matrix *A, 
				    ...
				    int *err)
{
    int n = gretl_vector_get_length(sel);
    gretl_matrix *C = NULL;

    if (n == 0) {
	if (sel->rows > 0 && sel->cols > 0) {
	    *err = E_NONCONF;
	} else {
	    C = gretl_null_matrix_new();
	    if (C == NULL) {
