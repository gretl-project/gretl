...
/* experimental placement of random-effects probit code */
			       ...
			       int quadpoints)
{
    ...
    C->y = malloc(nobs * sizeof *C->y);
    C->X = gretl_matrix_alloc(nobs, k-1);
    if (C->y == NULL || C->X == NULL) {
	return E_ALLOC;
    }

    C->B = gretl_matrix_block_new(&C->ndx, nobs, 1,
				  &C->P, C->N, C->qp,
				  &C->lik, C->N, 1,
				  &C->theta, k, 1,
				  &C->gh_nodes, 1, C->qp,
	...
	for (i=0; i<C->qp; i++) {
	    ...
	    gretl_matrix_set(C->gh_wts, i, 0, gretl_matrix_get(tmp, i, 1));
	}
    }
...
static double reprobit_ll (const double *theta, void *p)
{
    ...
    panelmod_t *pan = C->pan;
    double lambda, x, pit, a;
    int i, j, t, s, h = C->qp;
    int err = 0;

    /* note: theta is actually C->theta->val */
