			     ...
			     gretl_matrix *c)
{
    ...
    colsize = mi->nobs * sizeof(double);

    if (mi->method == MDS_COLS) {
	/* We'll be using OLS conditional on theta */
	mi->y = gretl_column_vector_alloc(mi->nobs);
	mi->xi = gretl_column_vector_alloc(mi->nobs);
	if (mi->y == NULL || mi->xi == NULL) {
	    return E_ALLOC;
	}
	if (mi->nalmonp == 0) {
	    memcpy(mi->y->val, dset->Z[mi->yno] + dset->t1,
		   colsize);
	}
	bigk = 0;
    }
    ...
    */
	...
	if (mi->bounds != NULL) {
		...
		if (bigk) k += takes_coeff(mt->type);
	    }
	} else {
	    if (bigk) k += takes_coeff(mt->type);
	}
	if (bigk || mt->type != MIDAS_U) {
	    k += mt->nparm;
	}
	if (mi->method == MDS_COLS) {
	...
	mi->theta = gretl_column_vector_alloc(npmax);
    }
    
    if (mi->b == NULL || mi->theta == NULL) {
	return E_ALLOC;
    }

    if (mi->method == MDS_COLS) {
	/* initialization of overall mi->theta: the
	   composition of individual theta vectors
	*/
	ii = 0;
	for (i=0; i<mi->nmidas; i++) {
	    mt = &mi->mterms[i];
	...
	/* "plain" L-BFGS-B */
	int clen = gretl_vector_get_length(c);

	for (i=0; i<mi->list[0]-1; i++) {
	    mi->b->val[i] = c->val[i];
	}
	...
	for (i=0; i<mi->nmidas; i++) {
	    mt = &mi->mterms[i];
	    if (takes_coeff(mt->type)) {
		mi->b->val[k++] = (ii < clen)? c->val[ii++] : 0;
	    }
	    for (j=0; j<mt->nparm; j++) {
		mi->b->val[k++] = (mt->theta == NULL)? 0 :
		    mt->theta->val[j];
	    }
	}
    }
...
static double bfgs_midas_SSR (const double *b, void *ptr)
{
	...
	gretl_matrix_reuse(mi->theta, mt->nparm, 1);
	for (j=0; j<mt->nparm; j++) {
	    mi->theta->val[j] = b[k++];
	}
	if (mt->type != MIDAS_U) {
	    w = midas_weights(mt->nlags, mi->theta, mt->type, &err);
	} else {
	    w = mi->theta;
	}
	if (!err) {
		...
		for (j=0; j<mt->nlags; j++) {
		    vi = mt->laglist[j+1];
		    mi->u->val[s] -= dset->Z[vi][t] * w->val[j];
		}
		s++;
	    }
	    if (w != mi->theta) {
    ...
    for (s=0; s<mi->u->rows; s++) {
	mi->SSR += mi->u->val[s] * mi->u->val[s];
    }
				 ...
				 int n, void *ptr)
{
    ...
    if (mi->nalmonp) {
	/* reset mi->y, if it may have been altered */
	size_t colsize = mi->nobs * sizeof(double);
	
	memcpy(mi->y->val, dset->Z[mi->yno] + dset->t1, colsize);
    }

    /* Fill relevant columns of mi->X with theta-weighted
    ...
    */
	...
	for (j=0; j<mt->nlags; j++) {
	    ...
	    s = 0;
	    for (t=dset->t1; t<=dset->t2; t++) {
		mi->xi->val[s++] += dset->Z[vj][t] * mg->val[j];
	    }
	}
	if (takes_coeff(mt->type)) {
	    gretl_matrix_multiply_by_scalar(mi->xi, mi->b->val[xcol++]);
	}
	gretl_matrix_reuse(mg, mt->nparm, 1);
	gretl_matrix_multiply_mod(mi->xi, GRETL_MOD_TRANSPOSE,
				  mi->u, GRETL_MOD_NONE,
				  mg, GRETL_MOD_NONE);
	for (j=0; j<mt->nparm; j++) {
	    g[k++] = mg->val[j];
...
#if 0    
    gretl_matrix_print(mi->b, "mi->b, cond_ols");
    fprintf(stderr, "SSR = %g\n", mi->SSR);
    return NADBL;
#endif    
				...
				BFGS_CRIT_FUNC func, void *ptr)
{
    ...
    colsize = mi->nobs * sizeof *targ;

    if (mi->X == NULL) {
	mi->X = gretl_matrix_alloc(mi->nobs, n);
	if (mi->X == NULL) {
	    return E_ALLOC;
	}
    ...
    */
	...
	if (!err) {
		...
		gretl_matrix_set(mi->X, s, xcol, xit);  
		s++;
	    }
	    gretl_matrix_free(w);
	}
	...
	if (mt->type == MIDAS_U && starting) {
	    ...
	    G = midas_gradient(mt->nlags, mi->theta, mt->type, &err);
	    if (!err) {
		pos = xcol;
		s = 0;
		for (t=dset->t1; t<=dset->t2; t++) {
			...
			gretl_matrix_set(mi->X, s, pos+ii, xit * hfb);
		    }
		    s++;
		}
		gretl_matrix_free(G);
    ...
    gretl_matrix_multiply_mod(mi->X, GRETL_MOD_TRANSPOSE,
			      mi->u, GRETL_MOD_NONE,
			      mi->g, GRETL_MOD_NONE);
		      ...
		      gretlopt opt)
{
    ...
    s->opt = opt;
    s->dv = mi->list[1];
    s->ncoeff = gretl_vector_get_length(mi->b);
    s->t1 = mi->dset->t1;
    s->t2 = mi->dset->t2;
    s->crit = mi->SSR;
		     ...
		     PRN *prn)
{
    ...
    colsize = mi->nobs * sizeof(double);
    nc = gretl_vector_get_length(mi->b);
    v = nc + 2;
    ...
    glist[0] = glist[1] = 1;
    memcpy(gdset->Z[1], mi->u->val, colsize);
    ...
    *pmod = GNR(glist, gdset, opt, prn);
	...
	transcribe_to_nlspec(&spec, mi, opt);
			   ...
			   gretlopt opt, PRN *prn)
{
    ...
    int n = gretl_vector_get_length(mi->b);
	...
	err = LBFGS_max(mi->b->val, n, 1000, reltol,
			&fncount, &grcount, bfgs_midas_SSR, 
			C_SSR, bfgs_midas_gradient, NULL,
			mi, mi->bounds, opt, prn);
    }

    if (!err) {
	err = bfgs_GNR(pmod, mi, opt, prn);
	if (!err) {
			       ...
			       gretl_matrix **cptr)
{
	...
	c = gretl_zero_matrix_new(nx + mi->hfslopes, 1);
	if (c == NULL) {
	    init_err = 1;
	}
    }
	...
	if (mi->hfslopes > 0 && mi->method != MDS_BFGS) {
		   ...
		   PRN *prn)
{
	...
	err = add_midas_matrices(mi, dset, &c);
    }
    ...
    if (!err && mi->method == MDS_BFGS) {
	/* estimate using L-BFGS-B */
	err = midas_bfgs_setup(mi, dset, c);
	if (!err) {
	    err = midas_bfgs_run(&mod, mi, opt, prn);
	}
    } else if (!err && mi->method == MDS_NLS) {
	/* estimate using NLS */
