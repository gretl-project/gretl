...
static int load_from_col (gretl_matrix *targ,
                          const gretl_matrix *src,
                          int t)
{
                                  ...
                                  gretl_matrix **Veps)
{
    ...
    if (!err) {
        ...
        *Veta = gretl_zero_matrix_new(K->r, K->r);
        if (*Veta == NULL) {
            err = E_ALLOC;
        }
    }
    ...
    if (!err && n > 0) {
        ...
        *Veps = gretl_zero_matrix_new(K->n, K->n);
        if (*Veps == NULL) {
            err = E_ALLOC;
        }
    }
...
static int retrieve_Tt (kalman *K)
{
    if (K->step == NULL || K->step->T == NULL) {
        ...
        return load_from_col((gretl_matrix *) K->T, K->step->T, K->t);
    }
}
...
static int retrieve_Zt (kalman *K)
{
    if (K->step == NULL || K->step->ZT == NULL) {
        ...
        return load_from_col((gretl_matrix *) K->uinfo->Z, K->step->ZT, K->t);
    } else {
        return load_from_col((gretl_matrix *) K->ZT, K->step->ZT, K->t);
    }
}
			     ...
			     int dj_extra)
{
    ...
    if (dj_extra) {
	load_from_col(K->djinfo->Vt, K->djinfo->V, K->t);
	load_from_col(K->djinfo->At, K->djinfo->A, K->t);
	load_from_col(K->Jt, K->djinfo->J, K->t);
    }
                          ...
                          int t, int DKstyle)
{
    ...
    if (K->p == 0) {
        ...
        if (DKstyle) {
            /* VY - VY D_t VY */
            fast_copy_values(Veps, K->VY);
            gretl_matrix_qform(K->VY, GRETL_MOD_TRANSPOSE,
                               D, Veps, GRETL_MOD_DECREMENT);
        } else {
            /* VY D_t VY */
            gretl_matrix_qform(K->VY, GRETL_MOD_TRANSPOSE,
        ...
        err = combined_dist_variance(K, D, Nt, Veta, Veps, BX,
                                     DKstyle);
        if (!err) {
            load_to_diag(K->Vsd, Veta, t, 0);
            load_to_diag(K->Vsd, Veps, t, K->r);
...
static int koopman_smooth (kalman *K, int DKstyle)
{
	...
	K->t = t;
        err = load_filter_data(K, NULL, SM_DIST_BKWD, 0);
        if (err) {
            break;
        }
    ...
    */
	...
	K->t = t;
        err = load_filter_data(K, NULL, SM_DIST_FRWD, 0);
        if (err) {
            break;
        }
...
static int anderson_moore_smooth (kalman *K)
{
    ...
    for (t=K->N-1; t>=0 && !err; t--) {
	K->t = t;
        err = load_filter_data(K, &nt, SM_STATE_STD, 0);
        if (err) {
            break;
        } else if (nt < K->n) {
...
/* write col @j of matrix @src into matrix @targ */

static void mat_from_col (gretl_matrix *targ, const gretl_matrix *src, int j)
{
    int i;
...
/* write vec of matrix @src into column @j of @targ */

static void col_from_vec (gretl_matrix *targ, const gretl_matrix *src, int j)
{
    int i;
...
static void state_rt_recursion (kalman *K,
				gretl_matrix *n1,
				gretl_matrix *r0,
				gretl_matrix *r1)
{
...
static void state_Nt_recursion (kalman *K,
				gretl_matrix *N0,
				gretl_matrix *N1)
{
    if (K->t == K->N - 1) {
...
static int state_smooth_dejong (kalman *K)
{
...
#if 0 /* not yet */
    if (B != NULL && K->exact) {
	B2 = gretl_matrix_block_new(&Bt, r, c,
				    &Rt, r, c,
				    &dhat, r, c,
				    &Psi, r, c,
				    NULL);
    ...
    for (t=K->N-1; t>=0 && !err; t--) {
	K->t = t;
        err = load_filter_data(K, &nt, smtype, 1); /* FIXME */
        if (err) {
            break;
        } else if (nt < K->n) {
            gretl_matrix_reuse(n1, nt, 1);
	...
	/* compute N_{t-1} */
	state_Nt_recursion(K, N0, N1);

	if (t >= K->d) {
	    /* regular recursion: compute r_{t-1} */
	    state_rt_recursion(K, n1, r0, r1);
	    /* a_{t|T} = a_{t|t-1} + P_{t|t-1} r_{t-1} */
	    fast_copy_values(K->a1, K->a0);
	    gretl_matrix_multiply_mod(K->P0, GRETL_MOD_NONE,
				      r0, GRETL_MOD_NONE,
	    ...
	    // } else {
            //    Ct = Pt * (Rt + Nt * At)
            //    BSC = Bt * S * Ct'
            //    Vplus = Bt * Psi * Bt' - BSC - BSC'
	    // }

	    // aht = (at + Pt * rt) + aplus
	    // Ahat[t,] = vec(aht)'
	    // vht = Pt - Pt * Nt * Pt + Vplus
	    // Vhat[t,] = vec(vht)'
	}
