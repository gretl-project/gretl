...
static void field_init (struct label_field *f)
{
    f->label = 0;
    f->literal[0] = '\0';
    f->vnum = -1;
...
int generate_obs_markers (const double **Z, DATAINFO *pdinfo,
			  char *s)
{
    struct label_field fields[3];
    char *p;
    int inquot = 0;
    int i, t, nf = 0;
    int err = 0;

    if (*s == ',' || *s == '\0') return E_SYNTAX;

    /* break up the comma separated fields */
    p = s;
    nf = 0;
    while (*p) {
	if (*p == '"') {
	...
	if (*p == '"') {
	    ...
	    fields[i].vnum = varindex(pdinfo, p);

	    if (fields[i].vnum >= pdinfo->v) {
		err = E_UNKVAR;
	    } 
	}
    }
	...
	for (i=0; i<nf; i++) {
	    ...
	    else if (fields[i].literal[0] != '\0') {
		strcat(tmp, fields[i].literal);
	    }
	    else if (fields[i].vnum > 0) {
		int zint = (int) Z[fields[i].vnum][t];

		if (zint < 999999 && zint > -99999) {
		    char numstr[8];

		    sprintf(numstr, "%d", zint);
		    strcat(tmp, numstr);
		}
	    }
	}
