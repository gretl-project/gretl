...
enum {
    ...
    P_LIST    = 1 <<  8, /* return a list result (only) */
    P_PRIVATE = 1 <<  9, /* generating a private or internal var */
    P_COMPILE = 1 << 10, /* just compiling tree, not evaluating */
    P_EXEC    = 1 << 11, /* evaluating pre-built tree */ 
    P_SLICE   = 1 << 12, /* compute matrix slice specification */
    P_UFUN    = 1 << 13, /* user function call with no assignment */
    P_SORT    = 1 << 14, /* doing a simple sort of a variable */
