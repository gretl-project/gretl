...
enum parser_flags {
    ...
    P_COMPILE = 1 <<  5, /* just compiling tree, not evaluating */
    P_EXEC    = 1 <<  6, /* evaluating pre-built tree */ 
    ...
    P_SAVEAUX = 1 << 16, /* indicates try aux node compilation */
    P_AUXDONE = 1 << 17, /* indicates aux nodes compiled */
    P_DELTAN  = 1 << 18, /* flag for change in series length */
    P_LHBKVAR = 1 << 19, /* LHS bundle key is string variable */
    P_CATCH   = 1 << 20, /* "catch" is in force */
    P_NODECL  = 1 << 21, /* type of result was not declared */
    P_LISTDEF = 1 << 22, /* defining a list on RHS */
    P_ANON    = 1 << 23, /* generating an anonymous object */
    P_VOID    = 1 << 24  /* function call, no assignment */
};
