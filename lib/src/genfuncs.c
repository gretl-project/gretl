...
gretl_matrix *gretl_periodogram (const double *x, int t1, int t2, int *err)
{
    gretl_matrix *X, *F, *P;
    double fij, pij, scale, iscale;
    int n, c, T = t2 - t1 + 1;
    int t, i, j;
	...
	*err = E_ALLOC;
    ...
    c = gretl_matrix_cols(F);
    n = T / 2;

    P = gretl_matrix_alloc(n, 2);

    if (P == NULL) {
	*err = E_ALLOC;
    } else {
	for (i=0; i<n; i++) {
	    gretl_matrix_set(P, i, 0, (i+1) * iscale); 
	    pij = 0.0;
	    for (j=0; j<c; j++) {
		fij = gretl_matrix_get(F, i+1, j);
		pij += fij * fij;
	    }
	    gretl_matrix_set(P, i, 1, pij / scale);
	}
    }
