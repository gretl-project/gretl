...
static int qr_make_vcv (MODEL *pmod, gretl_matrix *v, int hccme)
{
			  ...
			  gretl_matrix *xpxinv)
{
    ...
    /* vcv now holds HCCM */
    for (i=0; i<n; i++) {
	double x = gretl_matrix_get(tmp2, i, i);
...
static void save_standard_errors (MODEL *pmod, gretl_matrix *xpxinv,
				  int n)
{
    int i, k;
