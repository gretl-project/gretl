...
static int mle_model_allocate (MODEL *pmod, nlspec *spec)
{
    int k = spec->ncoeff;
    int nvc = (k * k + k) / 2;
...
static int make_mle_model (MODEL *pmod, nlspec *spec, 
			   const DATAINFO *pdinfo)
{
		       ...
		       gretlopt opt, PRN *prn)
{
	...
	if (spec->ci == NLS) {
	    ...
	    make_mle_model(&nlsmod, spec, pdinfo);
	    if (spec->depvar > 0) {
