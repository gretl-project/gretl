...
#define is_virtual_object(s) (s != NULL && strlen(s) > 15 &&	\
			      (strncmp(s, "virtual_series:", 15) == 0 || \
			       strncmp(s, "virtual_matrix:", 15) == 0))

static GretlType virtual_type (const char *s)
{
    if (!strncmp(s + 8, "series", 6)) {
	return GRETL_TYPE_SERIES;
    } else if (!strncmp(s + 8, "matrix", 6)) {
...
#define get_gen_str(s) ((const char *) s + 15)
...
static int bundled_item_copy_in_data (bundled_item *item, void *ptr)
{
    ...
    case GRETL_TYPE_UINT32:
        ...
        if (item->data != NULL) {
            uint32_t *up = item->data;

            *up = *(uint32_t *) ptr;
        }
        break;
    case GRETL_TYPE_UINT64:
        item->data = malloc(sizeof(uint64_t));
        if (item->data != NULL) {
            uint64_t *up = item->data;
                                       ...
                                       int *err)
{
    ...
    if (!copy && !bundled_scalar(item->type)) {
        ...
        *err = bundled_item_copy_in_data(item, ptr);
        if (*err) {
            free(item);
            item = NULL;
        }
    ...
    if (bundled_scalar(item->type)) {
        ...
        if (item->type == GRETL_TYPE_DOUBLE) {
            ...
            if (src_t == GRETL_TYPE_UINT32) {
                ...
                *up = gretl_uint32_from_double(*(double *) ptr, &err);
            } else {
                *up = (uint32_t) *(int *) ptr;
            }
        } else if (item->type == GRETL_TYPE_UINT64) {
            uint64_t *up = item->data;

            if (src_t == GRETL_TYPE_UINT64) {
                *up = *(uint64_t *) ptr;
            } else if (src_t == GRETL_TYPE_DOUBLE) {
                *up = gretl_uint64_from_double(*(double *) ptr, &err);
            } else {
                *up = (uint64_t) *(int *) ptr;
            }
        }
                                       ...
                                       int *err)
{
    ...
    ptr = gretl_bundle_get_data(bundle, key, &type, &myerr);
 ...
 * specified @bundle, if any, in the form of a column
 * vector; otherwise NULL.
 */
                                ...
                                int deflt)
{
    ...
    ptr = gretl_bundle_get_data(bundle, key, &type, NULL);
...
/**
 * gretl_bundle_get_uint32:
 * @bundle: bundle to access.
 * @key: name of key to access.
 * @err: location to receive error code.
 *
 * Returns: the 32-bit unsigned integer value associated with @key
 * in the specified @bundle, if any; otherwise 0.
 */

uint32_t gretl_bundle_get_uint32 (gretl_bundle *bundle,
                                  const char *key,
                                  int *err)
{
    uint32_t u = 0;
...
/**
 * gretl_bundle_get_uint64:
 * @bundle: bundle to access.
 * @key: name of key to access.
 * @err: location to receive error code.
 *
 * Returns: the 64-bit unsigned integer value associated with @key
 * in the specified @bundle, if any; otherwise 0.
 */

uint64_t gretl_bundle_get_uint64 (gretl_bundle *bundle,
                                  const char *key,
                                  int *err)
{
    uint64_t u = 0;
                              ...
                              void *ptr, GretlType type)
{
    ...
    return real_bundle_set_data(bundle, key, ptr, type, 0, NULL);
}
 ...
 * @key: name of key to create or replace.
 * @ptr: data pointer.
 * @type: the gretl type of the data.
 *
 * Sets the data type and pointer to be associated with @key in
 * the bundle given by @name. If @key is already present in
 * the bundle's hash table the original value is replaced
                           ...
                           void *ptr, GretlType type)
{
    ...
    if (bundle == NULL) {
        ...
        err = real_bundle_set_data(bundle, key, ptr, type, 1, NULL);
    }
...
int gretl_bundle_set_scalar (gretl_bundle *bundle, const char *key,
                             double val)
{
    return gretl_bundle_set_data(bundle, key, &val, GRETL_TYPE_DOUBLE);
}
...
int gretl_bundle_set_int (gretl_bundle *bundle, const char *key,
                          int val)
{
    return gretl_bundle_set_data(bundle, key, &val, GRETL_TYPE_INT);
}

/**
 * gretl_bundle_set_uint32:
 * @bundle: target bundle.
 * @key: name of key to create or replace.
...
int gretl_bundle_set_uint32 (gretl_bundle *bundle, const char *key,
                             uint32_t val)
{
    return gretl_bundle_set_data(bundle, key, &val, GRETL_TYPE_UINT32);
}

/**
 * gretl_bundle_set_uint64:
 * @bundle: target bundle.
 * @key: name of key to create or replace.
...
int gretl_bundle_set_uint64 (gretl_bundle *bundle, const char *key,
                             uint64_t val)
{
    return gretl_bundle_set_data(bundle, key, &val, GRETL_TYPE_UINT64);
}
        ...
        */
        if (src->type == GRETL_TYPE_DOUBLE) {
            ...
            if (targ->type == GRETL_TYPE_BOOL) {
                ...
                gretl_uint32_from_double(*px, &err);
            } else if (targ->type == GRETL_TYPE_UINT64) {
                gretl_uint64_from_double(*px, &err);
            }
        }
    } else {
...
static void check_bundled_item (gpointer key, gpointer value, gpointer p)
{
    ...
    if (targ == NULL) {
        ...
        *bc->err = bundled_item_replace_data(targ, m, GRETL_TYPE_MATRIX, 0);
        ...
        *bc->err = bundled_item_replace_data(targ, src->data, src->type, 1);
        if (*bc->err) {
...
static void real_print_bundled_item (bundled_item *item,
                                     struct b_item_printer *bip)
{
	...
	if (strlen(s) > 15) {
	    pprintf(prn, "%s (virtual %s: %s)", kstr, ts, s + 15);
	} else {
	    pprintf(prn, "%s (virtual %s)", kstr, ts);
	}
        ...
        uint32_t u = *(uint32_t *) item->data;

        pprintf(prn, "%s = %u", kstr, u);
    } else if (t == GRETL_TYPE_UINT64) {
...
/* wrapper for use with GList foreach */
...
static void print_bundled_item (gpointer value, gpointer p)
{
    ...
    real_print_bundled_item(item, bip);
}
...
static int is_bundles_array (bundled_item *item)
{
    if (item->type == GRETL_TYPE_ARRAY) {
        return gretl_array_get_content_type(item->data) == GRETL_TYPE_BUNDLE;
    }
...
static int print_bundle_tree (gretl_bundle *b, int level, PRN *prn)
{
    ...
    struct b_item_printer bip = {prn, level, 1};
    bundled_item *item;
    const char *key;
    gretl_array *A;
    gretl_bundle *bj;
    ...
    for (i=0; i<n; i++) {
        ...
        if (item->type == GRETL_TYPE_BUNDLE) {
            nmemb = gretl_bundle_get_n_members(item->data);
            bufspace(2*(level+1), prn);
            if (nmemb == 0) {
        ...
        } else if (is_bundles_array(item)) {
            A = (gretl_array *) item->data;
            nb = gretl_array_get_length(A);
            bufspace(2*(level+1), prn);
            pprintf(prn, "%s = array of bundles (%d elements)\n", key, nb);
            for (j=0; j<nb; j++) {
                bj = gretl_array_get_bundle(A, j);
                nj = gretl_bundle_get_n_members(bj);
                bufspace(2*(level+2), prn);
                pprintf(prn, "%s[%d] (%d members):\n", key, j + 1, nj);
...
static void xml_put_bundled_item (gpointer keyp, gpointer value, gpointer p)
{
    ...
    if (item->type == GRETL_TYPE_STRING || item->is_virtual) {
        ...
        pprintf(prn, "%" G_GUINT64_FORMAT, u);
    } else if (item->type == GRETL_TYPE_MATRIX ||
	       item->type == GRETL_TYPE_SERIES) {
        gretl_matrix *m = (gretl_matrix *) item->data;
    ...
    while (cur != NULL && !err) {
		...
		if (type == GRETL_TYPE_STRING || is_virtual) {
                    ...
                    if (!gretl_xml_node_get_double(cur, doc, &x)) {
                        ...
                        err = gretl_bundle_set_data(b, key, &x, type);
                    }
                } else if (type == GRETL_TYPE_INT) {
                    int i;
                    ...
                    if (!gretl_xml_node_get_int(cur, doc, &i)) {
                        ...
                        err = gretl_bundle_set_data(b, key, &i, type);
                    }
                } else if (type == GRETL_TYPE_UINT32) {
                    uint32_t u;

                    if (!gretl_xml_node_get_uint32(cur, doc, &u)) {
                        err = E_DATA;
                    } else {
                        err = gretl_bundle_set_data(b, key, &u, type);
                    }
                } else if (type == GRETL_TYPE_UINT64) {
                    uint64_t u;

                    if (!gretl_xml_node_get_uint64(cur, doc, &u)) {
                        err = E_DATA;
                    } else {
                        err = gretl_bundle_set_data(b, key, &u, type);
                    }
                } else if (type == GRETL_TYPE_SERIES && size > 0) {
		    /* legacy representation of bundled series */
			...
			gretl_bundle_donate_series(b, key, xvec, &dset);
			   ...
			   type == GRETL_TYPE_SERIES) {
                    ...
                    if (child == NULL) {
                        ...
                        if (!err) {
                            err = gretl_bundle_donate_data(b, key, baby, type);
                        }
                    }
                } else if (type == GRETL_TYPE_ARRAY) {
                    xmlNodePtr child = cur->xmlChildrenNode;
                    ...
                    if (child == NULL) {
                        ...
                        if (!err) {
                            err = gretl_bundle_donate_data(b, key, list, type);
                        }
                    }
                } else {
...
/* apparatus for supporting virtual bundle members */
                               ...
                               int *err)
{
    ...
    if (b != NULL) {
        ret = gretl_bundle_get_data(b, key, type, err);
    }
			     ...
			     gretl_bundle *b2)
{
    ...
    while (k1 != NULL && ret) {
	s1 = k1->data;
	v1 = gretl_bundle_get_data_full(b1, s1, &t1, &virt1, &err);
	if (!gretl_bundle_has_key(b2, s1)) {
	    ret = 0;
	} else {
	    v2 = gretl_bundle_get_data_full(b2, s1, &t2, &virt2, &err);
	    if (t2 != t1 || virt1 != virt2) {
		ret = 0;
	    } else if (t1 == GRETL_TYPE_STRING || virt1) {
		if (strcmp(v1, v2)) {
		    ret = 0;
		}
