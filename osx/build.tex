...
\item Apple's X11 and the Xcode development package.  If these are
  ...
  DVDs.
\item A basic installation of Fink, including \texttt{pkg-config} and
  \texttt{wget}.
...
The required packages will include gtk+2, gtk+2-dev, gmp, fftw3 and
readline5; recode may also be required; gnuplot is not required since
...
needed any longer.  I installed mpfr via Fink: this is not strictly
required for gretl, but if it's present it enhances the functionality
of the multiple precision plugin.
