/* lmmle.f -- translated by f2c (version 20030306).
...
static doublereal llsum_(integer *n, doublereal *x)
{
...
/* Subroutine */ int lmmle_(S_fp fcn, integer *m, integer *n, doublereal *x, 
...
/*     the purpose of lmmle is to maximize loglikelihood based on */
...
/*       subroutine lmmle(fcn,m,n,x,fvec,fjac,ldfjac,ftol,xtol,gtol, */
...
/*         the user wants to terminate execution of lmmle. */
...
/* L70: */
    ...
    xnorm = llsum_(n, &wa3[1]);
    delta = *factor * xnorm;
    if (delta == zero) {
	delta = *factor;
    }
    fprintf(stderr, "delta = %.8g\n", delta);
L80:
...
/* L210: */
    ...
    pnorm = llsum_(n, &wa3[1]);

/*           on the first iteration, adjust the initial step bound. */

    if (iter == 1) {
	delta = min(delta,pnorm);
	fprintf(stderr, "iter = 1, pnorm = %g, delta = %g\n", pnorm, delta);
    }
...
/* Computing 2nd power */
	...
	fprintf(stderr, "iter = %d, actred = %g\n", (int) iter, actred);
    }
#else
    actred = fnorm - fnorm1;
    fprintf(stderr, "actred = %g - %g = %g\n", fnorm, fnorm1, actred);
#endif
...
L240:
    ...
    delta = pnorm / p5;
    fprintf(stderr, "delta = pnorm/p5 = %g/%g = %g\n", pnorm, p5, delta);
    par = p5 * par;
L250:
L260:
...
/*           tests for convergence. */
	...
	*info = 1;
    ...
    if (delta <= *xtol * xnorm) {
	fprintf(stderr, "delta = %g, xtol = %g, xnorm = %g, prod = %g\n",
		delta, *xtol, xnorm, *xtol * xnorm);
	*info = 2;
    }
