...
these functions with brief commentary. We can expand on the finer
points later if there's sufficient interest. Note that this package
contains several test scripts that exemplify calls to the functions
listed below; this can be found in the \texttt{examples} subdirectory
of the installation directory. Likely locations for this are
...
\texttt{dataset} codes. The \texttt{limit} and \texttt{offset}
arguments permit specification ``paging'': so many results, starting
at a given offset into the full listing.
\begin{code}
# example
