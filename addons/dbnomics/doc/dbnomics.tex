...
\end{verbatim}
Creates a list from an array of bundles containing data, as generated
by functions such as, for example, \cmd{dbnomics\_get\_multiple}. The
second optional argument is a string: if the bundle contains a key
with the same name, and the value is a string, that string will be
used as the series identifier. Otherwise, an automatic name will be
provided.
\end{funcdoc}
