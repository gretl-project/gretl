...
However, for simple models a simple script should be quite easy to set
up: you estimate the model first, fetch all the necessary quantities
from the bundle containing your model, and create your forecast. A
simple example script follows.
...
# grab all the ingredients from the bundle               
series h = heraclitus.h
series uhat = heraclitus.uhat
matrix theta = heraclitus.coeff

# generate the parameters as scalars from the 
# estimated coefficients vector (see appendix B)

scalar omega = theta[2]
scalar alpha = theta[3]
scalar beta  = theta[5]

# now extend the dataset by 20 observations
dataset addobs 20

# and select a sub-sample
scalar latest = $nobs - 25
smpl latest ;

# initialize
series fore = h[latest]
# use GARCH formula while in-sample and 
# conditional variance afterwards
series fore = omega  + beta * fore(-1) + \
              alpha * (ok(uhat(-1)) ? uhat(-1)^2 : fore(-1))

# mummy, look at the pretty picture!
gnuplot fore time --with-lines --suppress-fitted --output=display
