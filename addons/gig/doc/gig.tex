...
\begin{eqnarray*}
  ...
  \zeta_t & = & \left\{
    \begin{array}{ll}
        \frac{a + b \stdu_t}{1-\lambda} & \mathrm{for\ } \stdu_t < -a/b \\
        \frac{a + b \stdu_t}{1+\lambda} & \mathrm{for\ } \stdu_t > -a/b
      \end{array}
      \right. \\
  D(\nu) & = &
...
\item[The regressors lists] These are two lists holding the exogenous
  ...
  equation \ref{eq:vareq}), respectively. They both default to empty
  lists, although in the variance regressors list a constant term is
  automatically included if absent. If some lists are already defined,
  you can pick them from the list; alternatively, you can create lists
  on the fly by using the ``+'' button.  Note that, from version 1.9.3
  of \app{gretl} onwards, you can use a single series in lieu of a
  list proper, so for example if you want a constant to appear in your
  conditional mean, you may just type \texttt{const} in the ``mean
  regressors'' text box.
...
estimated conditional standard deviation. The \dtk{gig_dplot}
function, instead, compares the estimated density of the standardised
innovations to their non-parametric kernel estimate and can be used
for judging visually how adequate the choice of a conditional
distribution is.

The behaviour of both functions can be inflected by means of an
optional bundle as their second argument. The accepted keys are
``\texttt{output}'' (a string for storing the output into a graphical
file) and ``\texttt{fontsize}'' (an integer for changing the font size
used in the plot). The key ``\texttt{title}'', for changing the plot
title is available only for the \dtk{gig_plot} function. 
...
produced by the \dtk{gig_var_fcast} function, followed by the model
bundle. The last two arguments are two scalars indicating how many
observation of the in-sample fitted variants you want in the graph and
the width of the coverage region (between 0 and 1).
...
model that has already been estimated. No check is performed.

\begin{funcdoc}{gig_print(bundle *b, scalar verbose)}
  Prints out a model.
\end{funcdoc}

\begin{funcdoc}{gig_plot(bundle *b, bundle opts)}
  Plots the residuals/conditional SE graph. The \texttt{opts} bundle
  is optional; the possible options are:
  \begin{description}
  \item[fontsize] a non-negative integer (default=9);
  \item[output] a string with an output file name
    (default="\texttt{display}");
  \item[title] a title string for the plot.
  \end{description}
\end{funcdoc}

\begin{funcdoc}{gig_dplot(bundle *b, bundle opts)}
  Plots the estimated density of the standardized residuals versus its
  nonparametric estimate.  The \texttt{opts} bundle
  is optional; the possible options are:
  \begin{description}
  \item[fontsize] a non-negative integer (default=9);
  \item[output] a string with an output file name
...
\begin{funcdoc}{gig_vfgraph(const matrix f, bundle mod, scalar before,
    scalar alpha)}
  \begin{enumerate}
  \item a matrix with the results of the simulations used to forecast
    the variances, such as the one produced by the \dtk{gig_var_fcast}
    function;
  \item the model bundle; 
  \item number of observation of the in-sample to plot; 
  \item width of the coverage region (between 0 and 1).
  \end{enumerate}
\end{funcdoc}
