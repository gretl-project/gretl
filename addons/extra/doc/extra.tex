...
\ArgRet{\texttt{matrix X}}{matrix}
This function relies on the relation
$vech(X'AX) = P (X \otimes X)' Q \; vech(A) = G \; vech(A)$,
where $P$ and $Q$ are certain interim results.
It returns the matrix $G$ such that the right-hand side
of the equalities becomes feasible, which should be 
