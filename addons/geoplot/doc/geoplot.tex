...
\item[\texttt{keypos}:] string, specifying the position of the key for
  ...
  see Section~\ref{sec:qualitative}).  This string is passed directly
  as an argument to the gnuplot \cmd{set key} command; therefore, any
  valid string should work. However, the most common usage takes the
  form of two words: \texttt{left}, \texttt{right} or \texttt{center}
  along with \texttt{top}, \texttt{bottom} or \texttt{center} (in
  either order). These two words can be preceded by \texttt{outside}
  for putting the key outside the plot area. For example,
  \verb|keypos="bottom center"| or \verb|keypos="outside top left"|.
