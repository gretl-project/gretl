...
The solution for subsampling is straightforward, as illustrated in
Listing~\ref{tab:USA}: just use the \texttt{smpl} command to cut out
the unwanted regions. Figure~\ref{fig:USA} shows the results with and
without exclusion of Alaska and Hawaii.\footnote{Given the role of
  this example we don't bother adding a real payload, but just
...
represent it. Otherwise you just get to view the map polygons.

Further, the window shown in response to \textsf{Display map} offers a
right-click menu that allows saving the map in PDF, EPS, PNG or EMF
format, or copying it to the clipboard. However, at this point the
option of saving the plot ``as an icon'', present for other sorts of
plots, is not available for maps.
...
save a modified version of the map file including the payload? And
in relation to a map of the USA (for example), is there a way to
include Alaska and Hawaii, but ``tuck them underneath'' the
continental US, like I see in some graphics on the web?
...
elements is as per GeoJSON, with gretl types in parentheses.
...
# add GDP per capita to metadata
nf = nelem(b.features)
loop i=1..nf
...
We now consider the more challenging case of rearranging
geographically dispersed regions of a given country into a compact
map\dots{}
...
gretl's mapping apparatus. Here we collect various ideas---no
promises, though!
