...
interface; and section~\ref{sec:expert} explains some ``expert''
refinements. Three appendices cover some technical points that may be
of interest to expert users.
...
are a pure encoding with no quantitative significance. A string-valued
series (and also a 0/1 dummy series) will be recognized by
\texttt{geoplot} as qualitative automatically, but otherwise a numeric
series \texttt{cvals} which serves as an encoding should be marked as
such, using the command
\begin{code}
setinfo cvals --coded
\end{code}
...
distinct colors. The values accepted under the \texttt{palette} key in
the \texttt{options} bundle are also different from the quantitative
case. We expect either the name of an array of $k$ strings, each of
which contains a color specification comprehensible by
\textsf{gnuplot}, or the keyword \texttt{auto} to confirm automatic
color selection. Here's an example, suitable for a binary payload
variable ($k=2$):
\begin{code}
strings mycolors = defarray("#D22532", "#244999")
opts.palette = "mycolors"
\end{code}
In addition a second element can be given when the payload data are
qualitative, namely an array of strings to appear in the ``key'' or
legend for the plot. Such strings will be set automatically if the
qualitative series is string-valued, but you may wish to supply
alternative strings (translated or abbreviated) for this purpose, or
to give suitable strings for a 0/1 binary variable (which cannot be
``stringified'' since that operation can only be done on a series with
a minimum value of 1 or greater). Examples of this usage can be found
in the \texttt{geoplot} example scripts.
...
handle qualitative payload data \\[6pt]
