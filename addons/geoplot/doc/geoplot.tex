...
latitude. However, we offer three options, as follows:
\begin{itemize}
\item Standard Mercator (\textsf{EPSG:3857})
\item ``Null'' projection (\textsf{EPSG:4326})
\item U.S. National Atlas Equal Area (\textsf{EPSG:2163})
\end{itemize}
...
To select one of the alternatives, you add a string to the
\texttt{options} bundle passed to the \texttt{geoplot} function under
the key \texttt{projection}: for Mercator, ``Mercator''; for
\textsf{EPSG:4326}, ``EPSG4326''; for \textsf{EPSG:2163},
``EPSG2163''. Please note that \textsf{EPSG:2163} is specially tuned
for the USA, and will produce weird-looking or non-existent results
