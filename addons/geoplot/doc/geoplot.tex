...
\begin{script}[htbp]
\begin{scode}
open founders.geojson --quiet
join founders.csv gdp pop --ikey=FID --okey=code
series gdppc = 1000*gdp/pop

# open full GeoJSON as bundle
bundle b = bread("founders.geojson")
...
\item As an alternative to having the user provide a \texttt{mask}
  (see section~\ref{sec:opts}) to limit the regions to be included in
  a map, we could arrange for gretl's \texttt{smpl} command (or its
  GUI counterpart) to govern the selection of regions transparently.
