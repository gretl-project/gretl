...
\option{okey} options worked fine. In some cases, however, the
respective identifiers may not match up. For example, region names are
in English in one dataset and in the corrsponding official language in
the other. Then you'll have to exercise your intelligence, but one
idea is to create an intermediate ``Rosetta stone'' file, maybe as
CSV, giving the mapping between the two identifiers, as in:
\begin{code}
# rosetta.csv
ID_en,ID_it
Apulia,Puglia
...
across Italian provinces as of 2020-04-27. The appearance of this plot
