...
\item We implemented automatic ``farming out'' of cross validation to
  ...
  \dtk{no_mpi} to the parameter bundle with a non-zero value. When
  MPI is being used one can specify the number of processes via
  \dtk{mpi_np} in the parameter bundle; this defaults to the lesser
  of the number of folds and the number of physical cores on the
  local machine.
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    \dtk{mpi_np} & optional integer & see Section~\ref{sec:speed}\\
...
2026-01-21: Fix problem whereby cross validation using MPI could
start more processes than there are folds, giving broken results;
augment/improve some printouts; fix bug in \dtk{glmnet_pred}.
