...
\end{verbatim}
  ...
  As in the \texttt{regls} function, the \texttt{X} list should not contain
  a constant: if the \texttt{B} matrix has one more row than \texttt{X} has
  members, the constant is handled automatically.

  The example script \texttt{lambda\_sequence.inp}, reproduced in part as
  Listing~\ref{script:lamseq}, illustrates how the information provided in
  the \texttt{regls} return bundle can be used to supply the arguments
  \texttt{B}, \texttt{lam} and \texttt{crit}.
\end{funcdoc}
...
\begin{funcdoc}
\begin{verbatim}
series regls_pred (const bundle b, list X)
\end{verbatim}
  Convenience function for producing a series of predicted values.  The
  bundle argument should be obtained via \texttt{regls} estimation; the list
  argument supplies values of the independent variables. This function
  automatically handles the presence or absence of an estimated intercept,
  as well as selection of a specific coefficient vector when estimation has
...
\end{verbatim}
  ...
  See also \texttt{mregls\_multiprint} and \texttt{mregls\_pred} below.
\end{funcdoc}
...
void mregls_multiprint (const matrix B, const matrix lam,
                        ...
                        const matrix crit[null])
\end{verbatim}
  Matrix-oriented counterpart of \texttt{regls\_multiprint} (see above).
  The arguments \texttt{y} and \texttt{X} must be $n \times 1$ and
  $n \times k$, respectively. They take the place of a series and a list in
  \texttt{regls\_multiprint}.
\end{funcdoc}
...
\end{verbatim}
  Matrix-oriented counterpart of \texttt{regls\_pred} (see above). The
  bundle argument should be obtained from \texttt{mregls}.
\end{funcdoc}
