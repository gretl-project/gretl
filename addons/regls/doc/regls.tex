...
several options via the toolbar buttons. Most of these are generic and
should be self-explanatory. Here we'll draw attention to the button
circled in red, with tooltip ``Graph''. This is active only when
estimation has been performed with multiple $\lambda$ values. In that
case it calls up a menu with two items: ``Criterion plot'' and
...
\begin{figure}[htbp]
  ...
  \caption{\texttt{regls} output window, graph button circled}
