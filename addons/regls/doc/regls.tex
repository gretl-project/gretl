...
The window that appears on clicking \textsf{OK} in the \texttt{regls}
dialog (Figure~\ref{fig:regls-window}) shows printed output and offers
several options. Reading left to right along the toolbar at the top of
the window, the buttons (identified here by their ``tooltip'' strings)
do the following.

\begin{figure}[htbp]
  \begin{center}
  \includegraphics[scale=0.5]{regls_output}
  \caption{\texttt{regls} output window}
  \label{fig:regls-window}
  \end{center}
\end{figure}

\begin{itemize}
\item ``Save'': save either the window text or the regls bundle.
\item ``Save bundle content'': enables saving of selected content of
  the regls bundle.
\item ``Print'': send the window content to a printer.
\item ``Copy'': copy the window content, as plain text or RTF.
\item ``Find'': search the window content.
\item ``Graph'': produce either a criterion plot or a plot of
  coefficient paths (see below for details). This item is active only
  when estimation involves multiple $\lambda$ values.
\item ``Forecast'': generate forecast information (see below for
  details).
\item ``Toggle split'': split the window content into two panes, or
  return to a single pane.
\end{itemize}

\textit{Graph button}: This gives two options. The first is a plot
showing the behavior of the minimand (MSE if cross validation is
selected, BIC otherwise) as $\lambda$ is relaxed. An example is shown
in Figure~\ref{fig:mse-plot}. The triangle indicates the MSE minimizer
and the circle indicates the point favored by the ``one standard
error'' criterion ($s^{\dagger}$, see Section~\ref{sec:xvalid}).
% The second option shows the paths of coefficient estimates as
% $\lambda$ is relaxed; an example is given in
% Figure~\ref{fig:coeff-plot}.

\textit{Forecast button}: TO BE ADDED.
...
\begin{funcdoc}
\begin{verbatim}
void regls_coeff_plot (const bundle b, const list L[null])
\end{verbatim}
  ...
  Section~\ref{sec:simple-search} or \ref{sec:xvalid} above. The
  optional list argument can be used to select for tracking a subset
  of the \texttt{X} list argument to \texttt{regls}.  This is
  ...
  coefficients are shown. See also \texttt{mregls\_coeff\_plot} below.
\end{funcdoc}

\begin{funcdoc}
\begin{verbatim}
void regls_criterion_plot (const bundle b)
\end{verbatim}
...
\begin{funcdoc}
\begin{verbatim}
void mregls_coeff_plot (const bundle b, const matrix sel[null])
\end{verbatim}
...
2024-04-27: Add specific prediction and plotting functions for use
...
variable; expand and update the documentation.\\[4pt]
