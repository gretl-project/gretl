...
\texttt{nzb} vector and \texttt{nzX} list (which includes
\texttt{const}, if applicable), such that fitted values can be
obtained thus:
\begin{code}
...
We've mentioned these points regarding fitted values in case you wish
to take charge of prediction yourself, but see
Section~\ref{sec:predict} for a more automated approach.
...
approach is to use the \texttt{regls\_pred()} function---or
\texttt{mregls\_pred()} if you're using data in matrix form and the
\texttt{mregls()} function for estimation. Both of these prediction
functions take two arguments: a bundle which includes estimated
coefficients and other relevant information from the estimation phase,
and either a list or a matrix holding values of the independent
variables over the prediction range.

In each case a single set of predictions is returned---as a series in
the case of \texttt{regls\_pred} or a column vector for
\texttt{mregls\_pred}---based on either the unique coefficient vector
if just one value of $\lambda$ was used, or the preferred coefficient
vector in the case of multiple $\lambda$ values. By ``preferred'' we
mean the BIC minimizer if cross-validation is not performed, or that
corresponding to either \texttt{idxmin} or \texttt{idx1se} (see above)
...
command is not relevant. If you have an integrated \texttt{X} matrix
of 1200 rows you might pass \texttt{X[1:1000,]} to \texttt{mregls},
then predict for the remainder of the observations via
\begin{code}
