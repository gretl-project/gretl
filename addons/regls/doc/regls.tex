...
\texttt{murdPerPop} as dependent variable and 101 regressors. The
$\lambda$ sequence was of length 20. In the LASSO tests we used the
first 800 observations and in the Ridge tests the first 1500 (to make
the test take longer and improve the resolution of the timer). We're
aware that the results we show are liable to be data- and
model-dependent and we offer some comments on this in the concluding
section.
...
%% LASSO: CCD and ADMM, using \texttt{lasso\_crit\_native2.inp}
...
hyperparameter (for LASSO, $\lambda$) that is likely to give
best results in genuine out-of-sample prediction. Do any of the points
of detail mentioned above make much difference in that regard?

Not a great deal can be said about this \textit{a priori}, though one
thing is clear: the more homogeneous the training data, the less such
details are going to matter. If the statistical properties of the
fold-complement samples are very similar to those of the full training
data then the locus of standardization (training data or
...
required---should be relatively unproblematic. The number of folds is
also relevant. For given $N$, more folds means smaller folds and
bigger fold-complements, comprising a larger proportion of the
training data and therefore presumably more representative of the
whole.

That said, real-world datasets of interest are not necessarily very
homogeneous so the details \textit{could} matter. So let's ask, when
the training data are not so homogeneous:
\begin{itemize}
\item Are we better off standardizing at the level of the training
  data or at the level of the fold-complement?
\item Is is better to work with a single $\lambda$ sequence or
  per-fold sequences?
\end{itemize}

Allowing that the answer to both questions may simply be ``it
depends'', we nonetheless tried experimenting with two rather
different datasets to see if anything systematic might emerge.
\begin{itemize}
\item Dataset 1: murder rates and covariates for US localities,
  referenced by Ryan Tibshirani in the LASSO context. Comprises 2215
  observations on 124 variables.\footnote{See
...
afforded by ADMM over CCD, at their respective default tolerances.
...
these are shown below, along with the correlations across the
methods per iteration.
\begin{center}
  \begin{tabular}{rrr}
    & $|z|$ & $\rho$ \\[4pt]
glmnet, CCD & 20.6 & 0.946 \\
glmnet, ADMM & 21.6 & 0.942 \\
CCD, ADMM & 8.4 & 0.996 \\
  \end{tabular}  
\end{center}
...
dataset, using the randomize-and-subset procedure described above, as
follows.  At each of 2000 iterations, $i$, we summed the absolute
proportional deviations of the fold-complement sample means,
...
\]
$H_\sigma$ was calculated in an exactly analogous way, based on the
sample standard deviation, $s$:
\[
  H_\sigma = \frac{1}{2000} \sum_{i=1}^{2000} \sum_{j=1}^{10} |(s_{ij} - s_i)/s_i|
\]
The values of these measures for the datasets were
\begin{center}
  \begin{tabular}{lcc}
    & $H_\mu$ & $H_\sigma$ \\
...
to details of the cross validation algorithm were much smaller for
Dataset 2.
...
%% FIXME The following needs updating!! %%
