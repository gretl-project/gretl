...
\title{\textsf{regls}: a gretl package for regularized least squares}
...
This function package is essentially a front-end for functionality
coded in C in the new gretl \texttt{regls} plugin; to run the package
you will need gretl version 2020a.  The plugin implements LASSO
...
(perhaps the best-known implementation, period) is that provided by
the \textsf{glmnet} package for \textsf{R}. Since we make several
...
The one required element in the \texttt{parms} bundle is a
...
\item under the key \texttt{nlambda}, the number of $s$ values
  to be used.
\end{itemize}
Note that the package contains a utility function
\texttt{lambda\_sequence()} that can be used to construct a
logarithmic sequence of $\lambda$ values, as per \textsf{glmnet}.
This function takes up to three arguments. The first and second
arguments (required) give the maximum $s$ and the number of values,
respectively. The third (optional) argument can be used to give the
minimum $s$ (by default 0.0001). For example,
\begin{code}
matrix lamseq = lambda_sequence(1, 5)
\end{code}
produces the set of $s$ values \{1, 0.1, 0.01, 0.001, 0.0001\}.

If the second option is taken---that is, \texttt{nlambda} is provided
instead of \texttt{lfrac}---the effect is equivalent to using a
\texttt{lambda\_sequence} vector with a maximum of $s=1$ and a minimum
of $s=0.0001$.

The most basic optional member of the parameter bundle is
\texttt{stdize}, a boolean switch to turn on or off standardization of
the data. The default is \textit{on}, but if the data are already
...
using a unified training sample; and search for optimal $\lambda$ via
cross validation.
...
This section treats of additional options that are obscure or
experimental. We're talking about values that can be passed via the
\texttt{parms} bundle to \texttt{lasso()} using the keys shown below.

\texttt{admmctrl}, a 3-vector. This can be used to inflect the
behavior of the ADMM algorithm; its elements are, in order:
\begin{itemize}
...
Non-positive values of these three terms are ignored, so one
can, for example, set a single element by passing a zero vector
with just the desired term set to a positive value.
...
we can demonstrate a good reason for divergence. We're now at that
point. On some examples gretl's \texttt{lasso} gets a smaller (better)
lasso criterion value than \textsf{glmnet} for a given $\lambda$, and
on some examples \textit{vice versa}, within a quite small range of
...
and included with our \texttt{lasso} package.
