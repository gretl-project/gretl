...
The \textbf{silhouette value} \citep{Rousseeuw87} for an observation is
a composite measure of its similarity to the members of its own cluster
and to the members of its nearest neighboring cluster. These values are
by construction in $[-1,1]$, where 1 is ``best'' (the point is firmly
located in its own cluster) and $-1$ is ``worst'' (the point appears to
belong elsewhere).  Values near 0 indicate overlapping clusters.  The
\dtk{kmeans_sil_plot} function plots these points, with a separate panel
for each cluster, along with the overall mean silhouette value. You pass
a bundle obtained via \texttt{kmeans} or \dtk{kmeans_fit} along with the
list of features. If the clustering is highly effective most of the
points should be in the positive zone. Example:
\begin{code}
kmeans_sil_plot(b, X)
\end{code}
