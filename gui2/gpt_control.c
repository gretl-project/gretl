...
static int allocate_plotspec_labels (GPT_SPEC *spec)
{
    int i;
	...
	designed specifically for files auto-generated by gretl. */
{
    ...
    if (datainfo->markers) {
	if (allocate_plotspec_labels(spec)) {
	    free(spec->data);
	    spec->data = NULL;
	    free(tmpy);
	    goto plot_bailout;
	...
	if (j == 1) { 
		...
		if (spec->labels[t][0] != 0) got_labels = 1;
	    }
	} else {      
...
static int redisplay_edited_png (png_plot_t *plot)
{
    ...
    /* destroy the old set of labeled flags for data-points */
    if (plot->labeled != NULL) {
	free(plot->labeled);
	plot->labeled = NULL;
    }
				     ...
				     int max_num2_width)
{
	...
	plot->pixel_ymin = PLOT_PIXEL_HEIGHT - 36;
    } else {
	plot->pixel_ymin = PLOT_PIXEL_HEIGHT - 24;
    }
