...
static void sheet_add_var (gpointer data, guint code, GtkWidget *w);
static void sheet_add_obs (gpointer data, guint code, GtkWidget *widget);
...
static void sheet_add_var (gpointer data, guint code, GtkWidget *w)
{
    ...
    *newvarname = 0;
	...
	gtk_tree_view_column_set_title(column, newvarname);
...
static void sheet_add_obs (gpointer data, guint where, GtkWidget *widget)
{
    spreadsheet *sheet = (spreadsheet *) data;
    GtkTreeView *view = GTK_TREE_VIEW(sheet->view);
    gint pointpath = 0;
    ...
    *newobsmarker = 0;
	...
	if (where == SHEET_AT_END) {
	    gtk_list_store_append(store, &iter);
	} else if (where == SHEET_AT_POINT) {
	    GtkTreePath *path;
	    GtkTreeViewColumn *column;
	...
	if (datainfo->markers) {
	    gtk_list_store_set(store, &iter, 0, newobsmarker, -1);
	...
	if (where == SHEET_AT_POINT && !datainfo->markers) {
