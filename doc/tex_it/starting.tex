...
del programma; si veda il capitolo~\ref{cli} e il \emph{Gretl Command
  Reference} per i dettagli sulla versione a riga di comando del
programma, \app{gretlcli}.
...
directory utente \app{gretl} (si veda il \GCR). Il contenuto di tale file
...
\item \textsf{Menù file}
  ...
  \item \textsf{Consulta database}: Si veda la parte~\ref{dbase}.
  \item \textsf{Crea dataset}: Apre il foglio elettronico integrato
    per inserire dati manualmente.  Si veda la parte~\ref{scratch}.
  ...
  \item \textsf{Preferenze}: imposta i percorsi per vari file a cui
    ...
    strumenti nella finestra principale.  Si veda il \GCR per i
    dettagli.
...
\item \textsf{Menù utilità}
  ...
  \item \textsf{Calcola p-value}: apre una finestra che mostra i
    ...
    \emph{F} o gamma. Si veda anche il comando \cmd{pvalue} nel \GCR.
...
\item \textsf{Menù sessione}
  ...
  \item \textsf{Visualizza icone}: apre una finestra che mostra la
    ...
    icone. Per i dettagli si veda la parte~\ref{session}.
...
\item \textsf{Menù campione}
  ...
  \item \textsf{Imposta in base a condizione}: simile al precedente,
    ...
    che soddisfano la condizione. Si veda la voce \cmd{genr} nel \GCR
  ...
  \item \textsf{Scarta valori mancanti}: scarta dal campione corrente
    ...
    mancante (si veda la parte~\ref{missing-data}).
...
\item \textsf{Menù modello}: per i dettagli sui vari stimatori offerti
  in questo menù, si consulti il \GCR oppure il capitolo ``Stima'' della
