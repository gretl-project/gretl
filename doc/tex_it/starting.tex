...
\item \textsf{Menù file}
  ...
  \item \textsf{Database}: permette di consultare alcuni ampi database di serie
    ...
    \ref{dbase}.
  \item \textsf{Funzioni}: (da scrivere).
...
\item \textsf{Menù strumenti}
  ...
  \item \textsf{Calcola p-value}: apre una finestra che mostra i
    ...
    \emph{F}, gamma o binomiale. Si veda anche il comando \cmd{pvalue} nella \GCR.
  \item \textsf{Calcola test}: calcola le statistiche test e i p-value
    ...
    delle varianze e proporzioni). Si veda anche la voce ``Test bivariati'' nel
    menù ``Modello''.
  ...
  \item \textsf{Preferenze}: imposta i percorsi per vari file a cui
    ...
    i risultati, attiva o disattiva la ``modalità esperto'' (che sopprime la
    visualizzazione di vari messaggi di avvertimento), attiva o disattiva
    l'avviso in caso di nuove versioni disponibili del programma, configura la
    barra degli strumenti, e molte altre opzioni. Per ulteriori dettagli, si
    veda la \GCR.
  \end{itemize}
...
\item \textsf{Menù visualizza}
  ...
  \item \textsf{Matrice di correlazione}: mostra i coefficienti di
    correlazione fra tutte le variabili nel dataset, o fra
    quelle selezionate.
