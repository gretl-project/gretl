...
risultato della regressione in formato {\LaTeX}. È possibile stampare
i risultati in formato tabulare (simile a quello usato nella finestra
dei risultati, ma con una migliore composizione tipografica) o sotto
forma di equazione. Per ognuna di queste opzioni è possibile scegliere
di vedere un'anteprima della pagina finale, o di salvare il risultato
in un file da includere in un documento {\LaTeX}. L'anteprima richiede
...
directory utente \app{gretl} (si veda la \GCR). Il contenuto di tale file
...
programma di arrivo.  Molte (non tutte) finestre di \app{gretl}
...
programma di arrivo: quando si conclude una sessione di \app{gretl} si
ha l'opportunità di salvare tutti i risultati della sessione in un
unico file.
...
da sinistra a destra, i menù File, Utilità, Sessione, Dati, Campione,
Variabile, Modello e Aiuto.
...
\item \textsf{Menù file}
  ...
  \item \textsf{Consulta database}: Si veda la sezione~\ref{dbase}.
  \item \textsf{Crea dataset}: Apre il foglio elettronico integrato
    per inserire dati manualmente.  Si veda la sezione~\ref{scratch}.
  ...
  \item \textsf{Preferenze}: imposta i percorsi per vari file a cui
    ...
    strumenti nella finestra principale.  Si veda la \GCR\ per i
    dettagli.
...
\item \textsf{Menù utilità}
  ...
  \item \textsf{Calcola p-value}: apre una finestra che mostra i
    ...
    \emph{F} o gamma. Si veda anche il comando \cmd{pvalue} nella \GCR.
...
\item \textsf{Menù campione}
  ...
  \item \textsf{Struttura dataset}: permette di modificare
    ...
    panel. Si veda anche il capitolo~\ref{chap-panel}.
...
\item \textsf{Menù modello}: per i dettagli sui vari stimatori offerti
  in questo menù, si consulti la \GCR\ oppure il capitolo ``Stima'' della
  guida in linea. Si veda anche il capitolo~\ref{chap-nls} a proposito
  della stima di modelli non lineari.
...
operazioni comuni utilizzando la tastiera:

\begin{description}
\item[Invio:] apre una finestra contenente i valori delle variabili selezionate,
  ossia, esegue il comando ``Dati, Mostra valori''.
\item[Canc:] cancella le variabili selezionate. Per evitare cancellazioni
accidentali è richiesta una conferma.
\item[Tasto "e":] ha lo stesso effetto del comando ``Modifica attributi'' del
menù ``Variabile''.
\item[Tasto F2:] ha lo stesso significato di ``e'', per compatibilità con altri programmi.
\item[Tasto "g":] ha lo stesso effetto del comando ``Definisci nuova variabile'' dal menù ``Variabile''
(che richiama il comando \texttt{genr}).
\item[Tasto "h":] apre la finestra di aiuto per i comandi di gretl.
\item[Tasto F1:] ha lo stesso significato di ``h'', per compatibilità con altri programmi.
\item[Tastp "t":] mostra in un grafico la variabile selezionata; per i dataset di tipo serie
storiche viene mostrato un grafico temporale, mentre per i dati di tipo cross
section si ottiene un grafico di distribuzione di frequenza.
\end{description}
