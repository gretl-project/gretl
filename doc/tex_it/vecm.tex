...
\begin{code}
  coint2 p listay [ ; listax ]
\end{code}
dove \texttt{p} è il numero di ritardi nella (\ref{eq:VECM-VAR}),
\texttt{listay} è una lista che contiene le variabili $y_t$, e
\texttt{listax} è una lista opzionale di variabili esogene.
...
\end{code}
dove \texttt{p} è il numero di ritardi nella (\ref{eq:VECM-VAR}),
\texttt{r} è il rango di cointegrazione, \texttt{listay} è una lista
che contiene le variabili $y_t$, \texttt{listax} è una lista opzionale di
variabili esogene, e \texttt{listaz} è un'altra lista opzionale di variabili
...
Sebbene bastino $r^2$ restrizioni per assicurare l'identificazione di 
$\beta$ in un sistema VECM, è possibile specificare ulteriori restrizioni e
controllare con un test del rapporto di verosimiglianza se queste sono
supportate dai dati. Queste restrizioni aggiuntive sono imposte di solito per
testare vincoli sui parametri che derivano dalla teoria economica su cui si
basano le relazioni di equilibrio.
...
globale\footnote{Nello sviluppo del codice per il test dei VECM in \app{gretl}
  ...
  Sven Schreiber della Johann Wolfgang Goethe-Universit\"at, per aver suggerito
...
Tuttavia, come detto in precedenza, \app{gretl} offre la possibilità di
