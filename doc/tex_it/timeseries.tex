...
è pari a $p$ in (\ref{eq:Joh-tests}) più uno. Di seguito è riportata parte
dell'output:
\begin{center}
\begin{code}
Test di Johansen:
...
\end{equation}
Prendendo il valore atteso non condizionale di (\ref{eq:condvar}) si ottiene
\[
...
\]
Affinché esista questa varianza non condizionale, occorre che
$\alpha_1 + \delta_1 < 1$, e quindi occorre richiedere che $\alpha_0 > 0$.
...
il modello GARCH in sé prevede una volatilità dei dati che dipende \textit{solo}
...
come mostrato in (\ref{eq:garch-meaneq}).
