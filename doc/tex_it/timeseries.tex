...
La struttura del comando \cmd{arma} di \app{gretl} non permette di specificare
modelli che contengono buchi nella struttura dei ritardi, se non usando la
specificazione stagionale discussa sopra. Ad esempio, se si ha una serie
mensile, non è possibile stimare un modello ARMA con termini AR (o MA) per i
soli ritardi di ordine 1, 3 e 5.

Un modo per aggirare questa limitazione con riguardo alla parte AR della
specificazione consiste nell'includere ritardi della variabile dipendente nella
lista delle variabili ``esogene''. Ad esempio, il comando
\begin{code}
...
\]
Tuttavia, questa scorciatoia non è raccomandabile: produce stime corrette, ma
rende inutilizzabile il meccanismo usato per le previsioni.
...
generalizzazione multivariata del test ADF. Si veda la sezione
\ref{sec:VECM-rep} per ulteriori dettagli.
...
\section{Cointegrazione e modelli vettoriali a correzione di errore}
\label{vecm-explanation}
 
\subsection{Modelli vettoriali a correzione di errore come rappresentazione di
...
\[
  ...
  \epsilon_t ;
\]
ma questo modello può essere riscritto in una forma più adatta per analizzare il
fenomeno della cointegrazione. Poiché $y_t = y_{t-1} - \Delta y_t$ e
$y_{t-i} = y_{t-1} - (\Delta y_{t-1} + \Delta y_{t-2} + \cdots +
\Delta y_{t-i+1}$, la forma \emph{vettoriale a correzione d'errore} del modello
...
Il test di Johansen per la cointegrazione viene usato per stabilire il rango di
$\beta$, o, in altre parole, per capire quanti vettori di cointegrazione ha il
sistema.  Questo test deve tenere conto di quali ipotesi vengono fatte a
proposito dei termini deterministici, per cui si possono individuare i ben noti
``cinque casi''. Una presentazione approfondita dei cinque casi richiede una
certa quantità di algebra matriciale, ma è possibile dare un'intuizione del
problema per mezzo di un semplice esempio.
...
\subsection{Identificazione dei vettori di cointegrazione}
\label{sec:johansen-ident}

Si può dimostrare che la stima di massima verosimiglianza di $\beta$ equivale a
risolvere un problema di autovettori. In pratica, se il rango di cointegrazione
è noto e pari a $r$, la matrice $n \times r$ $\beta$ viene stimata come
soluzione della seguente equazione matriciale:
\[
  M \beta = \beta \langle \lambda \rangle
\]
dove $\langle \lambda \rangle$ è una matrice diagonale che contiene gli
autovalori. Si noti comunque che se tutte le colonne di $\beta$ sono vettori di
cointegrazione, anche qualsiasi loro combinazione lineare è un vettore di
cointegrazione. Questo significa che la matrice matrice $\beta$ è
sotto-identificata, quindi i suoi elementi non possiedono una matrice di
covarianza; tuttavia questa difficoltà può essere aggirata imponendo un numero
appropriato di vincoli. Il metodo usato da \app{gretl} è chiamato
``Normalizzazione di Phillips''. Il punto di partenza consiste nello scrivere
$\beta$ in forma partizionata, come in
\[
  \beta = \left[
...
I coefficienti prodotti da \app{gretl} sono $\hat{\beta}$, mentre
$\hat{\beta_2}$ è nota come matrice dei coefficienti non vincolati.
