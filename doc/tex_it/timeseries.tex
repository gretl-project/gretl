...
Generalizzando questo principio alla stima di
(\ref{eq:arma-with-x}) o (\ref{eq:arma-with-z}), si ottiene che
\begin{code}
...
x_{t,2} \beta_2$. Usando l'opzione \texttt{--conditional}, come in
\begin{code}
...
\end{equation}
Sarebbe auspicabile che il comando \cmd{arma} fosse generalizzato in modo che
l'utente potesse specificare se, per ogni metodo di stima, certe variabili
...
traduce i comandi \cmd{arma} dalla sintassi vista sopra in una forma riconoscita
...
viene usata la massima verosimiglianza esatta, ma c'è l'opzione di usare la
...
trattamenti della media del processo $y_t$ \textit{non si applica}.
% TODO: the ``variant``???
% the comments above regarding the variant treatments of the
% mean of the process $y_t$ \textit{do not apply}.  That is, when you
Ossia, quando si usa \app{X-12-ARIMA} il modello stimato è
...
di differenziazione sulla variabile dipendente (stagionali e/o non stagionali)
