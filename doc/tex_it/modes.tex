...
File. Questo file di log è chiamato \verb+session.inp+ e viene
...
menù, i risultati dei comandi compariranno in un'unica finestra, dove possono
...
comando del menù \textsf{Aiuto, Comandi testuali}, oppure eseguire la
versione a riga di comando del programma \app{gretlcli} e consultare
il suo aiuto, oppure ancora consultare la \GCR.  
...
di sessione di \app{gretl} (si veda la sezione~\ref{session}) dopo l'esecuzione
...
Per aggiungere un grafico, occorre crearlo (dal menù Dati, \textsf{Grafico delle
variabili}, o attraverso uno degli altri comandi \app{gretl} di generazione dei
grafici).  Facendo clic sulla finestra del grafico si ottiene un menù da cui si
dovrà selezionare \textsf{Salva alla sessione come icona}.
