...
specificare l'opzione \verb+--verbose+ con \cmd{loop}.

L'opzione \verb+--progressive+ di \cmd{loop} modifica il comportamento
dei comandi \cmd{ols}, \cmd{print} e \cmd{store} rendendoli più comodi
per l'uso in analisi di tipo Monte Carlo (si veda Section
...
(si veda l'Example~\ref{loop-panel-script}), oppure è possibile usare
la sua rappresentazione come stringa \verb+$i+ (si veda l'Example
\ref{loop-string-script}).
...
La variable \verb+r+ assumerà i valori 0.01, 0.02, \dots{}, 0.99 nel
...
Altre regole per i tre componenti dell'espressione
di controllo: 
...
\item L'espressione che indica l'incremento o il decremento deve avere
  la forma X1 += DELTA, oppre X1 -= DELTA, dove DELTA è una costante
...
è possibile aprirlo usando il comando del menù ``File, Apri file
comandi, File di esempio, Greene...''.
