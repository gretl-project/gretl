...
\app{gretlcli}. Su Linux è possibile eseguirlo in una finestra di terminale
(xterm, rxvt, o simili), o in un terminale testuale. Su MS Windows può essere
...
l'appendice~\ref{app-technote}), è possibile richiamare e modificare i
comandi già eseguiti, inoltre è disponibile il completamento
automatico dei comandi. Per muoversi nella lista dei comandi già
eseguiti è possibile usare i tasti Freccia Su/Giù, mentre per muoversi
sulla riga di comando è possibile usare i tasti Freccia a
...
\section{La modalità non interattiva}
\label{cli-syntax}

Probabilmente, il modo più utile per svolgere grosse quantità di lavoro con
\app{gretlcli} consiste nell'utilizzare la modalità ``batch'' (non interattiva),
...
il programma resterà in attesa di input al termine dell'esecuzione dello script.
