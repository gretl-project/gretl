...
\begin{code}
  orthog x ; Z
\end{code}
dove \texttt{x} deve essere un vettore o una serie, e \texttt{Z} deve essere una
serie, lista o matrice. Nell'esempio~\ref{gmm-ols-ex}, la serie \texttt{e}
...
BFGS (si veda la sezione~\ref{genr-numerical} e il capitolo~\ref{chap:mle}). Il
...
sulla matrice di covarianza degli strumenti. L'esempio~\ref{gmm-tsls-ex} mostra
lo schema dell'approccio GMM.
...
\begin{code}
series e = 0
matrix beta = ... # Inizializzare le stime dei parametri
matrix X = ...    # Formare la matrice dei regressori
matrix Z = ...    # Formare la matrice degli strumenti
matrix V = inv(Z'Z)
...
ottenere usando il kernel di Bartlett. Una sua versione univariata è usata nella
funzione \texttt{lrvar()}, si veda l'equazione (\ref{eq:scalar-lrvar}). La
versione multivariata è riassunta nell'equazione (\ref{eq:gmm-hacvar}).
...
\app{Gretl} calcola in modo predefinito la matrice di covarianza HAC quando si stima
un modello GMM su serie storiche. È possibile controllare la larghezza di banda
(ossia il valore di $k$ nella \ref{eq:gmm-hacvar}) usando il comando
\texttt{set}. È anche possibile dire a \app{gretl} di \emph{non} usare la
versione HAC con il comando
%
\begin{code}
set force_hc on
\end{code}
