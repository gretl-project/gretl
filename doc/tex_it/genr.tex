...
\section{Massimizzazione numerica}
...
e \texttt{fdjac}, che produce un'approssimazione del Jacobiano.
% ??? ``a forward-difference approximation to the Jacobian''
...
\]
%
La dunzione ha un minimo globale in $(x,y) = (1,1)$ dove vale $f(x,y) = 0$.
...
\subsection{Calcolo del Jacobiano}

Per costruire una matrice di covarianza per le stime prodotte con
\verb+BFGS_max+, può essere utile calcolare un'approssimazione numerica del
Jacobiano rilevante.

La funzione \texttt{fdjac} accetta due argomenti: una matrice $n \times 1$
che contiene i valori iniziali dei parametri e una stringa che specifica una
chiamata a una funzione che calcola e produce una matrice $m \times 1$ (con $n
\leq m$), dati i valori attuali dei parametri e gli altri dati rilevanti.
