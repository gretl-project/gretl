...
La riga 4 impone un'interpretazione panel sui dati. Il primo parametro di
\texttt{setobs} è la ``frequenza'' dei dati, che nel contesto di una pila di
dati cross-section significa il numero di unità cross-section (qui \texttt{50}
per i 50 stati). Il secondo parametero denota l'osservazione iniziale, ossia
l'osservazione \texttt{01} (il primo dei 50 stati) per il primo anno (indicato
come \texttt{1}), ossia \texttt{1.01} (avremmo anche potuto scrivere
\texttt{1:01}). L'opzione \verb+--stacked-cross-section+ dice a gretl di
interpretare questa procedura nel modo giusto (50 unità cross-section, non 50
periodi temporali).

Nella riga 5 salviamo i dati in formato gretl, con un'interpretazione panel,
eliminando le ``variabili'' originali da \varname{p1} a \varname{p5}.
...
Il modello a effetti fissi aggiunge una variabile dummy per tutte le
...
Il modello a effetti casuali, d'altra parte, scompone la varianza dei
...
L'Example \ref{examp-pwt} sottostante apre il file
