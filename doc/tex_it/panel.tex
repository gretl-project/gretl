...
\item Un vincolo simile esiste per lo stimatore a effetti casuali. Questo
  ...
  osservazioni su $n$ unità e $k$ variabili indipendenti, e se $k>n$ lo
  stimatore ``between'' non è definito, visto che abbiamo solo $n$ osservazioni
...
L'ipotesi nulla è che la varianza di $v_i$ nell'equazione (\ref{eq:RE}) sia pari
a zero; se questa ipotesi non viene rifiutata, si può concludere che il semplice
modello ``pooled'' è adeguato.
