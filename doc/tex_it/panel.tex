...
Si considerino due osservazioni sulla stessa unità $i$ in due diversi periodi
...
\item Un vincolo simile esiste per il modello a effetti casuali. Per
  apprezzare questo punto occorre notare che lo stimatore a effetti casuali
  è una media ponderata (in senso matriciale) dello stimatore ``pooled OLS'' e
  dello stimatore ``between''. Se si hanno osservazioni su $m$ unità e $k$
  variabili indipendenti, e se $k>m$ lo stimatore ``between'' non è definito, visto 
  che abbiamo solo $m$ osservazioni effettive, e quindi non è definito neanche
  lo stimatore a effetti casuali.
\end{enumerate}
...
sarebbe inconsistente, mentre quello a effetti fissi sarebbe perfettamente
valido. È esattamente su questo principio che si basa il test di Hausman,
...
non viene rifiutata, si può concludere che il modello ``pooled'' è adeguato.
...
contenuti nella matrice $X$. Se il valore di $H$ è abbastanza alto, significa
