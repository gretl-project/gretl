...
Usando l'interfaccia grafica del programma, nel menù ``File, Funzioni'', si trova
una voce ``Editor pacchetti funzioni'', abilitata solo quando è stata caricata
in memoria almeno una funzione definita dall'utente.
...
Dopo aver caricato una funzione, la voce ``Editor pacchetti funzioni'' nel menù
``File, Funzioni'' dovrebbe essere abilitata. La prima finestra di dialogo presenta due
alternative:
...
chiudere gretl, a riaprirlo, e ad andare nel menù ``File/Apri file comandi/File
funzioni''. Se le operazioni precedenti sono andate a buon fine, troveremo il
pacchetto che avevamo creato, insieme alla sua breve descrizione. Facendo clic
su ``Info'', si ottengono tutte le informazioni disponibili sul pacchetto,
mentre facendo clic su ``Codice'' si aprirà una finestra che mostra il codice
delle funzioni contenute nel pacchetto. Facendo clic su ``Apri'', le funzioni
verranno caricate.
