...
\subsubsection{Modifica della frequenza dei dati}
...
periodo, dato di fine periodo). Se si intende importare serie con frequenza
diversa da un database, la procedura consigliabile consiste nel \emph{cominciare
ad importare le serie con la frequenza minore}. In questo
...
Se si desidera aggiungere a un dataset una serie di frequenza inferiore, occorre
per prima cosa ``espandere'' la nuova serie. Questa operazione è disponibile
solo per le serie annuali e trimestrali, a partire dal menù ``Campione'', ma è
da utilizzare con cautela, tenendo bene presente il suo funzionamento, che
consiste nel replicare i valori della serie a frequenza minore per quante volte
è richiesto dalla nuova frequenza. Ad esempio, ipotizzando di avere una serie
trimestrale che vale 35.5 in 1990:1, il primo trimestre del 1990. Dopo
