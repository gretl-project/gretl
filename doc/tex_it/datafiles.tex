...
\item File di lavoro di \app{Eviews} (\texttt{.wf1}).\footnote{Il supporto per
  questo tipo di file è sperimentale. Si veda \url{http://www.ecn.wfu.edu/eviews_format/}.}
\end{itemize}
...
Selezionando il comando ``File, Database'', vengono presentate tre alternative:
``Gretl'', ``RATS 4'' e ``Sul server di gretl''. Selezionando ``Gretl'', si
...
variabili, le unità cross-section (longitudinali) e i periodi temporali. Ad esempio,
