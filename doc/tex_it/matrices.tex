...
La sintassi per tutte queste funzioni, tranne l'ultima, segue la forma
%
\begin{code}
matrix B = func(A, &C)
\end{code}
%
mentre per \texttt{svd} è
%
\begin{code}
matrix B = func(A, &C, &D)
\end{code}
...
di cui è richiesta la scomposizione o l'analisi.

Il secondo argomento (il terzo nel caso di \texttt{svd}) deve essere il nome di
una matrice esistente, preceduto da \verb+&+ (per indicare l'``indirizzo'' della
matrice in questione), nel qual caso un risultato ausiliario viene scritto in
quella matrice, oppure la parola chiave \texttt{null}, nel qual caso il
risultato non è mostrato o è scartato.
...
contiene le componenti reali e la seconda quelle immaginarie. Se vengono
richiesti gli autovettori (ossia il secondo argomento di \texttt{eigengen} non è
\texttt{null}), essi vengono restituiti in una matrice $n \times n$.
...
\verb|$coeff| produce la matrice associata. Al momento non sono disponibili 
altri accessori per i sistemi di equazioni.
