...
<xsl:if test="(@alternate)"> o </xsl:if>
...
<xsl:template match="manref">
  <xsl:choose>
    <xsl:when test="$standalone='true'">
	  <xsl:text>il manuale completo di gretl</xsl:text>
	</xsl:when>
	<xsl:otherwise>
      <xref linkend="{@targ}"/>
	</xsl:otherwise>
...
<xsl:template match="menu-path">
  <para>Percorso menù: <xsl:apply-templates/></para>
</xsl:template>

<xsl:template match="other-access">
  <para>Accesso alternativo: <xsl:apply-templates/></para>
</xsl:template>
