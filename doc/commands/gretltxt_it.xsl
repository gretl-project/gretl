...
<xsl:template match="command[not(@context) or @context=$hlp]">
  ...
  <xsl:if test="(not(@context) and $hlp='gui')">
    <xsl:text>Comando script: </xsl:text>
...
<xsl:template match="options">
  ...
  <xsl:choose>
    <xsl:when test="count(option) > 1">
      <xsl:text>Opzioni:    </xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>Opzione:     </xsl:text> 
    </xsl:otherwise> 
  </xsl:choose>
  <xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="altforms">
  <xsl:call-template name="nl"/>
  <xsl:text>Forme alternative:  </xsl:text>
  <xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="examples">
  ...
  <xsl:choose>
    <xsl:when test="count(example) > 1">
      <xsl:text>Esempi:   </xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>Esempio:    </xsl:text> 
    </xsl:otherwise> 
  </xsl:choose>
  <xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="manref">
  <xsl:text>il manuale di gretl</xsl:text>
</xsl:template>

<xsl:template match="tabref">
  <xsl:text>La tabella seguente </xsl:text>
</xsl:template>

<xsl:template match="menu-path">
  <xsl:if test="$hlp='cli'">
Percorso menù:    <xsl:apply-templates/>
  </xsl:if>
</xsl:template>

<xsl:template match="other-access">
  <xsl:if test="$hlp='cli'">
Accesso alternativo: <xsl:apply-templates/>
  </xsl:if>
</xsl:template>
