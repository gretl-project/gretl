...
\[
  f_k = \sum_{j=1}^n e^{-i \omega_{j,k} } x_j 
\]
where $\omega_{j,k} = 2 \pi i \frac{(j-1) k}{n}$. Since the transformation
is invertible, the vector $\mathbf{x}$ can be recovered from
$\mathbf{f}$ via the so-called inverse transform
\[
  x_k = \frac{1}{n} \sum_{j=1}^n e^{i \omega_{j,k} } f_j .
\]
...
Conversely, \texttt{ffti} takes a complex argument and yields a real
output. For example:
\begin{code}
x1 = { 1 ; 2 ; 3 }
...
several vector with the same number of elements, it is more
numerically efficient to group the into a matrix rather than invoking
...
\end{eqnarray*}
the coefficient of the polynomial $c(x)$ are the convolution of the
coefficents of the two polynomials $a(x)$ and $b(x)$; the following
\app{gretl} code illustrates how to compute the coefficients of
$c(x)$:
\begin{code}
...
\ldots FFT and time series \ldots
...
S = sumr(F.^2)
S = S[2:($nobs/2)]/scale
omega = seq(1,($nobs/2)-1)' .* (2*pi/$nobs) 
