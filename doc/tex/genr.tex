...
THIS SECTION IS PROVISIONAL and may be moved or changed.

Two special functions are available to aid in the construction of
special-purpose estimators, namely \verb+BFGS_max+ (the BFGS
...
The \verb+BFGS_max+ function takes two arguments: a vector holding the
...
negative of the criterion.  On successful completion, \verb+BFGS_max+
...
\begin{code}
    matrix X = { dataset }
    matrix theta = { 1, 100 }'
    scalar J = BFGS_max(theta, "ObjFunc(&theta, &X)")
\end{code}
%
It is assumed here that \texttt{ObjFunc} is a user-defined function
(see Chapter~\ref{functions}) with the following general set-up:
%
\begin{code}
...
FIXME a full-blown example would be good here. 
