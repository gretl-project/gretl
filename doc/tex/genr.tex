...
prevent numerical problems when the the kernel function is evaluated
too far away from zero and is called the trim parameter.
...
more values actually observed in your sample, that is $m(x_i)$. In
this context, a common variant is the so-called ``leave-one-out''
estimator: this is a variant of the estimator that does not use the
$i$-th observation for evaluating $m(x_i)$. This makes the
Nadaraya-Watson estimator more robust numerically and is sometimes
advised what using it for inference purposes.  In formulae, the
...
FIXME: the trim parameter; syntax for the leave-one-out estimator.
