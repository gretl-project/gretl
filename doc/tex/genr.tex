...
holding initial parameter values and a string specifying a call to a
function that calculates and returns an $m \times 1$ matrix, given the
current parameter values and any other relevant data.  On successful
completion it returns an $m \times n$ matrix holding the Jacobian.
For example,
%
\begin{code}
...
estimate of its covariance matrix $\Sigma$, you may need do compute
