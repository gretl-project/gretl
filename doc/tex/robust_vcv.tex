...
in response to the \option{robust} option. These are available for
...
time-period, hence allowing for correlation of the disturbance term
...
\texttt{C2}, then two-way clustering is not called for since it is
equivalent to clustering on \texttt{C2} alone. The syntax for the
two-way case is on the following pattern
\begin{code}
