...
via fixed effects, pooled OLS, and pooled two-stage least
...
command \texttt{set pcse on}; the Arellano default can be
re-established via \texttt{set pcse off}.  (Note that in either case
the robust estimator is not used unless the \verb|--robust| flag is
given, or the ``Robust'' box is checked in the GUI program.)
