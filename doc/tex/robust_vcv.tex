...
an indequate model.  Data may be persistent though time, and if we fit
...
two such.  The default is $p = 0.75 T^{1/3}$, as recommended by Stock
and Watson (2003).  An alternative is $p = 4(T/100)^{2/9}$, as in
Wooldridge (2002b).  In each case one takes the integer part of the
...
\end{code}
%
As shown in Table~\ref{tab:haclag} the chice between \texttt{nw1} and
...
\subsection{Data-based bandwidth selection and prewhitening}
...
technique known as \emph{VAR prewhitening} (Andrews and Monahan, 1992)
can be illustrated by a simple example: if $x_t$ is a sequence of
...
\]
%
then the long-run variance of $x_t$ can be shown to be
%
\[
...
HAC estimate of $\hat{u}_t = x_t - \hat{\rho} x_{t-1}$; and (3) and
divide the result by $(1-\rho)^2$.

The application of the above concept to our problem implies estimating
a finite-order Vector Autoregression on the vector variables $\xi_t =
X_t \hat{u}_t$. In general, the VAR can be of any order, but in most
cases 1 is sufficient: the aim is not to build a watertight model for
$\xi_t$, but just to have the VAR ``mop up'' a substantial part of the
...
via fixed effects (FE), pooled OLS, and pooled two-stage least
squares.  The default robust estimator is that suggested by Arellano
(2003), which is HAC provided the panel is of the ``large $n$, small
$T$'' variety (that is, many units are observed in relatively few
...
cross-sectional units.  Cameron and Trivedi (2005) make a strong case
for using this estimator; they note that the ordinary White HCCME
can produce misleadingly small standard errors in the panel context
because it fails to take autocorrelation into account.

In cases where autocorrelation is not an issue, however, the estimator
proposed by Beck and Katz (1995) and discussed by Greene (2003,
...
\hat{\Sigma}_{\rm BK} = 
\left(X^{\prime}X\right)^{-1}
\left( \sum_{i=1}^n \sum_{j=1}^n \hat{\sigma}_{ij} X'_iX_i \right)
\left(X^{\prime}X\right)^{-1}
\]
