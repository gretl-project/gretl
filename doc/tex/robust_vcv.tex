...
autocorrelation is not a property of the data, it is a symptom of an
indequate model. Data may be persistent though time, and if we try to
fit a model that does not take this aspect into account properly, we
end up with a model with autocorrelated disturbances.

As a consequence, it is often possible to mitigate or to eliminate the
problem of autocorrelation altogether by including relevant lagged
variables in a time series model, or in other words, by specifying the
dynamics of the model more fully. HAC estimation should \textit{not} be
seen as the first resort in dealing with an autocorrelated error
process. 

In this context, the matrix $X' \Omega X$ is sometimes called the
\emph{long-run} covariance matrix of the vector $X_t u_t$, and it can
be shown to be the asymptotic covariance matrix of the time average
%
\[
\frac{1}{T} \sum_{t=1}^T X_t u_t. 
\]
%
...
can be illustrated by a simple example: if $u_t$ is a sequence of
...
therefore proceed in two steps: first, estimate $\rho$; then, obtain
a HAC estimate of $\hat{u}_t = x_t - \hat{\rho} x_{t-1}$ and divide
the result by $(1-\rho)^2$.
...
X_t u_t$. In general, the VAR can be of any order, but in most cases 1
is sufficient: the aim is not to build a waterproof model for $\xi_t$,
but just to have the VAR ``mop up'' a substantial part of the
...
\]
%
Then, the matrix $X'\Omega X$ can be recovered via
\[
  (I- \hat{A})^{-1} \hat{\Sigma}_u (I- \hat{A}')^{-1} ,
\]
where $\hat{\Sigma}_u$ is any HAC estimator, applied to the VAR
residuals.
