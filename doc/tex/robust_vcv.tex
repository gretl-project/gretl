...
keywords \texttt{period} and \texttt{unit} may be used to indicate
unique identifiers for the time periods and cross-sectional units,
respectively. Note that these keywords are not \textit{reserved};
there may be series of these names in the dataset, and if so they take
priority. As mentioned above, however, use of the keywords is more
efficient than using named series for period and unit.
