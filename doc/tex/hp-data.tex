...
when used as an argument to a command, so we used \texttt{0} instead
of \texttt{const}.
...
\subsection{Converting lists from/to matrices}

The relationship between lists and matrices is a little ambivalent, as
you may want to turn a list into a matrix by filling the latter with
the ID numbers contained in the list, or rather with the data in the
series the ID numbers refer to. In fact, both interpretations are
legitimate and either action can be useful at times, so hansl lets you
do both things.

If you assign a list to a matrix, like in
\begin{code}
  list L = moo foo boo zoo
  matrix a = L
\end{code}
the amtrix \texttt{a} will contain the ID numbers of the four series
...
If instead you want to create a matrix from the contents of the series
which belong to a given list, you have to enclose the list name in
