...
The columns of the dataset matrix are called \emph{series}.

\section{Reading and writing}

You can open a dataset by reading data from a disk file, via the
\app{gretl} \cmd{open} command, or by creating one from scratch,
through the \cmd{nulldata} command. The \cmd{smpl} command then allows
...
\GUG.\footnote{\app{R} users may think that a dataset is similar to
  what you get when you \texttt{attach} a data frame in \app{R}. Not
  really: in hansl, you cannot have more than one dataset open at the
  same time. That's why we talk about \emph{the} dataset.}
...
Don't be deceived by names: for example, the \cmd{probit} command can
...
These are useful if you want to write your own estimator.
