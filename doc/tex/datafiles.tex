...
\item \app{Eviews} workfiles (\texttt{.wf1}).\footnote{This is
    somewhat experimental.  See
...
within \app{gretl}, by using the tools under the ``Data, Add
variables'' menu and/or ``Variable, define new variable''.
...
If you do possess RATS data you should go into \app{gretl}'s ``File,
...
period).  If you want to import series of various different
frequencies from a database into \app{gretl}, the recommended
procedure is to \emph{start by importing a series of the lowest
  frequency you intend to use.} This will initialize your \app{gretl}
dataset to the low frequency, and higher frequency data can be
imported subsequently.

If you wish to add to a dataset a series that is of lower frequency,
it is necessary to ``expand'' the new series first.  This option is
available, for annual or quarterly time series only, under the
``Sample'' menu, but it should be considered an expert option: you
need to know what you are doing.  What \app{gretl} does is simply
...
therefore useless for fine-grained time-series analysis, outside of
the special case where you know that the variable in question does in
fact remain constant over the sub-periods.
...
Basically, the size of data sets (both in terms of number of variables
and number of observations per variable) is limited only by the
...
variables and observations are indexed by integers, and on a typical
PC these will be 32-bit values, capable of representing 2,147,483,647
at maximum.  
...
namely \app{TRAMO/SEATS} and \app{X-12-ARIMA}.  These programs used a
fixed-size memory allocation, and can't handle series of more than 600
observations.  
