...
(\ref{eq:fe-dyn}): if for each individual we have observations through
time that go from 1 to $T$, we may write the following system:
\begin{eqnarray}
  \label{eq:arbond3}
...
= T-2$. In practice, the Arellano--Bond technique estimates the above
...
parameters are estimated by assuming that the covariance matrix of the
$\eta_{i,t}$ terms is proportional to
\[
\left[
  \begin{array}{rrrrr}
    2 & -1 & 0 & \cdots & 0 \\
...
\right] ,
\]
as it should if the disturbances in the original model $u_{i,t}$ were
homoskedastic and uncorrelated. This yields a consistent, but not
necessarily efficient, estimator.

Step 2 uses the parameters estimated in step 1 to compute an estimate
of the covariance of the $\eta_{i,t}$ disturbances, and re-estimates
the parameters based on that. This procedure has the double effect of
handling heteroskedasticity and/or serial correlation, plus producing
estimators that are asymptotically efficient.

One-step estimators were sometimes preferred, in the past, on the
ground of them being more robust. Plus, computing the covariance
matrix of the 2-step estimator via the standard GMM formulae has been
shown to produce grossly biased results in finite
samples. \app{Gretl}, however, implements the finite-sample correction
devised in Windmeijer (2005), so standard errors for the 2-step
estimator can be considered relatively accurate.

By default, \app{gretl}'s \cmd{arbond} command estimates the parameters in
\[
  A(L) y_{i,t} = X_{i,t}\beta + v_i + u_{i,t} 
\]
via the 1-step procedure, by automatically differentiating the
dependent variable (note that the right-hand side variable are not
automatically differentiated) and using all available
instruments. However, these choices (plus some others) can be
overridden: please see the documentation for the \cmd{arbond} command
and the \texttt{arbond91} example file supplied with \app{gretl}.
