...
the sum $\sigma^2_v + \sigma^2_\varepsilon/T$ is estimated by the
...
An estimate of $\sigma^2_v$ can then be obtained by subtracting
$1/T$ times the estimate of $\sigma^2_\varepsilon$. 
...
\item A somewhat analogous issue arises with the random effects
  estimator.  The default Swamy--Arora method requires that the
  group-means model be estimated to obtain a measure of the between
  variance. Suppose we have observations on $n$ units or individuals
  and there are $k$ independent variables of interest.  If $k>n$, the
  group-means model cannot be estimated---since we have only $n$
...
Listing~\ref{fe-intercept}. The coefficients in the final OLS
...
be obtained by subtraction from \dollar{uhat}.
