...
suppressing the global constant).  Alternatively, one can subtract the
group mean from each of variables and estimate a model without a
...
\item A somewhat analogous prohibition applies to the random effects
  ...
  random effects estimator is a matrix-weighted average of the pooled
  OLS estimator and the ``between'' estimator.  The point is that if
  one has observations on $m$ units and $k$ independent variables of
  interest, then if $k>m$ the ``between'' estimator is undefined ---
  since we have only $m$ effective observations --- and hence so is
...
are tied to the validity of these additional hypotheses. If, for
...
estimates would be perfectly valid.  It is precisely on this principle
...
the matrix $X$.  If the value of $H$ is large enough this suggests
...
(1998).  These methods are not yet supported in \app{gretl}; the
former is likely to be added in a future release of the program.
