...
stacking, for both \varname{x1} and \varname{x2}.  I assume
...
Line 4 imposes a panel interpretation on the data.  The first
parameter to \texttt{setobs} is the data ``frequency,'' which in the
context of stacked cross sections actually means the number of units
in the cross-section (here \texttt{50} for the 50 states).  The second
parameter denotes the starting observation, here observation
\texttt{01} (the first state out of 50) for the first year (labeled
\texttt{1}), giving \texttt{1.01}.  (You could also write
\texttt{1:01}.)  The option flag \verb+--stacked-cross-section+ tells
gretl to interpret this structure in the right way (50 cross-sectional
units, not 50 time-periods).

On line 5 we save the data in gretl format, with the panel
interpretation, discarding the original ``variables'' \varname{p1}
through \varname{p5}.
...
previous section), \app{gretl} will handle the generation of such
