...
A note on performance: the mechanism \app{gretl} uses to invoke
\app{R} is far from being optimal in terms of speed. On some systems,
the overhead implied in a \texttt{foreign} block may even be in the
order of a few seconds; on the contrary, once \app{R} is loaded it
...
many \app{R} operations as you can, so to have as few \texttt{foreign}
blocks as possible.
