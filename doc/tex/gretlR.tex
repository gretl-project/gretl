...
chapter, the focus will be on the methods available to a \app{gretl}
to execute \app{R} commands without leaving \app{gretl}.
...
A note on performance here: the mechanism \app{gretl} uses to
invoke \app{R} is far from being optimal in terms of speed. The
overhead implied in a \texttt{foreign} block may even be in the order
of a few seconds. One \app{R} is loaded, on the contrary, it runs at
its full speed. Consequently, if performance is what you are after, it
is advisable to organize your script by grouping together as many
\app{R} operations as you can, so to have as few \texttt{foreign}
blocks as possible.
