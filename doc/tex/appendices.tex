...
version of \app{gretl} comes with a Windows version gnuplot 4.2; the
...
collinear, \app{gretl} automatically switches to use QR decomposition;
...
\cmd{print --long} (see the \GCR).  This will show the value to 10
digits (or more, if you set the internal variable \texttt{longdigits}
to a higher value via the \cmd{set} command).
