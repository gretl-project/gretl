...
(GMP and MPFR are optional, but recommended.)  The \texttt{dev}
packages for these libraries are necessary to \emph{compile}
\app{gretl} --- you'll also need the plain, non-\texttt{dev} library
packages to \emph{run} \app{gretl}, but most of these should already
be part of a standard installation.  In order to enable other optional
features, like audio support, you may need to install more libraries.

\textbf{Note!} The above steps can be much simplified on Linux systems
...
are rather remote for a reasonably up-to-date system, so the above
command should in most cases take care of everything correctly.
...
\app{cephes} code for computing p-values and other quantities relating
