...
above, or as the name of a pre-defined string variable.
...
these types only). In addition, the type of a new variable can be left
...
fly, \app{gretl} will nonetheless try to be as ``understanding'' as
...
x[3:15]    = 100  # panel: the 15th obs for the 3rd unit
\end{code}
...
\texttt{delete} command. Lists are an exception for this reason: in
the context of gretl commands, a named list expands to the ID numbers
of the member series, so if you say
