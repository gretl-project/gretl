...
\texttt{-=} can be used to remove array elements specified by their
\textit{content}. This is likely to be most useful for arrays of
strings. Note that \texttt{-=} works on all matching elements, so
...
Alternatively, a negative index can be used to remove a specified
element, as in
\begin{code}
strings S = defarray("a", "a", "b", "a")
S = S[-1]
