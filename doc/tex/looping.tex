...
In the loop body the following commands are accepted: \cmd{genr},
\cmd{ols}, \cmd{print}, \cmd{printf}, \cmd{pvalue}, \cmd{sim},
\cmd{smpl}, \cmd{store}, \cmd{summary}, \cmd{if}, \cmd{else} and
\cmd{endif}.By default, the \cmd{genr} command operates quietly in the
context of a loop (without printing information on the variable
generated).  To force the printing of feedback from \cmd{genr} you may
specify the \verb+--verbose+ option to \cmd{loop}.  The
\verb+--progressive+ option to \cmd{loop} modifies the behavior of the
commands \cmd{ols}, \cmd{print} and \cmd{store} in a manner that may
...
expression and provide some examples of use of loops.  \tip{ If you
  are carrying out a substantial Monte Carlo analysis with many
  thousands of repetitions, memory capacity and processing time may be
  an issue.  To minimize the use of computer resources, run your
  script using the command-line program, \app{gretlcli}, with output
...
example, \cmd{loop while essdiff > .00001} Execution of the commands
...
syntax looks like this: \cmd{loop i=1..20}.The index variable may be
...
the starting and ending values in the form of dates, as in \cmd{loop
  i=1950:1..1999:4}.
...
example:\cmd{loop for (r=0.01; r<.991; r+=.01)}In this example the
...
modified as follows.\cmd{ols}: The results from each individual
...
there is some random input at each step.\cmd{print}: When this command
...
error sum of squares from a regression.\cmd{store}: This command
...
their frequency distribution. Only one such \cmd{store} can be used in
a given loop.
...
\cmd{nulldata} is useful for Monte Carlo work.  Instead of opening a
``real'' data set, \cmd{nulldata 50} (for instance) opens a dummy data
...
\cmd{genr} command.See the \cmd{set} command for information on
...
(2000, Example 11.3).  This script is included in the \app{gretl}
distribution under the name \verb+greene11_3.inp+; you can find it in
\app{gretl} under the menu item ``File, Open command file, practice
file, Greene...''.The option \verb+--print-final+ for the \cmd{ols}
...
Example \ref{jack-arma} (kindly contributed by Riccardo ``Jack''
