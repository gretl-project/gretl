...
loop (or decremented if the starting value is greater than the ending
value).  The syntax looks like this: \cmd{loop i=1..20}.
...
variable is always incremented (or decremented) by one at each
iteration.  If you need more complex loop control, see the
``\texttt{for}'' form below.
...
the \cmd{set} command for information on generating repeatable
