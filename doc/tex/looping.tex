...
The command \cmd{loop} opens a special mode in
      which \app{gretl} accepts a block of
...
Five forms of control-expression
	...
	body the following commands are accepted:
      \cmd{genr}, \cmd{ols},
      \cmd{print}, \cmd{printf},
      \cmd{pvalue}, \cmd{sim},
      \cmd{smpl}, \cmd{store},
      \cmd{summary}, \cmd{if},
      \cmd{else} and \cmd{endif}.By default, the \cmd{genr} command operates
      quietly in the context of a loop (without printing information
      on the variable generated).  To force the printing of feedback
      from \cmd{genr} you may specify the
      \verb+--verbose+ option to \cmd{loop}.
    The \verb+--progressive+ option to
      \cmd{loop} modifies the behavior of the commands
      \cmd{ols}, \cmd{print} and
      \cmd{store} in a manner that may be useful with
	...
	program, \app{gretlcli}, with output
...
A second sort of control expression takes the form of the
	keyword \cmd{while} followed by an inequality:
...
A third form of loop control uses the special internal
	...
	like this: \cmd{loop i=1..20}.The index variable may be used within the loop body in one
...
The final form of loop control uses a simplified version
	of the \cmd{for} statement in the C programming
	...
	enclosed in parentheses.  For example:\cmd{loop for (r=0.01; r<.991; r+=.01)}In this example the variable \verb+r+ will
...
If the \verb+--progressive+ option is given for
      a command loop, the effects of the commands \cmd{ols},
      \cmd{print} and \cmd{store} are
      ...
      step.\cmd{print}:  When this command is used to
      ...
      for example the error sum of squares from a regression.\cmd{store}:  This command writes out the
      ...
      \cmd{store} can be used in a given loop.
...
This loop will print out summary statistics for the `a'
...
Example \ref{jack-arma} (kindly contributed by
...
Example \ref{loop-panel-script} shows an indexed loop in which
	the \cmd{smpl} is keyed to the index variable
...
The first time round this loop the variable
	...
	\cmd{ols} will be \verb+PBT1987+. The
