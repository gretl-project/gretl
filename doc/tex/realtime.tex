...
if holes > 0
  printf "File %s has %d realtime hole(s)!\n", fname, holes
else
  printf "Rest easy, file %s has no holes\n", fname
endif
\end{scode}
\end{script}
...
# plus 3 months
join @fname plus3 --data=INDPRO --tkey="observation_date,%Y%m%d" \
--timecols="observation_date,realtime_start_date" \
--filter="rel_ok(&observation_date, &realtime_start_date, 3)" \
--aggr=max(realtime_start_date)

# plus 6 months
join @fname plus6 --data=INDPRO --tkey="observation_date,%Y%m%d" \
--timecols="observation_date,realtime_start_date" \
--filter="rel_ok(&observation_date, &realtime_start_date, 6)" \
--aggr=max(realtime_start_date)

# plus 12 months
join @fname plus12 --data=INDPRO --tkey="observation_date,%Y%m%d" \
--timecols="observation_date,realtime_start_date" \
