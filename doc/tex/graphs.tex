...
symbol determined by gnuplot defaults. If you add the option \cmd{--with-lines} 
...
scatter plot can be controlled from gretl's side. The option \cmd{--suppress-fitted}
overrides gnuplot's default to draw a line if it deems the fit to be ``good enough''. The 
effect of \cmd{--inverse-fit} is to consider the variable on the y-axis as a function of 
...
\begin{script}[htbp]
  \caption{Plotting the log wage from the Mroz example dataset}
