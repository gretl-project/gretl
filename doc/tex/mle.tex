...
\item Choose your estimation method wisely.
\end{itemize}
...
fragments come from the \verb|mle-advanced.inp|, which is one of the
sample scripts supplied with the standard installation of \app{gretl}
(see under \emph{File $>$ Script File $>$ Practice files}).
...
using user-written functions and matrices. Consider the following code
fragment:
\begin{code}
...
matrices. However, proceeding in this way may imply inserting into the
\cmd{mle} block a long series of instructions; the example above
