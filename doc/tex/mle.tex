...
Chapter \ref{chap:functions}), so to extend \app{gretl}'s capabilities
in a modular and flexible way.

As an example, we will take a simple case of a model that \app{gretl}
does not provide natively yet: the zero-inflated Poisson model, or ZIP
...
is, we do not know which category a given individual belongs to. As an
example, consider the case when we have a sample of women and the
variable $y_t$ is the number of children. There may be a certain
proportion $\alpha$ of women for whom $y_t = 0$ with certainty (maybe
out of a personal choice or physical impossibility). In this case, we
have that the number of women for whom $y_t = 0$ include both the
women who won't have children and those who may have children, but
...
specification by, say, adding an explanatory variable. By using
user-written functions, we can simplfy this task considerably and
eventually be able to write something simple like
\begin{code}
list X = const x
zip(y, X)
\end{code}
...
example of such function can be seen in the example script
\ref{mle-zip-main}. By inspecting the function code, it is rather
obvious that the actual estimation does not happen here: rather, the
\texttt{zip()} function merely formats and prints out the results
coming from another user-written function, that is
\texttt{zip\_estimate()}. 
