...
follows:
\begin{code}
mle logl = p*ln(y) - lngamma(k) - ln(x) - y
  series y = x/theta
  params alpha p
end mle 
\end{code}
...
modified as follows:
\begin{code}
mle logl = check ? p*ln(y) - lngamma(p) - ln(x) - y : NA
  series y = x/theta
...
For reference, Listing~\ref{gamma-mle} presents a complete script that
...
\begin{script}[htbp]
  \scriptcaption{ML estimation of Gamma parameters}
  \label{gamma-mle}
\begin{scode}
