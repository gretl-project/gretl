...
The default method \app{gretl} uses to maximize the log-likelihood is
...
sample scripts supplied with the standard installation of \app{gretl}
...
score matrix is now easy achieved via
\begin{code}
mle logl = y*ln(P) + (1-y)*ln(1-P)
    series ndx = lincomb(X, b)
    series P = cnorm(ndx)
...
Since version 1.9.7, \app{gretl} offers the user the option of using
...
\item it is assumed that for some positive value of $\lambda$,
  ...
  in the direction $d(\mathbf{g}(\theta_0))$ leads upwards for some
  step length.
\end{enumerate}
...
certain optimality properties such that the number of iterations
required to reach the maximum is often much smaller than it would be
with other methods, such as BFGS. However, it may have some
disadvantages: for a start, the Hessian $\mathbf{H}(\theta)$ may be
difficult or very expensive to compute; moreover, the loglikelihood
may not be globally concave, so for some values of $\theta$, the
matrix $\mathbf{H}(\theta)$ is not negative definite or perhaps even
singular.  In those cases, a number of adverse consequences may occur,
which range from \emph{longer} computation time for optimization to
non-convergence of the algorithm.

As a consequence, using Newton's method is advisable only when 
\begin{itemize}
\item the computation of the Hessian is not too CPU-intensive;
\item the nature of the estimator is such that it is known in advance
  that the loglikelihood is globally concave.
\end{itemize}
The probit models satisfies both requisites, so we will expand the
preceding example to illustrate how to use Newton's method in \app{gretl}.
...
before the \cmd{mle} block.\footnote{To go back to BFGS, you use
  \cmd{set optimizer bfgs}.} This will instruct \app{gretl} to use
Newton's method instead of BFGS. If the \cmd{deriv} keyword is used,
\app{gretl} will differentiate the score function numerically;
otherwise, if the score has to be computed itself numerically,
\app{gretl} will calculate $\mathbf{H}(\theta)$ by differentiating the
...
\app{gretl} use its true value rather than a numerical
...
Two characteristics worth noting of the function above are: for a
start, it does return anything. The result of the computation is
...
\subsection{FIXME: TODO}
\begin{itemize}
\item how \app{gretl} handles cases when $-H$ is not pd
\item how to check analytical derivatives vs numerical ones
\item something on starting values, perhaps?
\end{itemize}
