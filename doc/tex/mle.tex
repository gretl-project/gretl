...
$b$. This could be easily achieved via 
\begin{code}
mle logl = y*ln(P) + (1-y)*ln(1-P)
    series ndx = lincomb(X, b)
    series P = cnorm(ndx)
...
this way may imply inserting into the \cmd{mle} block a long series of
instructions; the example above merely happens to be short because the
score matrix for the probit model is very easy to write in matrix
form.
...
\item it is assumed that for some positive value of $\lambda$,
  ...
  in the direction $d(\mathbf{g}(\theta_0))$ leads up for some step
  length.
\end{enumerate}
...
with other methods, such as BFGS. However, it may have the following
...
matrix $\mathbf{H}(\theta)$ is non-negative definite or perhaps even
...
\begin{itemize}
\item the computation of the Hessian is numerically not too demanding;
\item the nature of the estimator is such that it is known in advance
  that the loglikelihood is globally concave.
\end{itemize}
The probit models satisfies both requisires, so we will expand the
