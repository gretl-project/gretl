...
\newenvironment{code}{\par\footnotesize\verbatim}
{\endverbatim\normalsize}
...
In order to illustrate the examples that follow, we start from a brief
reminder on the basics of ML estimation.  Given a sample of size $T$,
it is possible to define the density function\footnote{We are
  ...
  exogenous variables.} for the whole sample, namely the joint distribution of
all the observations $f(\mathbf{Y} ; \theta)$, where $\mathbf{Y} =
\left\{ y_1, \ldots, y_T \right\}$.  Its shape is determined by a
vector of unknown parameters $\theta$, which we assume contained in a
set $\Theta$, and can be used to evaluate the probability of observing
a sample with any given characteristic.
...
function's argument, and the data $\mathbf{Y}$ only have the r\^ole of
...
The method gretl uses to maximise the log-likelihood is a
...
\]
and $\gamma(\cdot)$ is the log of the gamma function.
In order to estimate the parameters $\alpha$ and $p$ via ML, we need to maximise
(\ref{eq:gammaloglik}) with respect to them. The corresponding gretl
code snippet is
...
\begin{code}
    ...
    p = 1
\end{code}
are necessary to ensure that \texttt{p} and \texttt{alpha} exist
before the computation of \texttt{logl} is attempted. Their value will
be changed by the execution of the \texttt{mle} command and will be
replaced by the ML estimates after its successful completion. The
starting value is 1 for both; this is arbitrary and does not matter
much in this example (more on this later).
...
almost inconsequential, since the algorithm is likely to converge no
...
more suitable to be used as starting point for the algorithm. 
The gretl script code then becomes
\begin{code}
    scalar m = mean(x)
...
\subsection{Stochastic frontier cost function}
\label{sec:frontier}
...
$\varepsilon_i$ is the sum of $u_i$ and $v_i$.  A common assumption is
to postulate that $u_i \sim N(0,\sigma_u^2)$ and $v_i \sim
...
used in the article \emph{Banks' inefficiency and economic growth: a
  micro-macro approach}, \textbf{Scottish Journal of Political
  Economy}, vol. 48, n. 4. (2001), by Lucchetti, Papi and Zazzaro:
