...
that such a restriction can be represented as $g(\theta) = 0$, where
...
circumstances, the resulting system of equations cannot be solved
explicitly, so that numerical optimisation in necessary. In these
cases, the approach above is not particularly useful; instead, a
different strategy is much more convenient.

The idea is to find a function such that the vector $\theta$ is
expressed via an alternative parametrization in such a way that
$\theta$ becomes a (differentiable) function of a smaller set of
parameters $\psi$. In other words, find a function $h(\cdot)$ such
that any admissible value of $\theta$ can be writen as
$\theta = h(\psi)$ and $g[h(\psi)] = 0$ for any value of
$\psi$. Therefore, numerical maximization of the log-likelihood is
simply a question of operating on $\LogLik^*(\psi) = \LogLik[h(\psi)]$
and performing an ordinary unconstrained numerical optimization
routine.
...
\begin{script}[htbp]
  \caption{Example of ML estimation of a model under constraints}
  \label{ex:mle-constr}
  \begin{scodebit}
set verbose off
set seed 7120
...
\begin{script}[htbp]
  \caption{Handling non-convergence via \cmd{catch}}
  \label{ex:catch-mle}
\begin{scodebit}
set verbose off
nulldata 200
