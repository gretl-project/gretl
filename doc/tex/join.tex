...
\item The default assumption is that period information appears in the
  ...
  daily dates (format \texttt{YYYY-MM-DD}, for example 2013-01-31) and
  monthly observations (format \texttt{YYYY-MM}, for example 2013-03).
\item If the data have a frequency other than daily or monthly, or if
  the representation of the date is not as per ISO 8601, the
  \option{time} option can be used to specify the format in which
...
countries.\footnote{Retrieved 2013-08-05. The OECD files in fact
  contain two more columns with very long labels; these are irrelevant
...
the file in Table \ref{tab:oecd-gdp} has the name
\texttt{oecd.csv}, the following is an example script which
reconstructs the time series of Gross Domestic Product for several
countries:
...
have been added by storing the invariant part of the \cmd{join} option
