...
When a filter of this sort is applied, an error is flagged if a
missing value is encountered in the series referenced in the filter
expression This is because the condition then becomes
indeterminate; taking the last example above, if the \texttt{nkids}
value is \texttt{NA} on any given row we are not in a position to
evaluate the condition \texttt{nkids>2}. 

A variant of the filter option is available, however, to select rows
based on whether the value in a given named column is valid or
missing. Let \texttt{colname} stand in for the name of the column in
question; then
