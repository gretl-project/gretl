...
\item Thereafter, skip any ``illegal'' characters apart from spaces;
  one or more consecutive spaces are replaced by an underscore.
\item Stop when we reach 31 characters or the input is exhausted.
\end{enumerate}
...
\begin{itemize}
\item The default assumption is that time key appears in the first
  column, and conforms to the ISO 8601 standard: this covers daily
  dates (format \texttt{YYYY-MM-DD}, for example 2013-01-31), monthly
  observations (format \texttt{YYYY-MM}, for example 2013-03) and
  annual observations (format \texttt{YYYY}, 4-digit year).
\item If dates do not appear in the first column of the outer file, or
  if the format is not as described above, the \option{tkey} option
  can be used to indicate which column should be used and/or what
...
The third implies that dates are in the first column but are given
in the non-standard format year, ``\texttt{m}'', month.
