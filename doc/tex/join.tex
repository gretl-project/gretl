...
Rows from the outer dataset can be filtered using the \option{filter}
...
\end{itemize}
The first case is addressed in detail in the next section; here we
discuss the others.
...
The handling of the case where there's \textit{no} match on the right
...
data. However, if join is pulling in values for a series already
present on the left only matched rows will be updated. In other words
we do \textit{not} overwite an existing value on the left with
\texttt{NA} when there's no match on the right.
...
match'' should be taken as an implicit \texttt{NA}. In that case we
