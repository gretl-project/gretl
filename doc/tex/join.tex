...
option. In its basic version this option has a required argument of
the form
...
the type---numeric or string---matching that of the specified
series). For numerical data \textsl{op} can be any one of
...
\subsection{Filtering on missing/non-missing values}

A variant of the filter option is available, to select rows based on
whether the value in a given named column is valid or missing. Let
\texttt{colname} stand in for the name of the column in question; then

\begin{code}
--filter=missing(colname)
\end{code}

will select only rows on which there is no valid \texttt{colname}
value, while
...
will select only rows on which a valid \texttt{colname} value is
present. The selector \texttt{ok()} may be used as a synonym for
\texttt{!missing()}.
...
The \texttt{seq} method accepts three sorts of suffix. The basic one
is shown above: a positive integer representing the (1-based) position
of the observation in the sequence of matched rows. Alternatively, a
suffix of \texttt{last} can be given to select the last matching
observation, or a negative integer can be used to count down from the
last match (for instance, \texttt{seq:-1} selects the second-last
match). If the specified sequence number is out of bounds for
a given observation this method returns \texttt{NA}.
...
all we need is a means of identifying the period on the right. Most
likely, this information will appear in a single column in the outer
datafile, often but not always the first column.
...
options that can be used if the default is not applicable, as follows.
\begin{itemize}
...
contains a list of the most-often used codes.\footnote{The
  ...
  common in macroeconomics.} Here are a couple of simple examples:
...
same using the \option{time} and \option{tkey} options:

\begin{code}
join IBM.csv Close --time="%Y-%m-%d" --tkey=Date
\end{code}
