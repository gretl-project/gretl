...
\texttt{YYYYMMDD} (ISO 8601 ``basic'' format for daily dates).  Once
dates are in this form it is easy to use them in key-matching or
filtering.

By default it is assumed that the strings in the selected columns are
in ISO 8601 ``extended'' format, \texttt{YYYY-MM-DD}. If that is not
...
fixed-format data files. In that context the parameter to the
...
of the columns to be read from a delimited text data file (e.g.\
