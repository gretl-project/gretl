...
variable and possibly an ad-hoc parsimonious parametrization is
...
in \app{gretl} is explained in Chapter \ref{chap:mixed-frequency}; in
...
\end{equation}
where $\tau$ represents ``high-frequency time.'' and generalizations
should be obvious.
...
high-level functionality. At present \app{gretl} can handle natively
four commonly used parameterizations: normalized exponential Almon,
...
parameters (two being a common choice), and the beta takes either two
or three parameters. Full details on the forms taken by the $W(\cdot)$
function are provided in section \ref{sec:midas-param}.
...
coefficient is identically 1.0 and is omitted.
...
\app{Gretl} offers a dedicated command, \texttt{midasreg}, for estimation of
MIDAS models. (There's a corresponding item, \textsf{MIDAS}, under the
\textsf{Time series} section of the \textsf{Model} menu in the \app{gretl}
...
above,\footnote{The \textsf{Matlab} program includes an additional
  parameterization not supported by \app{gretl}, namely a
...
there, it is included in the current \app{gretl} package). Part of the
output from the replication script is shown in
Listing~\ref{ghysels-out}. The $\gamma$ coefficient is labeled
\texttt{HF\_slope} in the \app{gretl} output.
...
statistics such as $R^2$ and forecast RMSEs), \app{gretl}'s output agrees
...
produced by \app{gretl} are based on a Gauss--Newton Regression, as
...
\begin{itemize}
\item Levenberg--Marquardt. This is the back-end for \app{gretl}'s
  \texttt{nls} command.
...
familiar with the commands and functions available in hansl, \app{gretl}'s
...
normalized weights multiplied by the \texttt{HF\_slope}
coefficient. After estimation of a MIDAS model in the \app{gretl} GUI
this is available via the item \textsf{MIDAS coefficients} under the
\textsf{Graphs} menu in the model window. It is also easily generated
via script, since the \dollar{model} bundle that becomes available
following the \texttt{midasreg} command contains a matrix,
\texttt{midas\_coeffs}, holding these coefficients. So the following
is sufficient to display the plot:
%
\begin{code}
