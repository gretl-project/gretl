...
actual calculations done by \app{gretl} (although this statement does
...
OS X that's just a matter of launching \app{gretl} from the command
...
quite easily set up a special \app{gretl} shortcut that does the job.
...
When you start \app{gretl} in this mode, a ``console window'' appears
as well as the \app{gretl} window, and \texttt{stderr} output goes to
...
MS Windows, start the program \cmd{gretl.exe} from a console window
