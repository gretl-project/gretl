...
The attraction of the Vector Error Correction Model (VECM)---also
known as a cointegrated VAR or CVAR---is that it allows the researcher
to embed a representation of economic equilibrium relationships within
a relatively rich time-series specification.  This approach overcomes
the dichotomy between (a) structural models that faithfully
represented macroeconomic theory but failed to fit the data, and (b)
time-series models that were accurately tailored to the data but
difficult if not impossible to interpret in economic terms.
...
\[
  \alpha'_{\perp} \mu_0 = 0 ,
\]
where the $\perp$ symbol indicates the ``orthogonal complement''
operator: for if $A$ is an $r \times c$ matrix, with $r>c$, then
$A_{\perp}$ is a matrix (not necessarily unique) such that
$A'_{\perp}A = 0$. The matrix $\alpha_{\perp}$ is also known as the
left null space of the matrix $\alpha$.
...
As in the ADF test, the asymptotic distribution of the tests varies with
the deterministic component $\mu_t$ one includes in the VAR (see section
\ref{sec:coint-5cases} above). The following code uses the \cmd{denmark}
data file, supplied with gretl, to replicate an example found in
Johansen's 1995 book.
%
\begin{code}
open denmark
...
differently.  The command \dtk{set vecm_norm diag} selects a
normalization that simply scales the columns of the original $\beta$
such that $\beta_{ij} = 1$ for $i=j$ and $i \leq r$, as used in the
empirical section of \cite{boswijk-doornik04}.  Another alternative is
\verb+set vecm_norm first+, which scales $\beta$ such that the
elements on the first row equal 1.  To suppress normalization
altogether, use \dtk{set vecm_norm none}; to return to the default use
\dtk{set vecm_norm phillips}.} The starting point is writing $\beta$
in partitioned form as in
\[
  \beta = \left[
...
One purpose of imposing restrictions on a VECM system is to achieve
identification.  If the restrictions are simply normalizations, they
are not testable and should have no effect on the maximized
likelihood.  In addition, however, one may wish to formulate
...
\cite[section 4.4]{boswijk-doornik04}.
...
If the list of restrictions passed to the \texttt{restrict} command
contains more constraints than necessary to achieve identification,
then an LR test is performed. In addition, the \texttt{restrict}
command can be given the \option{full} switch, in which case full
estimates for the restricted system are printed (including the
$\Gamma_i$ terms) and the system thus restricted becomes the ``current
model'' for the purposes of further tests.  Thus you are able to carry
out cumulative tests as in Chapter 7 of \cite{johansen95}.

\subsection{Syntax}
\label{sec:vecm-restr-syntax}

The full syntax for specifying a restriction is an extension of
that exemplified in the previous section. Inside a
\texttt{restrict}\ldots\texttt{end restrict} block, valid statements
are of the form
\begin{center}
  \textsl{parameter-linear-combination} = \textsl{scalar}
\end{center}
where \textsl{parameter-linear-combination} is a weighted sum of
...
combinations involving $\beta$. Here are a few examples of valid
restrictions:
\begin{code}
  b[1,1] = 1.618
  b[1,4] + 2*b[2,5] = 0
...
If a certain constraint should be applied to all columns of $\beta$ a
single (row) index is attached to each \texttt{b} term and the square
brackets are dropped.  Hence, the following syntax
\begin{code}
restrict
  b1 + b2 = 0
end restrict
...
A variant of the single-index syntax for common restrictions on
$\alpha$ and $\beta$ is supported: you can replace the index number
with the name of the corresponding variable, in square brackets.
So if the third variable in a system is named \texttt{cpr}
one could write \texttt{a[cpr] = 0} instead of \texttt{a3 = 0}.
...
There are two points to note in relation to this last option. First,
...
traditional formulation of the Fisher equation would reverse the roles
of the variables in the second equation, but this detail is immaterial
in the present context. Per the expectation theory of interest rates
one would expect the third equilibrium relationship to include a
constant for the liquidity premium, but in this example the system is
estimated with the constant term unrestricted so the liquidity premium
gets absorbed into the system intercept.}  (The names for these
variables in the gretl data file are \verb|m_p|, \texttt{rl},
\texttt{rs}, \texttt{y} and \texttt{infl}, respectively.)

The cointegration rank assumed by the authors is $r=3$ and there are
five variables, giving 15 elements in the $\beta$ matrix; $r^2 = 9$
...
Another type of test that is commonly performed is for ``weak
exogeneity''. In the VECM context a variable is said to be weakly
exogenous if all coefficients on the corresponding row in the $\alpha$
matrix are zero. In that case the variable does not adjust to
...
each variable in turn, thus replicating the first column of Table 6 in
\cite{brand-cassola04}.  The results show that weak exogeneity might
perhaps be accepted for the long-term interest rate and real GDP
(p-values $0.07$ and $0.08$ respectively).
...
One aspect of VECM restrictions that can be confusing at first is that
identification (does the restriction identify the system?) and
...
maximized likelihood---but it is not identifying; it leaves open the
scale of $\beta$.
...
is a necessary but not a sufficient condition.  In fact, when $r>1$ it
...
where identification is assessed via the rank of the information
matrix.  It can be shown that for restrictions of the sort
(\ref{eq:vecbeta}) and (\ref{eq:vecalpha}) the information matrix has
the same rank as the Jacobian matrix
%
\[
...
via the modified eigenvalues method set out by \cite{johansen95}.  We
solve directly for the ML estimator, without any need for iterative
methods.}  In some cases convergence may be difficult to achieve, and
gretl offers several choices to solve the problem.

\subsection{Switching and LBFGS}
\label{sec:vecm-algorithms}

Two maximization methods are available in gretl. The default is the
switching algorithm set out in \cite{boswijk-doornik04}.  The
alternative is a limited-memory variant of the BFGS algorithm (LBFGS),
using analytical derivatives; this is invoked using the \option{lbfgs}
...
identify the parameters.  LBFGS, on the other hand, requires that the
model be fully identified.  When using LBFGS, therefore, you may have
to supplement the restrictions of interest with normalizations that
serve to identify the parameters.  For example, one might use all or
part of the Phillips normalization (see section
\ref{sec:johansen-ident}).
...
various sources. We'd like to thank Luca Fanelli of the University of
Bologna and Sven Schreiber of Free University of Berlin for their help
in devising torture-tests for gretl's VECM code.} The optimizer may
end up at a local maximum (or, in the case of the switching algorithm,
at a saddle point).
...
initialization using \textit{simulated annealing}.  This is invoked by
...
Besides annealing, a further alternative is \textit{manual
...
In this example---from \cite[pp.\ 114--115]{johansen95}---the
cointegration rank is 1 and there are four variables.  However, the
model includes a restricted constant (the \option{rc} flag) so $\beta$
has 5 elements.  The $\alpha$ matrix has 4 elements, one per equation.
So the matrix \texttt{BA} may be read as
\[
...
two free parameters in $\phi$ (the five elements in $\beta$ minus the
three restrictions).  There is no call to give values for $\psi$ since
...
unless you
\begin{itemize}
\item forbid this, by giving the \option{no-scaling} option
...
affect identification; and gretl would in fact perform scale removal
by default.

\section{Long-run effects}
\label{sec:vecm-lr}

A question that may be of interest following estimation of a
cointegrated VAR is, what are the long-run effects on the endogenous
variables of the innovations, $\epsilon_t$? This can be quite
complicated. Here we recall the CVAR equation~\ref{eq:VECMab} for
reference:
\[
    \Delta y_t = \mu_t + \alpha \beta' y_{t-1} + \sum_{i=1}^{p-1} \Gamma_i 
    \Delta y_{t-i} + \epsilon_t \qquad \epsilon_t \sim N(0, \Omega)
\]
Innovations in period $t$ have an immediate effect on the changes in
the endogenous variables, $\Delta y_t$, which in turn affects their
levels in the following period. This primary effect may then be
compounded in two ways:
\begin{itemize}
\item To the extent that changes in $y_t$ disturb the equilibrium
  conditions represented by the cointegrating vectors in $\beta$, this
  impacts $\Delta y_{t+1}$ via the adjustment coefficients in
  $\alpha$.
\item If the lag order of the CVAR is greater than 1, past changes in
  $\Delta y$ will result in further changes via the $\Gamma_i$ matrices.
\end{itemize}

The apparatus for summing these effects, in a matrix called $C$, is set
out thus in \citet[p.\ 49]{johansen95}:\footnote{In the following equations
our notation differs slightly from Johansen's.}
\[
  C = \beta_{\perp}(\alpha'_{\perp} \Gamma \beta_{\perp})^{-1}
  \alpha'_{\perp} \qquad \Gamma \equiv I - \sum_{i=1}^{p-1}\Gamma_i
\]
For a system with $n$ endogenous variables $C$ is an $n \times n$
matrix; $C_{ij}$ gives the long-run effect of an innovation to variable
$i$ on the level of variable $j$.

For inference regarding these effects one wants associated standard
errors. The asymptotic distribution of the estimator of $C$ is given by
\citet[p.\ 190]{johansen95} as
\[
  T^{\frac12}(\hat{C} - C) \overset{w}{\rightarrow} N(0,
  C\Omega C' \otimes \xi'\Sigma^{-1}\xi)
\]
where (see pages 190, 50, 188, 187 respectively)
\begin{align*}
  \xi' &= ((C'\Gamma' - I)\bar{\alpha}, i'_{p-1}\otimes C') \\
  \bar{\alpha} &= \alpha(\alpha'\alpha)^{-1} \\
  \Sigma &= {\rm Cov}(z_t(\beta)) \\
  z_t(\beta)' &= (y'_{t-1}\beta,\, \Delta y'_{t-1},\, \ldots,\,\Delta y'_{t-p+1})
\end{align*}
Note that the terms appearing in $z_t(\beta)$ are the stationary forms
of the endogenous variables: deviations from equilbrium and first
differences. The covariance of $z_t(\beta)$ is taken to be conditional
on the exogenous terms in the system (including seasonal dummies), if
any.

From the results above we obtain the following expression for the
covariance of the vectorization of $\hat{C}$:
\[
{\rm Cov}({\rm vec}(\hat{C})) = T^{-1}(\xi'\Sigma^{-1}\xi \otimes C\Omega C')
\]
where $T$ is the number of observations. This matrix is $n^2 \times
n^2$; standard errors for the $C_{ij}$s can be found by taking the
square root of its diagonal elements.

\subsection{Accessors}

Following estimation of a regular VAR or VECM in gretl, the
\verb|$system| accessor returns a bundle containing various quantities
of interest. In the cointegrated case there's a sub-bundle called
\dtk{vecm_info} holding additional information, including the
$C$ matrix described above under the name \dtk{long_run} and the
covariance of its vectorization under \dtk{cov_long_run}. As a
convenience we also provide the standard errors of the elements of
$C$ under the key \dtk{se_long_run}.

One can therefore print out the long-run effects with their standard
errors using something like the following code:
\begin{code}
bundle b = $system.vecm_info
matrix C = b.long_run
matrix Cse = b.se_long_run
n = rows(C)
loop i=1..n
   loop j=1..n
      printf "C[%d,%d] = %6.3f (%.3f)\n", i, j, C[i,j], Cse[i,j]
   endloop
endloop
\end{code}
