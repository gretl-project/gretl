...
normalization'', or ``triangular representation''. The starting point
is writing $\beta$ in partitioned form as in
\[
  \beta = \left[
...
(\ldots constraints on $\alpha$ could go here? later?\ldots)
...
\texttt{restrict} command can be given the \texttt{--full} switch, in
which case a new model is created, in which the matrix $\beta$ is
constrained accordingly.

Apart from a few special cases, the resulting estimator has no closed
form solution; hence, the maximum must be found via numerical
methods. Two such methods are available in \app{gretl}. 

(\ldots discuss switching algorithm and LBFGS + sim. ann.\ldots)
...
Cassola (2004). Part of the output is
\begin{code}
