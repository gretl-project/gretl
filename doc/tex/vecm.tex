...
the endogenous variables, $\Delta y_t$, which in turn affect their
...
errors. The asymptotic distribution of the estimator of $C$
\cite[p.\ 190]{johansen95} is
\[
  T^{\frac12}(\hat{C} - C) \overset{w}{\rightarrow} N_{p\times p}(0,
...
\begin{align*}
  ...
  \Sigma &= {\rm Var}(z_t(\beta)) \\
  z_t(\beta)' &= (y'_{t-1}\beta,\, \Delta y'_{t-1},\, \ldots,\,\Delta y'_{t-p+1})
\end{align*}

These results imply the following expression for the covariance of ${\rm
  vec}(\hat{C})$:
\[
T^{-1}(\xi'\Sigma^{-1}\xi \otimes C\Omega C')
\]%
