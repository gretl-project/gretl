...
\dtk{vecm_info} holding additional information, including the $C$ matrix
described above under the name \dtk{long_run} and the covariance of its
vectorization under \dtk{cov_long_run}. One can therefore print out
the long-run effects with their standard errors using something like
the following code:
\begin{code}
bundle b = $system.vecm_info
matrix C = b.long_run
n = rows(C)
Cse = mshape(sqrt(diag(b.cov_long_run)), n, n)
loop i=1..n
   loop j=1..n
