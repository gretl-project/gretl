...
equilibrium in the previous period. Note that when two indices are
given in a restriction on $\alpha$ the indexation is consistent with
that for $\beta$ restrictions: the leading index denotes the
cointegrating vector and the trailing index the equation number.
...
identifying.  \app{Gretl} uses the method suggested by \cite{doornik95},
