...
\app{gretl}.  FIXME they don't in fact do that yet!
...
the meaning of the five options, see section \ref{sec:coint-5cases} below):
\begin{center}
  \begin{tabular}{cc}
    \hline
    $\mu_t$ & command option \\
...
the data follow what appears to be a deterministic trend? And if so,
...
that the data do not fluctuate around a discernible trend. This means
that $\Delta y_t$ is on average zero, so it is reasonable to expect
...
$\mu_1 = 0$ is a safe bet. As for $\mu_0$, there are only two
possible ways that make the above expression true: either $\mu_0 = 0$ or, less
restrictively, $\mu_0$ happens to be exactly equal to $-\alpha
m_z$. This possibility is less restrictive in that the vector $\mu_0$ may be
non-zero, but it is constrained to be a linear combination cof the
columns of $\alpha$. But if this is the case, $\mu_0$ can be written
as $\alpha \cdot c$, and on may write (\ref{eq:VECM-poly}) as
\[
  \Gamma(L) \Delta y_t = \alpha \left[ \beta' \quad c \right] 
  \left[ \begin{array}{c} y_{t-1} \\ 1 \end{array} \right]  
  + \epsilon_t ,
\]
and the long-run relationship contains an intercept.
