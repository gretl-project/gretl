...
\tip{Please note that the matrices $\obsymat$ and (if applicable)
  $\obsxmat$ in the observation equation must be given in
  \textsl{transposed} form. This is required to preserve compatibility
  with gretl versions prior to 2022a. Correspondingly, if you retrieve
  either of these matrices using their keys, \texttt{obsymat} and
  \texttt{obsxmat}, it's the transpose you actually obtain.}
...
Naturally, the arguments you pass to \cmd{ksetup} must have coherent
...
\end{code}
%
Time-variation or no, \cmd{ksimdata} will ensure that the
...
For the purposes of \texttt{ksimul} the time-series length, $N$, is
...
By default, the value returned by \cmd{ksimul} is a ($N \times n$)
...
At this point, the reader is probably aware of the fact that setting
up a state space model could be quite a complex undertaking, and the
only general way to accomplish it is by writing a script.  However,
some cases are simple enough to lend themselves to a standardized
treatment, so that they can be handled via a relatively streamlined
graphical interface.  As of version 2022a, the gretl provides just
this: a GUI interface for estimating a subset of state space models
that, despite limited, may still be useful for didactical purposes,
sparing the user from the intricacies of scripting.  In this section,
we describe the GUI interface and the class of models it can be adapted
to.
...
\end{eqnarray}
where $V(\obsdist_t)$ is a diagonal matrix, possibly 0, in which case
the last term of equation (\ref{eq:GUIobs}) is dropped. As for the
covariance matrix of the shocks to equation (\ref{eq:GUIstate}), it is
assumed that $\steta_t$ is an IID sequence of normal random variable
with diagonal covariance matrix $\Sigma_\steta$. Therefore, the
covariance matrix we have been indicating with $\statevar$ so far
(whose corresponding key in the Kalman bundle is \cmd{statevar}), is
assumed to be $\statevar = R \Sigma_\steta R'$. Note that $\strR$ can have
fewer columns than $r$, thereby making $\statevar$ singular.

The system matrices $\obsymat$, $\statemat$ and $\strR$ are assumed to
be known, so estimation only concerns the variances of $\obsdist_t$
and $\steta_t$.  Clearly, this is a very limited subset of the range
of models that gretl can handle, but it may be of some pedagogical
value.
...
with the limited-memory version of the BFGS optimizer. For reasons of
numerical performance, it is convenient to have the choice of
representing variances as transformations of the BFGS parameters in
...
errors whatever the parametrization type.  Once the parameters are
estimated the user has the choice of performing the smoothing of the
states.

The GUI hook is shown in figure \ref{fig:GUI}: the ``observables'' box
is used for specifying a list of series for $\obsvec_t$. The next two
boxes handle the $\obsymat$ and $\statemat$ matrices,
respectively. These can be pre-existing matrices or created ``on the
fly''. The same applies for the next box, dedicated to the $\strR$
matrix. However, the $R$ matrix could be omitted, in which case it is
implicitly assumed $\strR = I$.  The remaining GUI elements should
...
and $V(\steta_t) = 1/8$, and sets up the two matrices \texttt{Z} and
\texttt{T} so that the can be entered into the second and third box of
the GUI helper, respectively; obviously, the first box will have to
contain the string \texttt{y}.
...
bundle. For example, the \texttt{kmod} key correspondes to the
estimated kalman bundle. Saving it under the name \texttt{b} and
running the code below will produce the plot shown in Figure \ref{fig:rw+seas}.
