...
\textbf{Please note}: The matrix $\obsymat$ must be given in
transposed form to the \texttt{ksetup} function. This is
(unfortunately) required to preserve compatibility with gretl versions
prior to 2022a. So, for example, you might write
\begin{code}
bundle SSmod = ksetup(y, Z', T, Q)
\end{code}
where $\obsymat$ is $n \times r$. This point also applies to the
matrix $\obsxmat$ in the observation equation, but not to any other
matrices.
