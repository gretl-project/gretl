...
\tip{By default, when you build a matrix from series that include
  missing values the data rows that contain \texttt{NA}s are skipped.
  But you can modify this behavior via the command
  \texttt{set skip\_missing off}.  In that case \texttt{NA}s are
  converted to NaN (``Not a Number'').  In the IEEE floating-point
  standard, arithmetic operations involving NaN always produce NaN.}
...
\item Single-equation models: \texttt{\$sigma} gets a scalar (the
  standard error of the regression); \texttt{\$coeff} and
  \texttt{\$stderr} get column vectors; \texttt{\$uhat} and
  \texttt{\$yhat} get series.
\item System estimators: \texttt{\$sigma} gets the cross-equation
  residual covariance matrix; \texttt{\$coeff} and \texttt{\$stderr}
  get matrices with one column per equation, as do \texttt{\$uhat}
  and \texttt{\$yhat}
\item VECMs: \texttt{\$vcv} is not available at present when $\alpha$
...
If the model in question a VAR or VECM (only) \dollar{compan} returns
the companion matrix.

After a vector error correction model is estimated via Johansen's
procedure, the matrices \dollar{jalpha} and \dollar{jbeta} are
