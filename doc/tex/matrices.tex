...
scalar $k$, \verb|B = A^x| produces a matrix $B$ which is $A$ raised
to the power $x$; if $A$ is a generic square matrix, Golub and Van
Loan's ``Binary Powering'' Algorithm 11.2.2 is used---see
\cite{golub96}--- and $x$ must be a non-negative integer. If instead
$A$ is positive semidefinite, the power is computed via its
eigen-decomposition, and $x$ can be a real number; note, however, that
$x$ can be positive only if $A$ is invertible.
