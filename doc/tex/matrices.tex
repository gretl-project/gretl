...
scalar $k$, \verb|B = A^k| produces a matrix $B$ which is $A$ raised
to the power $k$, but only if either $k$ is a non-negative integer, in
which case Golub and Van Loan's ``Binary Powering'' Algorithm 11.2.2
is used---see \cite{golub96}---or $A$ is positive definite, in which
case the power is computed via its eigen-decomposition.
