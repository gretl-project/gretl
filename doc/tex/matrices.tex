...
\texttt{r} variants) or a $1 \times n$ matrix (the \texttt{c}
...
\]
where $Z_{ti}$ is the standardized observation $t$ on variable $i$,
$Z_{ti} = (X_{ti} - \bar{X}_i) / \hat{\sigma}_i$, and $v^{(j)}$ is the
$j$th eigenvector of the correlation matrix of the $X_i$s, with the
eigenvectors ordered by descending value of the corresponding
eigenvalues.
