...
please refer to the \emph{Gretl Command Reference}.
...
\texttt{\$sigma}  & residual covariance matrix$^{\dagger}$ \\
\texttt{\$stderr} & vector of estimated standard errors \\
\texttt{\$uhat}   & matrix of residuals$^{\dagger}$ \\
\texttt{\$vcv}    & covariance matrix of parameter estimates \\
\texttt{\$yhat}   & matrix of fitted values$^{\dagger}$ 
\end{tabular}
\caption{Matrix accessors for model data}
\label{tab:matrix-accessors}
\end{table}

The accessors in Table~\ref{tab:matrix-accessors} marked with
$^{\dagger}$ are specific to models that take the form of systems of
equations: \texttt{\$sigma} gives the cross-equation residual
covariance matrix, and \texttt{\$uhat} and \texttt{\$yhat} give the
matrices of residuals and fitted values, respectively (one column per
equation).  In the single-equation case, however, \texttt{\$sigma}
gives a scalar (the standard error of the residuals), while
\texttt{\$uhat} and \texttt{\$yhat} give series.
