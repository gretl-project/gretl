...
\texttt{maxcidx}   & column maxima (indices) \\ 
\texttt{maxridx}   & row maxima (indices) & &
\texttt{minc}      & column minima (values) \\   
\texttt{minr}      & row minima (values) &  &
\texttt{mincidx}   & column minima (indices) \\ 
\texttt{minridx}   & row minima (indices) 
\end{tabular}
\end{center}
...
For an $m \times n$ matrix $A$, the \texttt{max} and \texttt{min}
family of functions return either a column vector of length $m$ (the
\texttt{r} variants) or a row vector of length $n$ (the \texttt{c}
variants).  The \texttt{max} vectors contain the values of the row or
column maxima while the \texttt{min} ones hold the row or column
minima.  The variants with an \texttt{idx} postfix return not the
values but the (1-based) indices of the respective maxima or minima.
