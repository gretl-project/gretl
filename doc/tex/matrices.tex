...
which fails if $B$ is singular.  If $B$ is a $T \times k$ matrix with
$T > k$, then $C$ is the least-squares solution, $C = (B'B)^{-1}B'A$,
which fails if $B'B$ is singular.  Otherwise the operation necessarily
fails.
...
\texttt{inv}      & inverse & &
\texttt{invpd}    & inverse of p.d. matrix \\
...
The \texttt{invpd} function returns the inverse of a symmetric,
positive definite matrix.  Use with caution: this is faster than the
standard \texttt{inv} function, but \app{gretl} does not check for
symmetry; it takes you at your word.
