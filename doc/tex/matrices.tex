...
Matrices can be created using these four methods:
...
\texttt{'} & multiplication by transpose \\
...
By ``multiplication by transpose'' we mean, for example, that 
%
\begin{code}
matrix C = X'Y
\end{code}
...
For a variant on element-wise multiplication that may be useful in
some contexts, see also the \texttt{colmult()} function below
(page~\pageref{matrix-two}).
...
distinct integer values in the input vector, sorted in ascending
order.
...
For a matrix $A$ with $T$ rows and $k$ columns, \texttt{mcov(A)} and
\texttt{mcorr(A)} both return $k \times k$ symmetric matrices, in the
first case containing the variances (on the diagonal) and covariances
of the variables in the columns of $A$, and in the second, containing
...
The function \texttt{colmult()} carries out column-wise multiplication
of two matrices, if they are conformable.  The syntax is
%
\begin{code}
C = colmult(A, B)
\end{code}
%  
Let $A$ be an $m \times n$ matrix and $B$ be $m \times k$.  By
column-wise multiplication we mean: if $n = 1$ then $c_{ij} = a_i
\times b_{ij}$; otherwise if $k = 1$ then $c_{ij} = a_{ij} \times b_{i}$;
otherwise if $k = n$ then $c_{ij} = a_{ij} \times b_{ij}$; otherwise
the result is undefined and an error is flagged.
...
preceded by \verb+&+ (to indicate the ``address'' of the matrix in
question), in which case an auxiliary result is written to that
matrix, or the keyword \texttt{null}, in which case the auxiliary
result is not produced, or is discarded.
...
To print a matrix, simply give the name of the matrix in question
on a line by itself:
\begin{code}
