...
\end{code}
%
generates a matrix such that $b_{ij} = \sqrt{a_{ij}}$.  (To find the
``matrix square root'' you need the \texttt{cholesky} function --- see
below).  All of these functions require a single matrix as argument,
...
The following code exemplifies the use of this sort of conversion to
``normalize'' a list, moving the constant (variable 0) to first
position.
