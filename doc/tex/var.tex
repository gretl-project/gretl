...
\]
so in this case $\Theta_i = A^i$. In general, it is possible to compute
...
of using the dedicated command is that, after estimation is done, it
makes it much easier to access certain quantities and manage certain
tasks. For example, the \dollar{coeff} accessor returns the estimated
coefficients as a matrix with $n$ columns and \dollar{sigma} returns
an estimate of the matrix $\Sigma$, the covariance matrix of
$\epsilon_t$.
...
$\Theta_i$ matrices are computed automatically. By default, this is a
...
the $(i+1)$-th row is the vectorized form of $\Theta_i$.
...
The most popular choice dates back to \cite{sims80} \ldots Cholesky
$\to$ ``most exogenous first'' mantra \ldots
