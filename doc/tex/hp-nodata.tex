...
algebraic operators with their traditional precedence rules: the caret
(\verb|^|) is used for exponentiation. So for example
\begin{code}
...
\item The usage of \verb|$pi| for the constant $\pi$. In a hansl
  script, you have certain identifiers with a ``dollar'' prefix, which
  work as read-only variables. Most of them are used in the presence
  of an open dataset (see part \ref{part:hp-data}), but some are just
...
A few constructs are borrowed from the C language family: one is the
...
\begin{code}
  ...
  print beer_bottles
\end{code}
That is, \texttt{x++} is the same as \texttt{x = x+1}. Note that this
only works with addition and that the C ``prefix'' operators are not
supported
...
Treatment of \texttt{NA}s.

The \cmd{eval} command
