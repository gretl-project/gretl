...
\cmd{toupper}, \cmd{tolower}, \cmd{strlen} etcetera, plus some more
...
moreover, they can contain special characters such as line breaks,
etcetera. As a consequence, it is possible to use hansl for performing
...
a very long string and then operating on that;\footnote{Of course, the
  ...
  for example \app{perl} or \app{python}. However, on modern hardware
  hansl may be a very defensible choice for text files up to a few
  hundreds megabytes.} interested readers should take a look at the
\cmd{readfile} and \cmd{getline} functions in the \GCR.

For \emph{creating} complex strings, the most flexible tool is
arguably the \cmd{sprintf} function. Its usage, however, is best
...
To give you more control over output, hansl provides you with
the \cmd{printf} function, which works in a very similar way as in a
...
The \cmd{printf} function gives you extreme flexibility in what to
...
Technically, \cmd{printf} is a function returning a scalar. Its real
job, however, is to do formatted printing of text. In fact, what gets
printed is the format string; this must be an expression that
evaluates to a string, but in most cases will just be a constant
string, that is an alphanumeric sequence surrounded by double
quotes. However, some character sequences in the format string have a
...
\tip{Important: the ``command vs function'' thing.}
