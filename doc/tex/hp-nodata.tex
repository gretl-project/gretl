...
\cmd{printf} command can be used for greater control over output.
...
\item The name of the variable (its \texttt{identifier}) must follow
...
commands. Describing the \texttt{set} command in detail would lead to
an overly long diversion; suffice it to say that here it is used as a
way to silence unwanted output. See chapter \ref{chap:settings} or the
\GCR{}.

\section{Manipulation of scalars}

Algebraic operations, obviously, and functions. 
\begin{code}
  a = 42
  b = a * 0.5
  c = sqrt(a*b)
