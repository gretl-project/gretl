...
suppose you know) is an open-source, cross-platform econometrics package
(\url{http://gretl.sourceforge.net/}). Support for \textsf{libsvm} in
gretl was added in the \texttt{2017d} release Since then several
improvements have been made and at present you'll be better off using
current gretl to work with this.
...
In general the workflow for supervised machine learning goes like this:
\begin{enumerate}
...
\item Using this model, generate predictions for the testing data and
  assess the fit.
\item If the out-of-sample performance is deemed satisfactory, the model
  may then be used to generate ``live'' predictions for additional data.
\end{enumerate}
...
\item Gretl examines your dependent variable (the first member of
  \texttt{L}): if this is a binary dummy, or its ``coded'' attribute is
  set, the $C$-SVC SVM type is selected (classification), otherwise
  $\epsilon$-SVR (regression) is selected. FIXME multinomial case. In
  both cases the default kernel is RBF (radial basis function).
...
smpl full
bundle parms = _(n_train = ntrain, quiet = 1)
list L = y X
...
list L = dataset
\end{code}
This works if your dependent variable occupies position 1 in the
dataset and you wish to use all the other series in the dataset as
...
open train.gdt
list L = dataset
bundle b = _(ranges_outfile = "train.ranges")
bundle savemod = defbundle()
svm(L, b, &savemod)

open test.gdt --preserve
list L = dataset
bundle b = _(ranges_infile = "train.ranges", loadmod = 1)
svm(L, b, &savemod)
\end{scode}
\end{script}
...
open train.gdt
list L = dataset
bundle b = _(ranges_outfile = "my.ranges", model_outfile = "my.model")
svm(L, b)

# script 2
open test.gdt
list L = dataset
bundle b = _(ranges_infile = "my.ranges", model_infile = "my.model")
svm(L, b)
\end{scode}
\end{script}
...
\begin{code}
bundle parms = _(svm_type = 4) # select nu-SVR
\end{code}
...
should be done on both the training data used to build the model (to get
a measure of fit) and also on any testing data present. By setting
\texttt{predict} = 1 you are requesting that prediction be done for (at
most) only the training set. By setting it to 0 you are telling gretl
...
The \texttt{ranges} keys work in a similar manner, letting you carry
the data-scaling ranges computed on the training data over to a
separate script or session in which prediction is performed---see
Listing~\ref{listing:s2-separate} above.

\section{Cross validation}
\label{sec:xvalid}

\textsf{Libsvm} provides a cross-validation function that works as
follows. Given a number of ``folds'', $v \geq 2$, the data are divided
randomly into $v$ equally sized subsets,\footnote{That is by default,
  but see also section~\ref{sec:user-folds} below.} then for each subset
$i$ a sub-model is trained on the complementary subset of the data (the
other $v-1$ folds) and predictions are generated for subset $i$. Once
this process is complete we have ``pseudo out of sample'' predictions
for all the data supplied (which should be either the full training
dataset or perhaps a subset thereof), and suitable figures of merit can
be calculated for these predictions.
...
smpl 1 1000
bundle b = _(grid = gmat, search_only = 1)
...
open data.gdt
list L = dataset
bundle b = _(n_train = 2000, search = 1)
series yhat = svm(L, b)
\end{scode}
\end{script}
...
list L = lny X
bundle b = _(n_train = 3000, shrinking = 0, search_only = 1)
b.grid = {-1, 5, 2; 3, -12, -2}
b.seed = 1611771
...
bundle parms = _(n_train = ntrain, search = 1)
...
\textsf{libsvm} C++ source file \texttt{svm.cpp}. It has been edited to
support parallelization via \textsf{OpenMP} when the symbol
\dtk{_OPENMP} is defined. The changes are as described in answer to
``How can I use OpenMP to parallelize LIBSVM on a
multicore/shared-memory computer?'' in the file \texttt{FAQ.html} in the
\textsf{libsvm} distribution. In addition, the pseudo-random numbers
used in cross validation are taken from the RNG used in libgretl
(Blackman and Vigna's \textsf{xoshiro256+}) instead of the C library's
\texttt{rand}.
