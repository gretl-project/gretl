...
argument is \dtkttt{n_train}, which sets the number of observations
...
\item Gretl finds the maxima and minima of the independent variables
  in the sample range 1 to \dtkttt{n_train} and scales them onto
  [$-1,1$].
...
\item If all has gone OK, gretl checks if you have enough observations
  ...
  least 10 observations between \dtkttt{n_train} and the end of the
...
file named \dtkttt{ahs_jep.gdtb}. This is a native gretl version of
...
\begin{table}[p]
  (a) Values for \dtkttt{svm_type} (default automatic):
  \begin{center}
  \begin{tabular}{clll}
...
4 & \verb|nu-SVR| & $\nu$-SVR & regression
  ...
  (b) Values for \dtkttt{kernel_type} (default 2, RBF):
  \begin{center}
  \begin{tabular}{clll}
...
\texttt{probability} & produce probability estimates, 0 or 1 (default 0)
  ...
  \begin{tabular}{ll}
    ...
    \dtkttt{n_train} & integer: the number of training observations \\
    ...
    \dtkttt{model_outfile} & filename for writing model \\
    \dtkttt{model_infile} & filename for reading model \\
    \dtkttt{ranges_outfile} & filename for writing data ranges \\
    \dtkttt{ranges_infile} & filename for reading data ranges \\
    \dtkttt{data_outfile} & filename for writing data values \\
    \dtkttt{range_format} & string: \texttt{libsvm} or \texttt{gretl}
  \end{tabular}
  \end{center}
...
\dtkttt{n_train}: has already been discussed: it simply tells gretl
...
then specifying \dtkttt{n_train} = 5000 will result in the use of
...
\dtkttt{model_infile} can be used to get gretl to read a previously
trained \textsf{libsvm} model file (which implies that \texttt{svm}
will skip the training step), and \dtkttt{model_outfile} can be used
...
Finally, \dtkttt{data_outfile} can be used to write data in the
format required by \texttt{svm-train} and \texttt{svm-predict}. Note
that there is no \dtkttt{data_infile} key: \texttt{svm} only accepts
...
\begin{table}[htbp]
 ...
 \textit{or} & \texttt{grid} & matrix: user-specified grid \\[4pt]
    & \dtkttt{search_only} & 0/1: see text \\
...
The \dtkttt{search_only} option has the effect of stopping
...
\dtkttt{n_train} if that is specified in the parameter bundle. Once
...
You should define \dtkttt{search_only} if you just want to find and
...
argument to \texttt{svm}, a matrix named \dtkttt{xvalid_results} is
...
\dtkttt{search_only} is the only cross-validation option given, this
...
\begin{script}[htbp]
  \caption{Parameter search, script \dtkttt{ca_xvalid.inp}}
...
\end{tabular}
\caption{Execution times (m:s) for \dtkttt{ca_xvalid.inp} (OMP =
...
However, the \dtkttt{svm_type} switch in the parameter bundle passed
to \texttt{svm} can be used to force the issue in favor of
classfication via $C$-SVC (\dtkttt{svm_type = 0}) or via $\nu$-SVC
(\dtkttt{svm_type = 1}).
...
form of a scalar value under the key \dtkttt{svr_sigma}: this is an
...
``\dtkttt{yhat_svm = svm(All, parms)}'', insert:
%
\begin{code}
parms.probability = 1
bundle bprob = defbundle()
...
\dtkttt{_OPENMP} is defined. The changes are as described in answer
