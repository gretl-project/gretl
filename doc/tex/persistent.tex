...
Versions of \app{gretl} higher than 1.6.0 offer this facility.
...
the \texttt{printf} command except that the ``format'' string must be
preceded by the name of a string variable.  For example,
%
\begin{code}
scalar x = 8
sprintf foo "var%d", x
...
of the variable preceded by the ``at'' sign, \verb|@|.  If a sequence
of characters in a \app{gretl} command following the symbol \verb|@|
is recognized as the name of a string variable, the value of that
variable is sustituted into the command line.  This is illustrated in
