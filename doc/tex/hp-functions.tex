...
\item The lines below the function definition give a simple example of
  ...
  \cmd{ln()} and \cmd{quasi\_log()} are indistinguishable from a
...
it somewhat unpleasant to use. Some programming languages (R, for one)
...
\item The next lines simply repeat the computation of the return
  string using the bundle contents.
\end{enumerate}

This means that a call to \texttt{Sub("nowhere")} would yield
\texttt{now}, but if we wanted the ouput to be capitalized, we could
have called the function as
\begin{code}
  bundle myopts = defbundle("capital", TRUE)
  string out = Sub("nowhere", myopts)
\end{code}
and the string \texttt{out} now contains \texttt{NOW}. Note that the
two lines could be coealesced as
\begin{code}
  string out = Sub("nowhere", defbundle("capital", TRUE))
\end{code}
but in some cases it may be convenient to keep the options bundle as a
persistent object so that successive calls to the function may happen
with incremental changes. For example, the following code:
\begin{code}
  bundle myopts = defbundle("capital", TRUE)
...
\end{code}
will result in the string \texttt{out1} containing \texttt{NOW} and
\texttt{out2} containing \texttt{NO}.

Moreover, from gretl version 2021a, the function \texttt{defbundle}
