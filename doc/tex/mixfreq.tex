\chapter{Handling mixed-frequency data}
\label{chap:mixed-frequency}

\section{Basics}
\label{sec:mixed-basics}

In some cases, one may want to handle data that are sampled at
different frequencies. The typical example is GDP, which is usually
available quarterly, and industrial production, often available
monthly. The most common context when this feature is required is
specification and estimation of MIDAS models (see Chapter
\ref{chap:MIDAS}), but other cases are possible.

Since a \app{gretl} dataset formally handles only a single data
frequency, we have adopted a relatively straightforward solution: a
higher frequency series $x_H$ is represented by a set of $m$ series,
each holding the value of $x_H$ in a sub-period of the ``base''
(lower-frequency) period (where $m$ is the ratio of the higher
...
the analysis. Then a relevant fragment of the \app{gretl} dataset might look
...
monthly data. Section \ref{sec:midas-daily} has details on our
...
Louis Fed) database which is supplied in the \app{gretl} distribution:
\begin{code}
clear
open fedstl.bin
data gdpc96
...
\begin{script}[htbp]
  \caption{Building a \app{gretl} MIDAS dataset via merger}
...
file, so we use \app{gretl}'s \texttt{rename} command to set distinct and
...
\end{code}
%
which tells \app{gretl} to compact an entire dataset (in this case, as it
...
is provided in the \app{gretl} package (\texttt{gdp\_midas}, which you can
find under the \textsf{Gretl} tab in the practice datafiles window
in the \app{gretl} GUI).
...
to tell \app{gretl} that it's a \textit{bona fide} MIDAS list:
%
\begin{code}
setinfo INDPRO --midas
\end{code}
%
This will spare you some warnings that \app{gretl} would otherwise emit when
...
frequency. Figure~\ref{fig:data-menu} shows the \app{gretl} main window with
...
long-standing behavior of \app{gretl}'s \texttt{lags} function; it is made
...
We could construct such a list in \app{gretl} using the following
standard syntax. (Note that the third argument of 1 to
\texttt{lags} below tells \app{gretl} that we want the terms ordered ``by
...
In \app{gretl} we have chosen to employ the first of these
...
ordinary \app{gretl} functions \texttt{diff} and \texttt{ldiff} will
...
have the same effects as with the \app{gretl}'s \texttt{gnuplot} command.
