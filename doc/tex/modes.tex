...
\app{gretl} Help item ``Script commands syntax,'' or start up the
command-line program \app{gretlcli} and consult its help, or consult
the \emph{Gretl Command Reference}. In addition, the \app{gretl}
...
introduction to scripting in \app{gretl} and to various points of
...
\item Start by establishing a new script file.  Type in any commands
  ...
  \cmd{genr} command in the \emph{Gretl Command Reference}). Typically
...
\app{gretl}'s Utilities menu you will find the item ``Gretl console''
...
``Graph specified vars'', or via one of \app{gretl}'s other
...
the models.  In the Icon View window \app{gretl} provides a means of
...
Rich Text Format).  Here is how to do it:\footnote{The model table can
  ...
  to do this, see the entry for \cmd{modeltab} in the \emph{Gretl
    Command Reference}.  }
...
A simple instance of \app{gretl}'s model table is shown in Figure
\ref{fig-model-table}.
\begin{figure}[htbp]
...
output.\footnote{Specifically, you must have \app{dvips} and
  \app{ghostscript} installed, along with a viewer such as \app{gv},
  \app{ggv} or \app{kghostview}. The default viewer for systems other
  than MS Windows is \app{gv}.}  In the Icon View window, you can drag
...
re-examine later, then before quitting \app{gretl} select ``Save
...
\item From the command line, type \cmd{gretl -r} sessionfile, where
