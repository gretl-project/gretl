...
Note that there is no way at present to run just a portion of a
script. However, it is possible to keep several script windows open at
the same time, so if you need to run only a subset of the commands
contained in a script, you can open a new script window, cut/paste the
relevant portion into the new window and run that.
