...
\label{scripts}
      ...
      As you execute commands in \app{gretl},
      ...
      \app{gretl} or the command-line client,
      \app{gretlcli}.
      To view the current state of the script at any point in a
      \app{gretl} session, choose ``View
...
\end{center}
\end{figure}
The toolbar at the top of the script window offers the
...
\item Start by establishing a new script file.  Type in any
	  ...
	  (see the \cmd{genr} command in the \emph{Gretl
...
Models saved in this way will appear as icons in the
      \app{gretl} session window (see Section \ref{session}) after the script is executed.  In
...
A further option is available for your computing
      convenience. Under \app{gretl}'s Utilities
      ...
      \app{gretlcli}'s mode of operation, except
...
\label{session-intro}
	...
	\app{gretl} offers the idea of a
...
\end{figure}
	...
	If you start \app{gretl} and open a data
	...
	\app{gretl}'s other graph-generating
...
\label{model-table}
	...
	In the Icon View window \app{gretl}
	...
	do it:\footnote{The model table can also be built
	    ...
	    do this, see the entry for \cmd{modeltab} in the
...
A simple instance of \app{gretl}'s
...
The ``graph page'' icon in the session window
	...
	output.\footnote{Specifically, you must have
	    \app{dvips} and
	    \app{ghostscript} installed, along
	    with a viewer such as \app{gv},
	    \app{ggv} or
	    \app{kghostview}. The default viewer
