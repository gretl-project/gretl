...
methods include non-linear least squares (\cmd{nls}), generic 
maximum-likelihood estimators (\cmd{mle}), generic GMM (\cmd{gmm}),
dynamic panel models (\cmd{dpanel} or its predecessor \cmd{arbond}),
interval regressions (\cmd{intreg}), bivariate probit models 
(\cmd{biprobit}), AR(I)MA models (\cmd{arima} or \cmd{arma}), and
(G)ARCH models (\cmd{garch} and \cmd{arch}).
