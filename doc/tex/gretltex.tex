...
section~\ref{tex-install} gives some pointers on installing \TeX\ (and
learning) if you do not already have it on your computer.  (Just to be
clear: \TeX\ is not included with the \app{gretl} distribution; it is
a separate package, including several programs and a large number of
supporting files.)
...
particuarly useful for publication purposes is the ability to produce
a typset version of the ``model table'' (see
...
Beyond that you can, for instance, change the type size or the font by
altering the \texttt{documentclass} declaration or including
additional packages.
...
MATERIAL on editing gretl's output.
...
As for learning \TeX, many useful resources are avauilable both online
and in print.  Among these, we recommend XXX
