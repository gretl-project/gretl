...
\item (more commonly nowadays) a program called \app{pdflatex} that
  generates PDF output.\footnote{Experts will be aware of something
    ...
    \app{Gretl} does not support plain \TeX.}
\end{itemize}
...
On MS Windows and Mac OS X, \app{gretl} calls \app{pdflatex} to
...
In addition, if you wish to typeset \app{gretl} output in more than
