...
\item \textsf{Tools menu}
  ...
  \item \textsf{P-value finder}: Look up p-values from the Gaussian,
    ...
    distributions. See also the \cmd{pvalue} command in the
  ...
  \item \textsf{Preferences}: Set the paths to various files
    ...
    messaging about the availability of program updates.  Configure
    the main-window toolbar, and so on. See the \GCR\ for further
    details.
  \end{itemize}
...
under the ``Tools, Preferences, General'' menu: just uncheck the
bix labeled ``show gretl toolbar''.
