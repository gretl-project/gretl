...
for details on the command-line program, \app{gretlcli}.
...
\verb+data3-6+.  \tip{In \app{gretl} windows containing lists,
  double-clicking on a line launches a default action for the
  associated list entry: e.g. displaying the values of a data series,
  opening a file.}  This file contains data pertaining to a classic
...
You can control the appearance of \app{gretl}'s {\LaTeX} output using
a file named \verb+gretlpre.tex+, which should be placed in your
\app{gretl} user directory (see the \emph{Gretl Command Reference}).
...
Note that the \verb+amsmath+
 and \verb+dcolumn+ packages are required.To import \app{gretl} output
 ...
 the target program.  Many (not all) \app{gretl} windows offer the
 ...
 target program.  When you finish a \app{gretl} session you are given
 the option of saving all the output from the session to a single
 file.Note that on the \app{gnome} desktop and under MS Windows, the
 \textsf{File} menu includes a command to send the output directly to
 a printer.\tip{When pasting or importing plain text \app{gretl}
 output into a word processor, select a monospaced or typewriter-style
 font (e.g. Courier) to preserve the output's tabular formatting.
 Select a small font (10-point Courier should do) to prevent the
...
\item \textsf{Sample menu}
  ...
  \item \textsf{Restrict, based on criterion}: Similar to the item
    ...
    See the entry for \cmd{genr} in the \emph{Gretl Command Reference}
