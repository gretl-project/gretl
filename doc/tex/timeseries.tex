...
\cite{ERS96} proposed a variant of the ADF test which involves an
...
99\% quantiles. The critical values are computed via the method
presented by \cite{sephton95}, which offers greater accuracy than
the values tabulated in \citep{KPSS92}.
...
$\varphi = 0$ and the alternative is that $\varphi < 0$. While the
null still says that all the individual series are non-stationary, the
alternative now says that they are \emph{all} stationary.

TODO: finish this discussion and tie in with the gretl commands.


\subsection{Cointegration tests}
\label{sec:coint-test}

TODO: discuss Engle---Granger here, and refer forward to
