...
integrated, moving average (ARIMA) models.  The most general ARIMA
model that can be estimated by \app{gretl} may be represented as
follows:
\begin{equation}
  \label{eq:general-arma}
  \phi(L) \Phi(L^s) (1-L)^d (1-L)^D \left(y_t - x_t \beta\right) = 
    \theta(L) \Theta(L^s) \epsilon_t
\end{equation}
where $\phi()$, $\Phi()$, $\theta()$ and $\Theta()$ are polynomials in
the lag operator, $L$, defined such that $L^n x_t = x_{t-n}$; $s$ is
the data frequency for seasonal time series (e.g.\ 4 for quarterly, 12
for monthly series); $x_t$ is a vector of exogenous variables; and
$\epsilon_t$ is a white noise process.  This specification allows for
both non-seasonal and seasonal components throughout: $\phi(L)$ is the
non-seasonal AR lag polynomial and $\Phi(L^s)$ the seasonal
counterpart; $\theta(L)$ is the non-seasonal MA polynomial and
$\Theta(L^s)$ its seasonal counterpart; and $d$ and $D$ are the orders
of non-seasonal and seasonal differencing respectively.

If this seems a little indigestible at first, let's back up to a more
basic specification.  For many purposes an ARMA model with no
differencing and no seasonal component may be adequate.  Such a model
can be written as
\begin{equation}
  \label{eq:plain-arma}
...
constant $\mu$.  The equation above may be written more explicitly
as
\begin{equation}
  \label{eq:plain-arma2}
  y_t - \mu = \phi_1\left(y_{t-1} - \mu \right) + \ldots + 
  \phi_p \left( y_{t-p} - \mu \right) + 
...
At this point we must digress briefly to address the fact that the
basic ARMA model with constant can be represented in either of two
ways.  One we have given above (\ref{eq:plain-arma2}); the alternative
is
\begin{equation}
  \label{eq:plain-arma3}
y_t = \alpha + \phi_1 y_{t-1} + \ldots + \phi_p y_{t-p} +
 \epsilon_t + \theta_1 \epsilon_{t-1} + \ldots + \theta_q
 \epsilon_{t-q} 
\end{equation}
%
These specifications are essentially equivalent, but if they represent
one and the same process, $\mu$ and $\alpha$ are, fairly obviously,
not numerically identical; rather
\[
\alpha = \left(1 - \phi_1 - \ldots - \phi_p\right) \mu
\]
From the software user's point of view it doesn't matter which
convention is followed, provided it is clear whether the number
presented as the ``constant'' is $\hat{\mu}$ or $\hat{\alpha}$.  With
one exception to be discussed below, \app{gretl} presents $\hat{\mu}$.

If you want to estimate a model with explanatory variables, the above
syntax must be extended as in the following example
\begin{code}
  arma p q ; y const x1 x2
\end{code}
This command would, by default, estimate the following model:
\[
...
x_{t,2} \beta_2$.  This is sometimes known as a ``regression model
with ARMA errors'', and is perhaps more clearly represented by the
pair of equations:
\begin{eqnarray*}
  y_t & = & x_t \beta + u_t \\
...
\begin{equation}
\label{eq:reg-arma}
  \phi(L) y_t = \phi(L) \left(x_t \beta \right) + \theta(L) \epsilon_t
\end{equation}

We now approach a tricky point.  The model just presented is also
sometimes known as ``ARMAX'' (ARMA + eXogenous variables).  It seems
to us, however, that this label is more appropriately applied to a
different model, namely,
\[
  y_t = x_t \beta + \phi_1 y_{t-1} + \ldots + \phi_p y_{t-p} + 
  \epsilon_t + \theta_1 \epsilon_{t-1} + \ldots + \theta_q \epsilon_{t-q}
\]
or, equivalently,
\begin{equation}
...
The formulation (\ref{eq:transfer}) has the advantage that $\beta$ can
be immediately interpreted as the vector of marginal effects of the
$x_t$ variables on the conditional mean of $y_t$.  And by adding lags
of $x_t$ to this specification one can estimate \emph{Transfer
  Function models} (which generalize ARMA by adding the effects
of exogenous variable distributed across time).

We can perhaps clarify the issue broached above by writing a more
general specification that nests the alternatives; that is
\begin{equation}
 \label{armax-general}
  \phi(L) y_t = \phi(L) \left(x_t \beta\right) + z_t \delta 
 + \theta(L) \epsilon_t
\end{equation}

The practical question from the user's point of view is: when you add
exogenous variables to an ARMA specification in \app{gretl}, are they
treated as the $x_t$s in (\ref{armax-general}) or as the $z_t$s?  Or
do you have a choice?  The short answer is that they are treated as
$x_t$s by default, but you can arrange to have them treated as $z_t$s
by passing the \verb|--conditional| option to the \cmd{arma} command,
\textit{without} adding the \verb|--x-12-arima| option.  

There are four options for ARMA estimation --- in effect a $2 \times
2$ matrix with one dimension being ``native'' ARMA versus use of the
helper program \app{X-12-ARIMA} and the other being exact Maximum
Likelihood versus conditional ML.  (For more on these options see the
section on ``Estimation'' below.)  At present the native, conditional
option is the odd man out, treating exogenous regressors as in
(\ref{eq:transfer}), not as in (\ref{eq:reg-arma}).  In line with this
difference, the native, conditional option also reports the ARMA
``constant'' (if present) as in (\ref{eq:plain-arma3}), not
(\ref{eq:plain-arma2}).

Ideally, we would like to generalize the \cmd{arma} command so that
...
that display strong seasonal patterns.  For such cases, a fuller form
of the syntax is available, namely,
\begin{code}
  arma p q ; P Q ; y
\end{code}
...
As mentioned above, ARMA models may be estimated using \app{gretl}'s
own code, or with the help of \app{X-12-ARIMA}.  We begin by
discussing the \app{gretl} methods.

The default method is exact maximum likelihood estimation (under the
assumption that the error term is normally distributed), using the
Kalman filter in conjunction with the BFGS maximization algorithm.
The gradient of the log-likelihood with respect to the parameter
estimates is approximated numerically.  This method produces results
that are directly comparable with many other software packages.  The
constant, and any exogenous variables, are treated as in equation
(\ref{eq:reg-arma}).  The covariance matrix for the parameters is
computed via the OPG (Outer Product of the Gradients) method.

The alternative ``native'' method is conditional maximum likelihood
(CML), also known as ``conditional sum of squares'' --- see Hamilton
(1994, p.\ 132).  This method was exemplified in the
...
the observations $t_0, \ldots, T$.\footnote{The numerical method used
  is BHHH.  As with exact ML, the covariance matrix is computed via
  the OPG method.}  The starting point $t_0$ depends on the orders of
the AR polynomials in the model.
...
constant and exogeneous variables as per equation (\ref{eq:transfer}).
...
reported constants differ widely, as expected.  However, dividing the
CML constant by $1-\phi$ we get 7.38, which is not far from the ML
estimate of 6.93.
...
ADD short section on X-12-ARIMA here.
...
models with gaps in the lag structure, other than via the seasonal
specification discussed above.  For example, if you have a monthly
time series, you cannot estimate an ARMA model with AR terms (or MA
terms) at just lags 1, 3 and 5.

At a pinch, you could circumvent this limitation in regard to the AR
specification by the trick of including lags of the dependent variable
in the ``exogenous'' list.  For example, the following command
\begin{code}
...
correct estimates, but would break the existing mechanism for
forecasting.
