...
Creating the zip file can be done ``by hand'':
\begin{code}
cd ..
zip mypkg/mypkg.zip mypkg/ mypkg/mypkg.gfn mypkg/somedata.gdt
\end{code}

Or you can have gretl take care of everything for you, by by using the
...
\end{code}
where the \texttt{-q} flag makes \textsf{gretlcli} operate quietly,
and \texttt{-b} tells the program to run in ``batch mode''
(non-interactively).
...
versus spaces. Consult the \app{make} documentation for more details.
...
shell, and it will be necessary to translate the unix-type shell
commands we used in section \ref{sec:cli-build} to their Windows
equivalents (e.g.\ \cmd{cp} becomes \cmd{copy}, \cmd{ls} becomes
\cmd{dir} and so on).
...
If you wish to use \app{make}, and/or run scripts such as those shown
in section~\ref{sec:publish}, you will probably have to install one
or other of these packages.
