...
holding a pull-down list of $n$ options. Such options can be regarded
as numbered from 1 to $n$ (regardless of how they look in the
GUI). Here are some examples of usage, where \texttt{bar} is the
parameter on which conditionality rests:
\begin{code}
# sensitive only if the second option for bar is selected
ret.baz = _(depends = "bar=2")
# sensitive if any option but the first is selected
ret.baz = _(depends = "bar!=1")
# sensitive if an option higher than 2 is selected
ret.baz = _(depends = "bar>2")
\end{code}
