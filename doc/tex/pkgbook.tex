...
optional second argument. In CLI mode the relevant \textsf{spec} files
would be:
...
\textit{Further reading}: For more on the special roles for packages
within functions see sections~\ref{sec:spec-gui} and
\ref{sec:spec-bundle}. For a discussion of how a real
...
in the sense that they naturally appear somewhere in gretl's main menu
tree.
...
actually do something with the model we're displaying?  REF
MODEL-REQUIREMENT
...
package. Parallel to the apparatus to identify a function as (e.g.) a
bundle-printer or bundle-plotter you can also mark a function as a
pre-checker, using the keyword \texttt{gui-precheck}, as in
...
question. Among these is the
\verb|$command| accessor, which gives the command-word for the
estimator. So, for example, the pre-check function for an addon which
targets OLS models might look like Code Example~\ref{code:precheck}.
...
The \textsf{bandplot} package creates a confidence band plot, that is,
a plot displaying a confidence band for the effect of a selected
regressor in the context of a multiple regression.  In GUI use, this
package latches onto windows displaying models estimated via OLS,
attaching itself to the \textsf{Graphs} menu.
...
Some addditional features helpful for this sort of thing. First,
besides the \texttt{[min:max:default]} fields for a function
parameter, you can add a fourth field to specify a ``step''. This is
used only for non-integer scalar parameters. To make the step value
...
Second, there's a facility specific to model-related packages, and in
...
directly; so we want a means of signaling to gretl that the list
...
gretl GUI with ``\texttt{gretl mypkg.gfn}''. No need to mess with File
Open dialogs since you're already in the directory where
\texttt{mypkg.gfn} is located.
