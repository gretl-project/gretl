...
motion at business-cycle frequencies from annual, quarterly or monthly
macroeconomic data, and in this case plausible lower and upper limits
for the pass band (minimum and maximum number of periods) can easily
be calculated based on the data frequency. However, the filter could
also be used on data observed at other frequencies; then it's not so
clear what the pass band should look like.
...
Second, let's take a look at the following (slightly simplified)
signature of the \texttt{gui-main} function for a package in
development:
\begin{code}
