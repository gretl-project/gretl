...
\item A text file containing help on the packaged function(s); call
  this either \texttt{mypkg\_help.md}, if you're using gretl's
  markdown option (recommended: see Section~\ref{sec:md} below), or
  \texttt{mypkg\_help.txt} if it's just plain text.
\end{enumerate}
...
Gretl's markdown variant currently supports the following featues.
...
hyperlinks.  Paired ASCII straight double quotes are replaced by left-
and right-hand quotes.
...
\item \texttt{ui-maker} (optional): This element allows a degree of
  fine-tuning of the dialog box shown when a package is called via
  gretl's graphical interface. At present the following points can be
  configured (more options may be added in future):
  \begin{itemize}
  ...
  \end{itemize}
  This is achieved by including a private function which returns a
  bundle, holding a sub-bundle for each parameter to be inflected.
  This is most easily explained by example. Here's the signature of
  the \texttt{gui-main} function for a package currently in
  development:
\begin{code}
...
\end{code}
  ...
  \texttt{X} are inflected. With \texttt{seed}, the \texttt{depends}
  ...
  The function designated as \texttt{ui-maker} is called internally
  when the relevant dialog box is to be shown, and on successful
  execution the directives in the returned bundle are followed.
...
\item \texttt{wants-data-access} (optional): In some cases your
  package may need to read data outside of the active sample
  range. This could be past initial values in a dynamic model, or it
  could be future values of exogenous regressors used for
  out-of-sample (conditional) forecasting.  But since a user-written
  hansl function is normally restricted to the incoming sample
  specified by the caller of the function, this would not be
  possible. In such situations, your package should specify this
  switch, and if upon execution the gretl user agrees, your relevant
  packaged function may use the \texttt{smpl} command to reset the
  active sample inside the function even beyond the incoming
  limits. (The original sample will be restored when exiting the
  function, as always.)
