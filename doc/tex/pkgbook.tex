...
facilities over recent months. If you plan to follow along you should
update to gretl 2015d or higher.
...
\texttt{tags} or \texttt{min-version}) see
section~\ref{sec:spec-basic}.

In fact, writing a \textsf{spec} from scratch may be tricky. Some may
prefer adapting a pre-existing template \textsf{spec} file; you can
find one at
\url{https://sourceforge.net/projects/gretl/files/misc/template.spec}.
Regardless, you still may want to refer to chapter \ref{chap:specfile}
for a fuller description of the various entries.
...
other place (Figure~\ref{fig:gfnsave}).
...
ASCII, which is a proper subset of UTF-8). We also recommend that
lines of text are kept to around 70 characters in width: some people
may like to run gretl windows at full-screen size, but many of us do
not!
...
has little to do with \textsf{DOS}---a long-obsolete 16-bit operating
...
Windows, \textsf{CygWin} and \textsf{MSYS}. Both are likely to take
...
\textit{lot} ($>$ 100 MB), does not include all of the utilities
you'll want. When running the \textsf{CygWin} setup program you have
the option of adding packages on top of the ``base'': if you want to
be able to do everything discussed in this chapter you'll need to
select \textsf{zip} and \textsf{unzip} from the \textsf{Archive}
package-set; \textsf{make} from the \textsf{Devel} set; and
\textsf{curl} from the \textsf{Net} set.

As for \textsf{MSYS}, the place to start is
\url{http://sf.net/projects/mingw/files/Installer/}; from
there you can download \app{mingw-get-setup.exe}. Run this to set up
the MinGW and MSYS installation manager. For our purposes here the
base install of MSYS is sufficient, except that you may want to add
the ``bin'' packages for \textsf{msys-zip} and \textsf{msys-unzip},
and possibly the ``ext'' package for \textsf{msys-coreutils}. An
installation with these extras weighs in at about 24 MB.  Note that
\textsf{curl} is not available as a specific MSYS package, but it's
pretty much an optional extra, and can be installed separately if you
want it; see \url{http://curl.haxx.se/download.html}.

We haven't given full details on running the installers for
\textsf{CygWin} or \textsf{MSYS}; If you're taking this route we
assume you know what you're doing, and these programs are reasonably
intuitive. Nonetheless we advise you to take it slowly and pay close
attention to (a) the installation path and (b) which packages you are
selecting for installation.

Once one or other of these packages is installed, you have access to a
terminal window running a bash shell. With \textsf{CygWin} this should
be apparent in the form of a desktop icon (unless you chose to disable
it). With \textsf{MSYS} you have to establish the desktop link
yourself: create a ``shortcut'' pointing at \texttt{msys.bat} in the
\textsf{MSYS} installation. If you followed the defaults this will be
at
%
\begin{code}
...
\end{code}
%
In this case you can customize the window that appears as described 
in section~\ref{sec:cmd.exe}.
...
or a unix-type shell is a very nice way of exploiting certain gretl
...
\item \texttt{tags} (required): Here you must specify at least one
  ...
  \url{http://ricardo.ecn.wfu.edu/gretl/cgi-bin/tags.cgi}.
...
\item \texttt{sample-script} (required): the name of a hansl
 ...
 \item \texttt{help} (required): The name of the file in which
  ...
  PDF file, in which case the suffix must be \texttt{.pdf} and
  the basename of the file must be the same as that of the package
  (as in \texttt{mypkg.gfn} and \texttt{mypkg.pdf}). 
...
\item \texttt{data-files} (optional): Specifies an extra file or
  subdirectory that should be included in the package. Use of this
  option implies that the package should take the form of a
  zipfile. If just one extra file is to be included you may give its
  name as the parameter to \texttt{data-files} (as in the example
  below); if several extra files are to be packaged, put them into a
  subdirectory and give its name as parameter. See
  chapter~\ref{chap:zipfile} for examples, and in particular
  section~\ref{sec:HIP} for the special meaning of including a
  subdirectory named \texttt{examples}.

  \ttusage{data-files = special.gdt}

\item \texttt{depends} (optional): Specifies one or more packages upon
  which the given package depends. This information should be added if
  the package calls functions that reside in other packages.  Use of
  this option ensures that when your package is loaded, any
  dependencies are also loaded.  For example, there's a package named
...
\item \texttt{provider} (optional): This should be used if your
  package requires access to the \textit{private} functions of another
  package, a particularly close form of dependency. Only one package
  can be specified in this way.  In the GUI this feature is
  represented by a check box against the first entry under the
  \textsf{Dependencies} tab. In the spec file, specifying a package as
  provider automatically includes it as a dependency.

  \ttusage{provider = extra}

\item \texttt{R-depends} (optional): This should be used if your
  package depends on \textsf{R} for its functionality. You should give
  an \textsf{R} version with which the package is known to work, along
  with the names and known-good version numbers of any \textsf{R}
  packages it requires. The example below might be suitable for a
  package that requires \textsf{R} and its \textsf{glmnet} package.

  \ttusage{R-depends = R 3.5.3 glmnet 2.0-18}

\item \texttt{wants-data-access} (optional): In some cases your package
  may need to read data outside of the active sample range. This could
  be past initial values in a dynamic model, or it could be future values
  of exogenous regressors used for out-of-sample (conditional) forecasting.
  But since a user-written hansl function is normally restricted to the
  incoming sample specified by the caller of the function, this would not
  be possible. In such situations, your package should specify this switch,
  and if upon execution the gretl user agrees, your relevant packaged function
  may use the \texttt{smpl} command to reset the active sample inside the
  function even beyond the incoming limits. (Of course, the original sample
  will be restored when exiting the function, as always.)

  \ttusage{wants-data-access = true}

\end{description}

In each case multiple elements should be separated by spaces.


\section{A note on gretl versioning}
\label{sec:versioning}

For some years now gretl version identifiers have taken the form
``year of release plus sequential letter'', as in \texttt{2018a}, \texttt{2018b} and so
on.  Prior to October 2015, however, a different scheme was in use:
version identifiers took the form \texttt{M.N.P}: three integers with
the interpretation \texttt{M} = ``major version'', \texttt{N} =
``minor version'', \texttt{P} = ``patch-level''.  For authors of
function packages the question arises: given this change, how should
you identify the minimal gretl version with which your function
package is compatible?

In a package \textsf{spec} file the best policy is to use the
``historical'' identifiers for gretl releases up to \texttt{1.10.2},
and the new style for gretl \texttt{2015d} onward. In the GUI package
editor only the new style is supported. The gretl ChangeLog
(\url{http://gretl.sourceforge.net/ChangeLog.html}) can be used to map
from historical identifiers to their new-style equivalents. For
example, the old-style version number \texttt{1.9.8} maps to
\texttt{2012a} (being the first release in 2012).

\chapter{Zip package details}
\label{chap:zipfile}

The zip format was originally supported only for officially recognized
``addons'' such as \textsf{gig} and \textsf{SVAR}, but since 2015 it
is also supported for contributed function packages.
