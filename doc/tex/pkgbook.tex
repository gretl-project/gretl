...
This is also easily and intuitively done via the GUI.  From the list
of locally-installed packages (choose ``Tools, Function packages, On
local machine,'' from the main menu, or simply go for the
``\textit{fx}'' button on the toolbar), select the package you want to
get rid of and click on the ``Unload/delete'' icon, or right-click to
the same effect.

You will be asked if you want to (a) just unload the package or (b)
remove it from your system. The former might be useful during an
interactive session in which you want to clear up all the functions
you have in memory and start from scratch with no possible confusion
between functions you have written and those provided by a
package. The latter is of course more radical, and requires you to
re-install the package if you change your mind.
...
package. Even if you prefer to use a graphical interface whenever
possible, we recommend that you at least skim this section before
...
OK, so you now know how to use the CLI for putting together a
\textsf{gfn} file. What if you want to distribute it together with
some extra material (say, a data file)?

The first thing is updating the \texttt{spec} file to that effect:
therefore, you'll want to add a line like
\begin{code}
data-files = somedata.gdt
\end{code}
to it. We assume that the \texttt{somedata.gdt} file exists and is in the
\texttt{mypkg} directory.

A possible way is to do this ``by hand'': 
\begin{code}
cd ..
...
you might find a Makefile helpful. It is not obligatory to use a
...
run Bib\TeX{}, then compile it again, etcetera.  The \app{make}
...
\item CygWin may be a nice idea: you get bash plus all sorts of extra
  goodies, including \cmd{make} and \cmd{curl}.
\end{itemize}
