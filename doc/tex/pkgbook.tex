...
commands. With the exception of subsection~\ref{sec:using-make} all
the commands shown in this section have been tested on Windows'
\app{cmd.exe} as well as the unix-type Bourne shell.
...
\begin{code}
echo makepkg mypkg.zip | gretlcli -q
\end{code}
where the \texttt{-q} flag makes \textsf{gretlcli} operate
(relatively) quietly.
...
In the first case you will presumably be using \app{cmd.exe} as your
shell. This supports the basic commands shown in
section~\ref{sec:cli-build} but it doesn't support \app{make}; if you
want to use that you will have to install it yourself. The simplest
way of doing this is to run the installer for GNU \app{make} from the
\textsf{GnuWin32} project on sourceforge: see
\url{http://gnuwin32.sf.net/packages/make.htm}.

Alternatively, there are two main tool-sets which give you a working
unix-like environment with Bourne-type shell:
\begin{description}
...
\item[MSYS] (\url{http://www.mingw.org/wiki/MSYS}): this is a minimal
  but consistent set of tools that will get the job done.
\end{description}

If you wish to run scripts such as those shown in
section~\ref{sec:publish}, you will probably have to install one or
other of these packages, unless perhaps you are very handy with
Windows batch (\texttt{.bat}) files.
...
In all cases, you will have to ensure that \cmd{gretlcli} is in your
\texttt{PATH}. Recent versions of the gretl installer will arrange
this automatically, but it can be done manually via \app{cmd.exe} like
this:
\begin{code}
set PATH=%PATH%;c:\program files\gretl
\end{code}
Similarly, if you want to use \app{make} from \textsf{GnuWin32}:
\begin{code}
set PATH=%PATH%;c:\program files\gnuwin32\bin
\end{code}
In each case you need to check for the specific path to add (e.g.\ the
correct programs directory might be ``\texttt{program files (x86)}'').
...
install: $(PKG).zip
	echo install $(PKG).zip --local | gretlcli -q > /dev/null
...
as in ``\texttt{make install}''.\footnote{There are two unix-specific
  idioms in this Makefile. If you were running \app{make} under
  \app{cmd.exe} on Windows you would have to make these substitutions:
  \texttt{NUL} for \texttt{/dev/null} under the zipfile and install
  targets, and \texttt{del} for \texttt{rm -f} under the clean
  target.}
...
so useful! So, for example, ``\texttt{make install}'' will not fail
