...
This chapter explains how to build a gretl function package using
...
We'll get to the independent upload method in a moment, but first a
word on building a zip package yourself, in case that is required.

An easy way of doing this is by using the \cmd{makepkg} command in the
gretl console or in \app{gretlcli}, as in
%
\begin{code}
makepkg mypkg.zip
\end{code}
...
\textit{Independent upload method}: This relies on the \app{curl}
program, which is standard kit on platforms other than MS
Windows.\footnote{For a Windows version of \textsf{curl} see
