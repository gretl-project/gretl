...
facilities since the release of gretl version 1.10.1. If you plan to
...
Select ``New package''.  In the first dialog to appear (see
Figure~\ref{fig:startpkg}) you must specify the name of the
package, one or more public functions to package, and 
zero or more ``private'' helper functions.

\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.5]{figures/newpkg}
  \caption{Starting a new function package}
  \label{fig:startpkg}
\end{figure}
...
window).  You have a further chance to edit the code of the
function(s) to be packaged, by clicking on \textsf{Edit function
  code}.  (If the package contains more than one function, a drop-down
...
Example~\ref{ex:bunret} shows a modification of our toy package to
...
Example~\ref{code:precheck} (it could be written a good deal more
...
\texttt{libraries} that are widely used by other programs, for reasons
...
on. (Not that we're devoted to copying anyone else, but this is how
\textsf{Matlab} currently identifies its releases.)
...
example 1.9.8 maps to \texttt{2012a} (being the first release in
2012).
