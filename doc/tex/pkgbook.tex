...
you'd expect to find it (Figure~\ref{fig:ghegy-hook}).
...
in general, see the \textit{Hansl Primer}, available in the gretl GUI
program under the Help menu \citep{hansl-primer}.
...
\item \texttt{menu-attachment} (optional): Specifies a place within
  ...
  gretl source package or in git.\footnote{See
    \url{https://sourceforge.net/p/gretl/git/ci/master/tree/gui/gretlmain.xml}
    for the main-window menus or
    \url{https://sourceforge.net/p/gretl/git/ci/master/tree/gui/gretlmodel.xml}
    for the model window menus.}
...
\item \texttt{model-requirement} (optional): When the ``gui-main''
  ...
  package will be shown only for such models).  The right-hand value
...
\item \texttt{must-assign} (optional): A list of public functions whose
  return value must be assigned by the caller, in GUI usage.  Putting a
  function in this list is a way of telling the user that it produces
  neither printed output nor a plot, so there's no point in calling it
  unless you mean to make use of the value it returns. If the user
  clicks \texttt{OK} in the function-call dialog without specifying
  assignment, an error is flagged. (But note that there's no way to stop
  the user from discarding a function's return value in command-line
  usage.)

  \ttusage{must-assign = \detokenize{silent_func}}
...
contains one or more keys corresponding to recognized directives. In
outline such a function should look like this:
...
The aspects of the GUI that a function writer can influence via
\texttt{ui-maker} fall into three categories: conditional sensitivity of
argument widgets, run-time determination of the minimum, maximum and/or
default values for arguments, and restrictions relating to list
arguments (lists of series, that is). We go into each of these
below. For concreteness, we'll assume that the \texttt{ui-maker} returns
a bundle named \texttt{ret}, as in the example above (though you're free
to choose the name yourself).

\subsection{Conditional sensitivity}

The widget for a given parameter---denoted by \textsf{W}$_i$, where $i$
indicates its place in the parameter list---can be made sensitive or
insensitive (``grayed out'') conditional on the state of the widget
representing a prior parameter, \textsf{W}$_j$, $j<i$. This is achieved
via a string under the key \texttt{depends} in the sub-bundle for
\textsf{W}$_i$.  In the simplest case this string is just the name of a
prior parameter. That works if the widget associated with the prior
parameter is
\begin{itemize}
\item a check box (boolean switch), or
\item a drop-down chooser pertaining to an \textit{optional} selection,
  with a default value of \texttt{null}.
\end{itemize}
Suppose we want the widget for a parameter \texttt{baz} to be shown as
sensitive conditional on the state of a prior parameter \texttt{foo}.
The following statement in the \texttt{ui-maker} will do the job:
\begin{code}
ret.baz = _(depends = "foo")
\end{code}
If the widget for \texttt{foo} is a check box, \texttt{baz} will be
sensitive if and only if that box is checked, while if it's a drop-down
chooser \texttt{baz} will be sensitive if and only if something other
than \texttt{null} is selected.

Alternatively, the \texttt{depends} string may take the form

\qquad \textsl{name}\,\textsl{op}\,\textsl{value}

where \textsl{name} is the name of parameter $j$, \textsl{value} is an
integer indicating the state of \textsf{W}$_j$ and \textsl{op} is a
comparison operator: ``\texttt{=}'' or ``\texttt{==}'' for equality;
``\texttt{!=}'' for inequality; ``\texttt{<}'' and ``\texttt{>}'' for
less than or greater than, respectively.

If the prior widget \textsf{W}$_j$ is a check box only the equality
comparator is supported, but this allows for inverse dependency. For
example, given
\begin{code}
...
The extended case also allows for dependency on a chooser offering a
fixed list of options \textit{without} a \texttt{null} option. Say
there's a parameter named \texttt{bar}, represented thus in the
signature of the function in question:
\begin{code}
...
ret.baz = _(depends = "bar<2")
\end{code}
Note that the \textsl{value} to the right of the operator refers to
...
For another example, say we have a integer parameter supporting a choice
of estimation method, as in this fragment from a possible function
signature:
\begin{code}
int method[1:3:1] {"ols", "gls", "arma"}
\end{code}
Method 3, \texttt{arma}, should presumably be shown only if the current
dataset is time series. To achieve that effect we could add this
statement to the bundle returned by the \texttt{ui-maker} function
\begin{code}
ret.method = _(maximum = "$datatype==2 ? 3 : 2")
\end{code}
since \texttt{\$datatype==2} means we have time-series data. If at run
time the dataset is not time series, only the \texttt{ols} and
\texttt{gls} options will be shown in the drop-down selection.

\subsection{Restrictions on series arguments}

A parameter calling for an individual series argument can be inflected
in either of two ways, via the following keys.

\begin{itemize}
\item \texttt{binary} is a boolean switch; give a value of 1 to insist
  that the series in question is a 0/1 dummy variable.
\item \texttt{discrete} is also a boolean. Give a value of 1 to insist
  that the argument is discrete. Note that the test in this case is
  somewhat more liberal than gretl's \texttt{isdiscrete()} function; it
  doesn't require that the series has been explicitly marked as
  discrete, and if it is not so marked the test succeeds provided the
  series has values that are divisible without remainder by 0.25 and
  does not have ``too many'' distinct values relative to the length of
  the series.
\end{itemize}

The following indicates that the series \texttt{y} must be binary and
series \texttt{z} must be discrete:
\begin{code}
ret.y = _(binary=1)
ret.z = _(discrete=1)
\end{code}
...
\subsection{Assignment names}

When a function offers some sort of return value (as most do), by
default an entry box is placed towards the bottom of its dialog box
(with accompanying text ``Assign return value'') where the user is
invited to type a name for this value. However, in certain cases a
suitable name for the return value may already be implicit.

For example, the \dtk{FRED_get} package---which supports downloading of
data from the \textsf{FRED} data system at the Federal Reserve Bank of
St.\ Louis---has as a required parameter the name of a \textsf{FRED}
series. The user may well wish to use the same name for the
series in gretl, but the interface includes an optional parameter to
allow specification of a different name. It then makes sense that the
name for assignment should be either the optional alternate name, if
that is given, or otherwise the \textsf{FRED} identifier---and the usual
assignment-name entry box is not wanted.

Suppose the required \textsf{FRED} series parameter is called
\texttt{seriesname} and the optional alternate name parameter
\texttt{outputname}. And suppose the package has a \texttt{ui-maker}
function which returns a bundle called \texttt{ret}. Then the following
lines in the \texttt{ui-maker} will do what's wanted:
\begin{code}
ret.outputname = _(auto_assign = 1)
ret.seriesname = _(auto_assign = 2)
\end{code}
The \dtk{auto_assign} keyword indicates that these two parameters are
candidates for supplying the name of the imported series on
assignment. The associated numbers indicate a preference ordering.
First priority goes to \texttt{outputname}: if something it typed into
its entry box, that will be used as the assignment name. Otherwise we
fall back on \texttt{seriesname}. The usual assigment entry box is
automatically suppressed.

For this mechanism to work there must be one \textit{required} parameter
with the \dtk{auto_assign} tag. This tag may be given to as many
optional parameters as makes sense.
...
\item \texttt{conflict}: When a new version of an existing function
  package is made available the name of the package usually remains
  unchanged, in which case installing the new version automatically
  overwrites the old one. But in some cases---for example, if the new
  version generalizes the functionality of the package---a change of
  name may be desirable. In that case installation of the new version
  should imply deletion of the prior version (if present). That can be
  arranged by specifying a conflict with the old name of the package,
  using this syntax:

  \ttusage{conflict = oldname}

  where \texttt{oldname} should of course be replaced by the prior name
  of the package.
