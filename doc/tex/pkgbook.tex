...
Figure~\ref{fig:startpkg} you must specify:
...
file to be deleted doesn't actually exist.\footnote{This rule contain
  ...
  you would have to substitute \texttt{del} for \texttt{rm -f}.}
...
\item \texttt{tags} (required): Here you must specify at least one
  ...
  one tag, the tags should be separated by spaces.
...
\chapter{Gretl versions}
\label{chap:gretl-versions}

As time passes this addendum should become redundant, but as of this
writing (in the last quarter of 2015) gretl is in transition between
...
For most users of gretl the change in versioning scheme should be of
no consequence, but for authors of function packages the question
arises: how should I identify the minimal gretl version with which my
function package is compatible?
