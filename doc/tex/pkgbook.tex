...
command-words corresponding to the various estimators (\cmd{ols},
\cmd{logit}, \cmd{mle} and so on). So for a function specifically
designed to handle logit models one could specify
\begin{verbatim}
...
targets binary logit and probit models would look like
Example~\ref{code:precheck}.
...
function scalar lpbin_precheck (void)
  string c = $command
  if c != "logit" && c != "probit"
    # can't handle estimator
    return 1
  elif !isdummy($ylist[1])
    # non-binary, can't handle it
    return 1
  endif
  return 0
end function
...
For a simple but idiomatic example of a model-related package, take a
look at \textsf{bandplot}, which creates a a plot displaying a
confidence band for the effect of a selected regressor in the context
of a multiple regression.  In GUI use, this package latches onto
...
Some additional features helpful for this sort of thing. First,
