...
the dummy series you want to create (but keep reading --- the
``proper'' solution is probably what you want if you have more than
one or two).
...
The best solution, both in terms of computational efficiency and code
clarity would be using a ``conversion table'' and the \cmd{replace}
function, after which you can use the \cmd{dummify} command. However,
this would entail generating a matrix with a complete list of
corresponding values from the finer grid to the coarser one: in some
cases, this could be just a waste of time and energy, so the quicker
hack above may be preferable.
...
\cmd{qrdecomp()}. The function below defines a user-level function
