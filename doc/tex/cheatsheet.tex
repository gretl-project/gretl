...
strings state_codes = strsplit(readfile("markers.txt"))
genr index
...
markers to a plain text file, one per line. We then read that file
using \texttt{readfile()} and split its content into an array of
strings using \texttt{strsplit()}. The command \texttt{genr index}
