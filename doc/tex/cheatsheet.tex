...
\emph{Problem:} You have a panel dataset (comprising observations of
$n$ indidivuals in each of $T$ periods) and you want to transform it
by lowering the time frequency by averaging. This is commonly done in
growth empirical economics, where units are countries and yearly data
are turned into 3- or 4- or 5-year averages (see \emph{eg}
\cite{Islam1995}).

\emph{Solution:} In a panel dataset, \app{gretl} functions that deal
...
Besides: it has been brought to our attention that some mischievous
...
\emph{Problem:} \app{Gretl} provides native functions for moving
...
\emph{Problem:} I'd like to generate a bunch of data whose sample
covariance matrix is exactly equal to a given matrix $\Sigma$. How can
I do this in \app{gretl}?

\emph{Solution:} The Cholesky decomposition is your friend. Try this:
\begin{code}
S = {2,1;1,1}
T = 10
X = mnormal(T, 2)
...
\cmd{filter} is quicker and more natural.
