...
\texttt{bundle}. As we've just seen, string variables are used to
...
unwanted output. See chapter \ref{chap:settings} or the \GCR{} for
more on \texttt{set}.
...
long string and then operating on that;\footnote{Of course, the reader
  is invited to take this with a pinch of salt. If your job involves
  ...
  hansl may be a defensible choice for text files up to a few hundred
  megabytes.} interested readers should take a look at the
\cmd{readfile}, \cmd{getline} and \cmd{regsub} functions in the \GCR.
...
\ref{chap:formatting}. Finally, it is quite common to use \emph{string
