...
\texttt{m3B}, where \texttt{m}$ij$ represents the quantity of
component $j$ ($j=$ \texttt{A}, \texttt{B}) produced by machines of
type $i$ ($i=1,2,3$). The \textsf{lpsolve} script for this problem was
shown in Figure~\ref{fig:kantor1}; we reproduce it in
...
m2A + m2B <= 3; // 3 machines of type 2
m3A + m3B <= 1; // 1 machine of type 3
