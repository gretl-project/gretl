...
in hansl. With one or more \cmd{elif}s, hansl will execute the topmost
...
decide if a group of statements must be executed or not; hansl's
\cmd{if} is what Stata calls this the ``branching \texttt{if}''.}
...
\texttt{result} will contain \texttt{<false-value>}.\footnote{Some
  ...
  operator works exactly the same way as the \texttt{=IF()} function
...
\item otherwise, we compute a matrix of zeros and ones via the
  \verb|m.={4,6,9,11}| operation (note the usage of the ``dot''
  operator to perform an element-by element comparison --- see section
  \ref{sec:mat-op}); if \texttt{m} equals any of the elements in the
  vector, the corresponding element of the result will be one, and
  zero otherwise;
...
\tip{At this point, it's likely that some reader would think ``Oh,
  what the heck, this may be as cool as you want, but it's way too
  complicated for me; I'll just use the traditional \cmd{if}''. Of
  course, there's nothing wrong with this, but in some cases the
  ternary assignment operator can lead to substantially faster code,
  and comes surprisingly natural when one gets used to it.}
...
loop, the expression \dollar{i} will be substituted by the hansl
interpreter with the string representation of the current value of the
index variable. An example should made this clearer: the following
input
\begin{code}
scalar a_1 = 57
scalar a_2 = 85
