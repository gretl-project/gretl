...
\tditem[opts](bundle, optional): a bundle holding additional
  ...
  \begin{description}
  \tditem[aggtype](string): Specifies the type of temporal aggregation
  appropriate to the series in question. The value must be one of
  \texttt{avg} (each low-frequency value is the average of $s$ high
  frequency values, the default); \texttt{sum} (each low-frequency
  value is a sum of $s$ high-frequency values); or \texttt{last} or
  \texttt{first}, indicating respectively that each low-frequency
  value is the last or first of $s$ high frequency values.
