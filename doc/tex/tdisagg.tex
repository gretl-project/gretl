...
matrix form alignment is truly the user's responsibility. But if both
...
values, and automatically retard the end of the sample ranges for
$\Xb$ and $\Yb$ to exclude trailing missing values (allowing for the
possibility that $\Xb$ may extend beyond $\Yb$). In addition we
further advance the sample start if this is required to ensure that
the $\Xb$ data begin in the first high-frequency sub-period (e.g.\ the
first quarter of a year or the first month of a quarter). But please
note: such automatic alignment is \textit{not} currently enabled when
$\Xb$ takes the form of a MIDAS list; and even when gretl
