...
within gretl and adding string values using the \texttt{stringify()}
...
\begin{itemize}
\item By default the \texttt{print} command shows us the string values
  ...
  very elegantly).
\item The \verb|--numeric| option to \texttt{print} exposes the
...
This is done via the \texttt{stringify()} function, which takes two
...
One aspect of \texttt{stringify()} is debatable. At present the
...
adjust the threshold values. Note the use of \texttt{strsplit()} to
...
operation of \texttt{stringify()}: the \texttt{strvals()} function
...
More likely, one would want to use \texttt{dummify} on a string-valued
...
sources and we'll create them natively via \texttt{stringify}---and
...
depending on how it's defined; you can use \texttt{dummify} if you
need it.
