...
The operators \textit{not} accepted for complex values are:
\begin{itemize}
...
functions (or commands) accept complex input you can calculate the
parameter estimates for a regression of complex \texttt{Y}
($T \times 1$) on complex \texttt{X} ($T \times k$) via
\verb|B = X \ Y|.
