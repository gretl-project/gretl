...
If you're reading this, you're probably thinking ``Do I really need to
go through the whole thing? After all, I've been coding econometric
stuff for a while, and I'm pretty confident I can work my way
through. I just need to get started and, perhaps, a few tips''. Well,
you may be right. In this case, we'll just give you a one-page hansl
script which exemplifies a hefty share of the topics we'll go through
in the rest of this manual. We will use, for our example, a Vector
AutoRegressive model, or VAR for short.

As you probably know, a finite-order VAR can be estimated via
conditional maximum likelihood simply by repeatedly applying OLS to
each equation. That amounts to solving a least-squares problem and its
solution can be easily written, in matrix notation, as $\hat{\Pi} =
(X'X)^{-1} X'Y$, where $Y$ contains your endogenous variables and $X$
contains their lags plus other exogenous series (typically, a constant
...
these actions:
\begin{enumerate}
\item Read data from a disk file.
\item Perform some data transformation and simple visualization.
\item Estimates the VAR via 
  \begin{enumerate}
  \item the native hansl command
  \item iterated OLS
  \item matrix algebra (in 3 different ways)
  \item numerical maximization the log-likelihood.
  \end{enumerate}
\item Print out the results.
\end{enumerate}
...
loop foreach i X                       # loop over the 3 var equations
    ...
    B ~= $coeff                        # and store estimated coefficients
end loop                               # as matrix columns
...
document (but we don't mind if you do). If, instead, you find this too
scary, never mind: we'll take things step by step. Read on.
