...
in relation to such systems. We do not offer a \dollar{vcv} accessor,
but instead accessors named \dollar{xtxinv} (the matrix $X'X^{-1}$ for
the system as a whole) and \dollar{sigma} (an estimate of the
...
VARs and VECMs, \dollar{ncoeff} gives the number of coefficients per
equation.)
