...
reported by \app{gretl} as \texttt{cut1}, \texttt{cut2} and so on. For
...
\texttt{wtp.inp}, provided with the \app{gretl} distribution.
...
which
\begin{eqnarray*}
  P(y_i | x_i) & = & e^{\mu_i} \frac{\mu_i^Y}{Y!} \\
  \mu_i & = & \exp\left( \sum_j x_{ij} \beta_j \right)
\end{eqnarray*}

\begin{itemize}
\item Estimation is, again, typically carried out by maximum
  likelihood. \app{Gretl} command: \texttt{poisson depvar indep}.
\item In some cases, an ``offset'' variable is needed. The number of
  occurrences of $y_i$ in a given time is assumed to be strictly
  proportional to the offset variable $n_i$. In the epidemiology
  literature, the offset is known as ``population at risk''. Syntax:
  \texttt{poisson depvar indep ; offset}.
\item The Poisson model has the peculiar feature that $E(y_i | x_i) =
  V(y_i | x_i) = \mu_i$; this may be at odds with the data
  (overdirspersion). Overdispersion can be attributed to individual
  heterogeneity.
\item Standard solution to the overdispersion problem: use another
  probability distribution. Gamma mixture leads to
  \begin{equation}
    \label{eq:negbin2}
  P(y_i | x_i) = 
  \frac{\Gamma(y_i + \alpha)}{\Gamma(\alpha)\Gamma(y_i + 1)}
  \left[ \frac{\mu_i} {\mu_i + \alpha }\right]^{y_i}
  \left[ \frac{\alpha} {\mu_i + \alpha }\right]^{\alpha}
\end{equation}
which is known as the ``Negative Binomial Model''. The parameter
$\alpha$ is a positive real number, which measures the degree of
heterogeneity between indviduals. The conditional mean is still $E(y_i
| x_i) = \mu_i$, but the variance equals $V(y_i | x_i) = \mu_i \left(
  1 + \mu_i/\alpha \right)$. The \app{gretl} command for this model is
\texttt{negbin depvar indep}.
\item There is also a less used variant of the negative binomial
  ...
  \eqref{eq:negbin2} is termed ``Type 2''.  \app{Gretl} implements
