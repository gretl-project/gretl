...
\app{Gretl} provides FE logit as a function package\footnote{Although
  this may change in the near future.} and RE probit
natively. provided your dataset has a panel structure, the latter
option can be obtained by adding the \option{random} option to the
\cmd{probit} command:
\begin{code}
  probit depvar const indvar1 indvar2 --random
\end{code}
as exemplified in the \texttt{reprobit.inp} sample script. The numeric
technique used for this particular estimator is \emph{Gauss-Hermite
  quadrature}, which we'll now briefly describe. Generalize equation
\eqref{eq:qr-latent} to a panel context, that is

\begin{equation}
  y^*_{i,t} = \sum_{j=1}^k x_{ijt} \beta_j + \alpha_i + \varepsilon_{i,t} = z_{i,t}  +
  \omega_{i,t} 
\end{equation}
in which you assume that the individual effect $\alpha_i$ and the
disturbance term $\varepsilon_{i,t}$ are both zero-mean Gaussian
random variables, independent of each other. The composite error term
$\omega_{i,t} = \alpha_i + \varepsilon_{i,t}$, therefore, is a normal
...
not independent; hence, the likelihood has to be evaluated on a
per-unit basis as
\[
...
However, the above probability \emph{can} be written as a product if
we treated $\alpha_i$ as a constant: in this case we would have that
\[
...
\option{quadpoints=\emph{n}} option, as in
\begin{code}
