...
\begin{script}[htbp]
  \scriptcaption{Estimation of simple logit and probit models}
  \label{simple-QR}
\begin{scode}
open greene19_1
...
economics is evaluated by the improvements of students' grades.  The
dependent variable, \texttt{GRADE}, equals 1 if a student's grade
increased over a certain period, 0 otherwise. The independent
...
initial Grade Point Average (\texttt{GPA}) and score on an economics
test (\texttt{TUCE}).

Running the code in Listing~\ref{simple-QR} gives the output reported
...
\begin{script}[htbp]
  \scriptcaption{Variable addition test in a probit model}
  \label{QR-add}
\begin{scode}
open greene19_1
...
\begin{script}[htbp]
  \scriptcaption{Ordered probit model}
  \label{ex:oprobit}
\begin{scodebit}
/*
...
\begin{script}[ht]
  \scriptcaption{Interval model on artificial data}
  \label{ex:interval}
Input:
\begin{scodebit}
nulldata 100
...
of $y^*_i$ by summing this estimate to $x_i \hat{\beta}$. Listing
...
\begin{script}[htbp]
  \scriptcaption{Models for recidivism data}
  \label{ex:duration1}
Input:
\begin{scodebit}
open recid.gdt
...
\begin{script}[htbp]
  \scriptcaption{Create plots showing conditional hazards}
