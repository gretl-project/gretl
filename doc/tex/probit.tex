...
dependent variable is the time until they return to prison. The
information was gathered retrospectively by looking at records in
April 1984; the maximum possible length of observation is therefore 81
months.  Right-censoring is rather important: as of April 1984 about
62 percent had not returned to prison.  The dataset contains several
covariates, which are described in the data file; here we will focus on
interpretation of the \texttt{married} variable, a dummy which equals
1 if the respondent was married when imprisoned.

Example~\ref{ex:duration1} shows the \app{gretl} commands for
a Weibull model along with 
