...
Wooldridge (2002a), and it concerns recidivism rates.  The data
(filename \texttt{recid.gdt}) pertain to a sample of 1,445 convicts
released from prison between July 1, 1977 and June 30, 1978. The
dependent variable is the time in months until they return to
prison. The information was gathered retrospectively by looking at
records in April 1984; the maximum possible length of observation is
therefore 81 months.  Right-censoring is important: as of April 1984
about 62 percent had not returned to prison.  The dataset contains
several covariates, which are described in the data file; we will
focus below on interpretation of the \texttt{married} variable, a
...
0.048.  Recall that $\sigma$ corresponds to $1/\alpha$; we can
therefore be confident that $\alpha$ is less than 1, so that
recidivism displays negative duration dependence.  This makes sense:
it is plausible that if a past offender manages to stay out of trouble
for an extended period his risk of engaging in crime again
diminishes. The exponential model would therefore not be appropriate
in this case.
...
the 5th percentile for \texttt{durat} is 6 months. Perhaps the
log-normal specification, which would allow the hazard to rise and
then fall, might be more appropriate.  Using the \texttt{duration}
command again with the same covariates but the \verb|--lognormal|
flag, we get a log-likelihood of $-1597$ as against $-1633$ for the
