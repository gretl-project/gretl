...
\item The Poisson model has the peculiar feature that $E(y_i | x_i) =
  V(y_i | x_i)$; this may be at odds with the data
...
heterogeneity between indviduals. \app{Gretl} command: \texttt{negbin
  depvar indep}.
\item There is also a less used variant of the negative binomial
  model. To distinguish between the two, the model \eqref{eq:negbin2}
  is termed ``Type 2''.  \app{Gretl} implements model 1 via
  \texttt{negbin depvar indep} \option{model1}.
\end{itemize}
