...
phenomenon is called ``overdirspersion''). The output from the
\texttt{poisson} command automatically includes a conditional moment
...
$x_i = x_j$ may have different means because of some unobserved scale
factor $s_i \ne s_j$ so that
\[
   E(y_i | x_i, s_i) = \mu_i s_i \ne \mu_j s_j = E(y_j | x_j, s_j)
\]
even though $\mu_i = \mu_j$. It is often assumed that $s_i$ can be
represented as a gamma random variable with mean 1 and variance
$\alpha$: the parameter $\alpha$ is estimated together with the vector
$\beta$, and measures the degree of heterogeneity between indviduals.
...
x_i) = \mu_i \left( 1 + \mu_i \alpha \right)$. The \app{gretl} command
...
\item A script which exemplifies the above models is included among
  \app{gretl}'s sample scripts, under the name \texttt{camtriv.inp}.
\end{itemize}
