...
If $\rho$ was 0, then ML estimation of the parameters $\beta_j$ and
...
estimation via numerical optimization by the Newton-Raphson method
with analytical derivatives. The command takes three mandatory
arguments and an optional one: the first three arguments are two
series names for $y_{1,i}$ and $y_{2,i}$ and a list of explanatory
variables for $x_{i}$. This covers the common case when the regressors
of the two equations coincide; if, on the contrary, the list $z_{i}$
is different from $x_i$, the the following syntax may be used:
\begin{code}
  biprobit y1 y2 X ; Z
\end{code}
After successful completion of the command, an LR test for the
hypothesis $\rho = 0$ will be printed.  An example is provided in the
\texttt{biprobit.inp} sample script.

Note that, since what is estimated is a two-equation system, the
\dollar{uhat} and \dollar{yhat} accessors will return two
matrices. The former is a two-column matrix containing the generalized
residuals. The \dollar{yhat} accessor, instead, will contain four
columns, with the estimated probabilities for
$(y_{1,i}, y_{1,i}) = (1,1)$ in column 1, $(y_{1,i}, y_{2,i}) = (1,0)$
in column 2, $(y_{1,i}, y_{2,i}) = (0,1)$ in column 3 and
$(y_{1,i}, y_{2,i}) = (0,0)$ in column 4.
