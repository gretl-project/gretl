...
\texttt{biprobit.inp} sample script.  The command takes three
mandatory arguments and an optional one. The first three arguments are
two series names for $y_{1}$ and $y_{2}$ and a list of explanatory
variables for $x$. In the common case when the regressors are the same
for the two equations this is sufficient, but if $z$ differs from $x$
the following syntax should be used:
\begin{code}
  biprobit y1 y2 X ; Z
\end{code}
Output from estimation includes a Likelihood Ratio test for the
hypothesis $\rho = 0$. This can be retrieved in the form of a bundle
named \texttt{independence\_test} under the \dollar{model} accessor,
as in
\begin{code}
