...
The dayspan function can also be helpful if you wish to construct a
suitably sized ``empty'' daily dataset prior to importing data from a
third-party database (for example, stock prices from Yahoo). Say
the data to be imported are on a 5-day week and you want the range to
be from 2000-01-03 (the first weekday in 2000) to 2020-12-30 (a
Wednesday). How many observations should be allocated?
\begin{code}
...
and an epoch day (see Section \ref{sec:cal-epochday}). This is
...
series tstr = strftime(usecs, "%Y-%m-%d %H:%M:%S")
print tstr -o
\end{code}
