...
report the time of an event in the local time of the event; and if we
do otherwise we need to state the metric we're using (for example, one
might say that this event occurred at 2001-09-11 12:46 UTC).
...
\begin{script}[htbp]
  \fragcaption{Enabling invariance with respect to current time zone}
  \label{ex:tzones}
\begin{scodebit}
...
\end{scodebit}
  ...
  Unix time value will be 1000212360. Then we come to unpacking the
  Unix time value:
\begin{scodebit}
...
\end{scodebit}
  ...
  offered by gretl are supported by \textsf{GLib} functionality.
\end{script}
