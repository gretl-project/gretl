...
\texttt{d}=30, \texttt{m}=11 and \texttt{y}=1983. You can use \cmd{printf}
...
function (see the \GCR{} for a full illustation). The
string \texttt{s}=``1983-11-30'' can be broken down into three scalars
as
\begin{code}
  scalar d m y
  n = sscanf(s, "%d-%d-%d", y, m, d)
\end{code}
Note that in this case the \texttt{\%d} string in nthe format
