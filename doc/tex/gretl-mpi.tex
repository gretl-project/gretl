...
or higher recommended) for MS Windows, macOS, or Debian-based Linux
distributions. On non-Debian Linux you may have to build
gretl yourself.\footnote{To see if the gretl build supplied by a
  distro is MPI-enabled, check for the presence of \texttt{gretlmpi}
  in \texttt{/usr/bin}.} In each case you will need to have a suitable
MPI package installed. The details differ by platform and are given
in section~\ref{sec:platforms} below.
...
be worth outlining some of the basic ideas they implement.
...
On Debian-based systems gretl should be MPI-enabled by default
As mentioned above, on other Linux systems you may have to build gretl
yourself (either from a released source package or from the git
sources). The tell-tale is whether or not the \texttt{gretlmpi}
executable is installed, and the command ``\texttt{which gretlmpi}''
in a terminal window should tell you that.

If the gretl package provided by your distro includes
\texttt{gretlmpi} you just have to ensure that MPI itself is
installed. Here are three variants of the required command:
\begin{code}
...
pacman -S openmpi
\end{code}
And the rest of this section needn't concern you.
...
\url{https://www.microsoft.com/download/details.aspx?id=100593}
...
MPI support is enabled only in the ``quartz'' (Intel 64-bit) build of
gretl. This build includes \texttt{gretlmpi}, linked against version
1.6.5 of \textsf{Open MPI}, which is assumed to be installed under
...
To unpack the package you would do, in \textsf{Terminal},
\begin{code}
cd /
