...
By default we assume that the random sequences generated by multiple MPI
processes should be mutually independent. With gretl's current RNG,
\textsf{xoshiro256+}, that is easily accomplished: at the start-up of
MPI, process 0 obtains a seed and broadcasts it to the other processes,
each of which calls \textsf{xoshiro}'s \texttt{jump()} function, which
advances the RNG by $2^{128}$ steps. Process 1 calls \texttt{jump} once,
process 2 calls it twice, and so on. This means that the RNG for each
process has an effective period of
$2^{128} \approx 3.403 \times 10^{38}$ without any overlap with other
processes.

If for some reason you want each MPI process to generate the
\textit{same} sequence of pseudo-random variates that can be arranged.
If you're running \texttt{gretlmpi} yourself, you can use the
command-line option \verb|--single-rng|. This flag can also be attached
to the start or end of an \texttt{mpi} command block, with the same
effect. Listing~\ref{script:rng} illustrates. As written, it will print
$n$ identical matrices, but if you delete \option{single-rng} the
matrices will all be different.
...
It's also possible to switch to a single RNG in the course of an
MPI-enabled script: just put a \texttt{set seed} command, with a
specific seed, in a common portion of the script, where it will be
...
and gretl builds for Windows have supported this since 2017. When you
run the \textsf{MS-MPI} installer this automatically adds the relevant
directory to your path, and you should be good to go.  Note, however
that if you're running MPI on multiple machines (or even if you're just
running on a single machine but are using a hosts file) you may have to
start the ``daemon'' program \texttt{smpd.exe} first. You can do that by
going to the Windows Start menu, selecting \textsf{Run\dots{}} and
typing
\begin{code}
smpd -d
\end{code}
...
The situation with Windows on Arm is a little different. In this case
basic \textsf{MS-MPI} kit, as ported to ARM64 by Anthony Roberts, is
included with the gretl installer. That's sufficient to run multiple
processes on a single multi-core machine. If you need more than that,
you can create a Feature Request at
\url{https://sourceforge.net/projects/gretl/}.
...
The rank 0 process first prepares the gamma data, then broadcasts the
initial parameter vector (which we deliberately make inexact so that
