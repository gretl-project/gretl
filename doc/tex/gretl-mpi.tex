...
Right now you can make use of MPI in gretl only by building gretl from
the git sources yourself or using a current snapshot (on Windows or OS
X). You will also need to have a suitable MPI package installed.

Running an MPI-enabled program---such as the new
\texttt{gretlmpi}, described below---requires a special launcher,
\texttt{mpiexec}.  This is supplied by MPI packages, the best known of
which are \textsf{Open MPI}, \textsf{MPICH} and Microsoft's
\textsf{MS-MPI}. See section~\ref{sec:platforms} for
details.\footnote{MPI-enabled programs are compiled with a special
  compiler-wrapper, \texttt{mpicc}, which is supplied by both
...
\end{tabular}
\end{center}
By ``object'' above we mean (at present) either a matrix or a
scalar.\footnote{A possible future extension is to support the passing
  of bundles between processes.} Note that the scalar return value
from these functions (apart from \texttt{mpirecv}, which returns a
matrix or scalar depending on the context) is merely nominal: they
return 0 if they succeed. The \texttt{root} argument to
\texttt{mpibcast}, \texttt{mpireduce} and \texttt{mpiscatter} is
optional, and defaults to 0; the use of this argument is discussed
below.
...
will eventually be handled by the Linux distributions.)
...
The current gretl snapshots for Windows (both 32- and 64-bit) provide
support for this.\footnote{In principle you could install \textsf{Open
    MPI} or \textsf{MPICH} on Windows---though you'd have to build
  them yourself---but at present gretl on Windows only supports
  \textsf{MS-MPI}.}
...
smpd -d
\end{code}
(This opens a console window, which you can just minimize.)
...
The quartz snapshot includes a build of \texttt{gretlmpi} which is
linked against the MPI libraries supplied with OS X 10.6 (Snow
Leopard)---namely, \textsf{Open MPI} version 1.2.8. Gretl/MPI therefore
works ``out of the box'' on Snow Leopard.

\vspace{4pt}

As of OS X 10.7, however, Apple discontinued its support for MPI. So
if you're running Lion or higher and want to activate the MPI features
in the gretl snapshot you will have to install MPI yourself, if you
haven't already done so.

\subsubsection*{Lion and higher, MPI not installed}

The case where you don't have any MPI installation on your Mac should
be fairly simple. You just need to install an MPI version that is
compatible with 1.2.8. The last \textsf{Open MPI} release in the 1.2
series was 1.2.9. Presumably this differs from 1.2.8 only in respect
of bug fixes, so it's a good candidate. We recommend installing the
64-bit binary package of \textsf{Open MPI} 1.2.9 available from the
``OSX testing'' area on the gretl website at
  
\url{http://sourceforge.net/projects/gretl/files/osx-testing/}

The file is called \texttt{openmpi-1.2.9-mac64.tar.bz2}. To unpack it
you would do
\begin{code}
cd /
sudo tar xvfj /where/you/put/openmpi-1.2.9-mac64.tar.bz2
\end{code}
with ``\texttt{/where/you/put}'' replaced appropriately, of
course. This will install the libraries in \texttt{/usr/lib}, where
\texttt{gretlmpi} expects to find them. Hopefully you should then
be good to go.

If you prefer to build and install MPI yourself, you should probably
choose version 1.2.9, and should configure the build with
\verb|--prefix=/usr|.

\subsubsection*{Lion and higher, MPI already installed}

If you're running Lion or higher and already have MPI installed there
are three sub-cases to consider.
\begin{itemize}
\item The installed MPI version is compatible with 1.2.8, and the
  MPI libaries are in \texttt{/usr/lib}. Fine, you should be good to
  go.
\item The installed MPI version is compatible with 1.2.8 but the
  libraries are somewhere other than \texttt{/usr/lib}. Then you
  should be able to use Apple's \verb|install_name_tool| on
  \texttt{gretlmpi} to re-target the links to
  \texttt{libmpi.0.dylib}, \texttt{libopen-rte.0.dylib} and
  \texttt{libopen-pal.0.dylib}.
\item The installed MPI version is not compatible with
  1.2.8. (Unfortunately this is quite likely, since 1.2 is pretty old
  and if you were installing MPI independently you'd probably choose a
  more current version.) In this case you can't use
  \texttt{gretlmpi} from the snapshot---or not without attempting
  the potentially risky and confusing step of installing two MPI
  version in parallel. However, there remain two possibilities: build
  gretl yourself (so that \texttt{gretlmpi} gets linked to the
  right libraries), or post to the \texttt{gretl-users} mailing list,
  noting your \textsf{Open MPI} version. If there's enough interest,
  we could perhaps prepare variants of \texttt{gretlmpi} linked
  against more recent versions of MPI and make them available as
  drop-in replacements for the program in the snapshot.
\end{itemize}
