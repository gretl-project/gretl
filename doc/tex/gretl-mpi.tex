...
\texttt{gretlmpi} is linked against \textsf{Open MPI} version 1.6.5,
while for Macs with arm64 processors (``Apple Silicon'') the linkage is
to \textsf{Open MPI} 4.1.5. To activate MPI support it's necessary to
install the appropriate package, as follows:

\begin{center}
\begin{tabular}{ll}
  intel & \texttt{openmpi-1.6.5-mac-intel.tar.xz} \\
  arm64 & \texttt{openmpi-4.1.5-mac-arm64.tar.xz}
\end{tabular}
\end{center}
...
To unpack the package you execute the following commands in a
\textsf{Terminal} window:
\begin{code}
# change to root directory
cd /
# for Intel
sudo tar xvf /where/you/put/openmpi-1.6.5-mac-intel.tar.xz
# OR for arm64
sudo tar xvf /where/you/put/openmpi-4.1.5-mac-arm64.tar.xz
\end{code}
%
