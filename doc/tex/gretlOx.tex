...
\tip{To enable support for \app{Ox}, go to the
  Tools/Preferences/General menu item and check the item labeled
  ``Enable Ox support''.  A notification box will appear, telling you
  that this requires restarting \app{gretl}.  Close that box, click
  ``OK'' in the preferences dialog, then quit and restart \app{gretl}.
  You will now find, under the Programs tab in the
  Tools/Preferences/General dialog, an entry for specifying the path
  to the \texttt{oxl} executable (i.e., the program that runs \app{Ox}
  files (on MS Windows it is called \texttt{oxl.exe}).  Make sure that
  is right, and you're ready to go.}
