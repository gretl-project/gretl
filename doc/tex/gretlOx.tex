...
with a few differences.  You can open and edit \app{Ox} programs in
the \app{gretl} GUI.  Clicking the ``execute'' icon in the editor
window will send your code to \app{Ox} for execution.
...
\verb|gretl_export| on the \app{Ox} side and mread on the \app{gretl}
side.  The \verb|gretl_export| function takes two arguments, a
...
portion of the output from Example~\ref{Ox-DPD} is shown below:
%
\begin{code}
