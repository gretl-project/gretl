...
of these relate to Ramanathan (2002), but they may also be used as a
...
Format).  Here is how to do it:\footnote{The model table can also be
  ...
  this, see the entry for \cmd{modeltab} in the \emph{Gretl Command
    Reference}.  }
...
output.\footnote{For PDF output you need \app{pdflatex} and either
  Adobe's PDF reader or \app{xpdf} on X11.  For PostScript, you must
  have \app{dvips} and \app{ghostscript} installed, along with a
  viewer such as \app{gv}, \app{ggv} or \app{kghostview}. The default
  viewer for systems other than MS Windows is \app{gv}.}
...
``Programs'' tab in the Preferences dialog box (under the ``Tools''
