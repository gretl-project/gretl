...
the Levenberg--Marquandt algorithm.  The user must supply a
...
parameters.  The tolerance (criterion for terminating the iterative
estimation procedure) can be adjusted using the \cmd{set} command.
...
for the parameter estimates.  If the \verb|--robust| flag is given a
...
OLS, and in light of the reasoning given in Davidson and MacKinnon
(1993), the estimates shown in \app{gretl} \emph{do} use a degrees of
freedom correction.
