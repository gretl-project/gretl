...
#include <libgnomeprint/gnome-print.h>
	  ...
	  echo "***"
        ...
        [ echo "*** The test program failed to compile or link. See config.log for the"
          echo "*** exact error that occured. This usually means GNOME-PRINT was incorrectly"
          echo "*** installed or that you have moved GNOME-PRINT since it was installed."
          echo "*** In the latter case, you may want to edit the gnome-config script:"
          echo "*** $GNOME_CONFIG" ])
          CFLAGS="$ac_save_CFLAGS"
          LIBS="$ac_save_LIBS"
       fi
